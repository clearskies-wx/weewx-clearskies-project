...
Bathymetry data for SWAN grids is accessed through the `bathymetry_resolver` service (`services/bathymetry_resolver.py`) which implements a priority chain: operator-supplied file → NCEI regional coastal DEM (OPeNDAP) → USGS Great Lakes DEM (GeoTIFF) → CRM/DEM_all fallback. The resolver uses a static JSON index of 199 NCEI regional DEMs (`data/ncei_regional_dem_index.json`, built offline by `scripts/build_ncei_dem_index.py`). Vertical datum consistency is enforced by matching the CO-OPS tide prediction datum to the bathymetry DEM's native datum (ADR-098). No local datum conversion for the common case. `swan.py`'s `download_bathymetry_for_level(domain, level, allow_download=...)` consults the resolver before falling back to the legacy CRM `getSamples` endpoint — but only when `allow_download=True`, the apply-time chain's default. **At SWAN runtime (T4A.3) `allow_download=False`**: a missing cache is an ERROR and that level falls back to uniform 15 m depth for the cycle; a stale (>180 day) cache is used anyway with a WARNING rather than triggering a download — staleness only resolves on the next `/setup/apply`.
