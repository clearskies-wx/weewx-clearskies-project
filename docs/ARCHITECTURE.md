...
Last verified: 2026-07-18 (Phase 17: 3-level SWAN nesting redesign — 1 km/100 m/10 m levels, GSFM-based domain sizing, spot clustering, per-level bathymetry, compute calculator endpoint). Previous: 2026-07-18 (ADR-095/096/097: SWAN model corrections — cross-shore transect, WLEVEL/CURRENT/OBSTACLE inputs, scoring restructure, beach profile endpoint, TruShore branding removed).
...
>
> **Compute offloading.** Operator-configurable via `surf_compute_host` in `api.conf [providers]`. When set, SwellTrack and SurfBeat computations are POSTed to a remote compute service (e.g., on librewxr) instead of running in-process on the weewx host. Authenticated with `SURF_COMPUTE_SECRET` (Bearer token, stored in `secrets.env`). TLS required. Fallback: in-process when unconfigured or unreachable (graceful degradation). Compute service endpoints: `POST /compute/swelltrack`, `POST /compute/surfbeat`, `GET /health`.
>
