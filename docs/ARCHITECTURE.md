...
>
> **SWAN model outputs (ADR-095, amended for 1D model).** CURVE transect retained as L3 diagnostic/validation output. Primary cross-shore data now from the 1D model at 3-5m CUDEM resolution. Two SPECOUT types per spot: (1) deep-water reference at ~15m from L2 (swell display card — pre-nearshore spectrum comparable to NDBC buoy readings), (2) handoff SPECOUT at per-transect handoff depth from L3 (1D model boundary condition — includes structure effects). TABLE output: HSIGN, HSWELL, DIR, TM01, DEPTH, QB, DISSURF, DSPR. Break points from 1D model H/d = gamma crossing (primary authority); SWAN QB retained as diagnostic.
>
> **Multi-transect architecture (SURF-1D-IMPLEMENTATION-PLAN).** Operator draws a shoreline segment (replaces pin). Transects generated at 10m spacing perpendicular to local isobath orientation. Each transect cross-checked against OBSTACLE structures — structure-affected transects excluded from headline metrics. 1D model runs independently per transect per swell partition from handoff to shore. L3 optional per location (auto-detected from structures). L3 smart-sized around structure clusters (not entire beach). Model selection pending Phase 1 benchmark.
>
