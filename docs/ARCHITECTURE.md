...
>
> **SWAN model outputs (ADR-095):** Cross-shore CURVE transect per surf spot (10–20 points, ~15m to ~1m depth, ~50m spacing). TABLE output: HSIGN, HSWELL, DIR, TM01, DEPTH, QB (breaking fraction), DISSURF (breaking dissipation), DSPR (directional spread). SPECOUT (2D spectrum) at ~10m depth point per spot for spectral decomposition (replaces NDBC for multiSwell). Break points detected from QB peaks along the transect.
>
