...
| `/setup/marine/compute-estimate` | GET | Compute runtime estimate for current spot configuration: grid cell counts for all 3 levels, per-run seconds at operator's configured core count, full-day budget; inputs are spot positions, GSFM shelf distance, and core count (Phase 16) | Session |
...
| `/setup/providers/test-compute` | POST | Test connectivity to remote wave modeling compute service: makes authenticated `GET /health` to `{url}/health` with `Authorization: Bearer {secret}`, returns `{ok, version, error}` (SURF-MODEL-FIX-PLAN T5.2) | Session |
| `/setup/apply` | POST | Write final config, API restarts. Accepts an optional `marine` block with SWAN grid config when `[nearshore]` extra is installed. New fields: `surf_compute_host`, `surf_compute_secret`, `surf_compute_verify_tls` (top-level); `friction_coefficient`, `surfbeat_enabled`, `surfbeat_cadence_hours`, `max_hs_m` (per marine surf spot). **(T4A.3, 2026-07-25)** When `marine` locations with a `surf` block are present, schedules the CUDEM download / 3-level grid sizing / profile generation chain as a `BackgroundTasks` job — see the SWAN nearshore model section above. **(target — pending ADR-099 acceptance):** `surf_compute_host` and `surf_compute_secret` are replaced by `marine_service_url` and `MARINE_SERVICE_SECRET`. | Session |
