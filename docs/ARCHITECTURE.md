...
>
> **API footprint.** With the marine service installed, the API contains zero marine physics code, zero marine provider modules, and zero hardcoded marine routes. The generic companion proxy module (`services/companion_proxy.py`, ~200–300 lines) handles manifest fetch, route registration, response envelope wrapping, and unit conversion for proxied responses. All marine physics and provider logic is in the marine service repo.
