...
| **8767** | TCP | SWAN standalone service (target — replaced by marine service on port 8780, pending ADR-099 acceptance) | `librewxr` | `0.0.0.0` | Currently-running `weewx-clearskies-swan-swelltrack` service. Eliminated when the unified marine service is deployed. |
| **8770** | TCP | Compute service (target — replaced by marine service on port 8780, pending ADR-099 acceptance) | `librewxr` | `0.0.0.0` | Currently-running `compute_service.py` serving `POST /compute/swelltrack`, `POST /compute/surfbeat`, `GET /health`. Eliminated when the unified marine service is deployed. |
| **8780** | TCP | Marine service (target — pending ADR-099 acceptance) | compute host (`librewxr` or weewx host) | `0.0.0.0` | TLS always. Serves all marine endpoints, `GET /health`, `GET /manifest`, `POST /config`. Replaces ports 8767 (SWAN standalone) and 8770 (compute service). Authenticated with `MARINE_SERVICE_SECRET`; `/health` and `/manifest` require no auth. |
...
>
> **Compute offloading.** Operator-configurable via `surf_compute_host` in `api.conf [providers]`. When set, SwellTrack and SurfBeat computations are POSTed to a remote compute service (e.g., on librewxr) instead of running in-process on the weewx host. Authenticated with `SURF_COMPUTE_SECRET` (Bearer token, stored in `secrets.env`). TLS required. Fallback: in-process when unconfigured or unreachable (graceful degradation). Compute service endpoints: `POST /compute/swelltrack`, `POST /compute/surfbeat`, `GET /health`. **(target — pending ADR-099 acceptance):** The compute service on port 8770 and the `surf_compute_host` config key are superseded by the unified marine service (port 8780). `marine_service_url` in `api.conf [providers]` replaces both `surf_compute_host` and `[swan] service_url`. `SURF_COMPUTE_SECRET` in `secrets.env` is replaced by `MARINE_SERVICE_SECRET`. The compute service repo artifacts (`services/compute_service.py`, `services/compute_client.py`) are deleted from the API repo.
>
...
| weewx-clearskies-marine | `repos/weewx-clearskies-marine` | main | Python 3.12+ | Yes | (target — pending ADR-099 acceptance) Unified marine companion service. Same repo structure and provider module conventions as API. `pip install -e .` for core; `pip install -e ".[nearshore]"` adds SWAN binary support. |
