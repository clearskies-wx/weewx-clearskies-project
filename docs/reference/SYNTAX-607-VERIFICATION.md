...
| 7 | 9/10 | **Transfer-file format, now proven authoritative** (supersedes prior uncertainty): header `'ID'(A22 fixed-width field, functionally forgiving of minor padding) NFREQ NDIR NPOINTS 'GRIDNAME'`, read via fixed format `(A1,A22,A1,X,2I6)` for the first two integers only; freq/dir arrays free-format; per-time `yyyymmdd hhmmss` free-format; per-point info line **fixed-format** `(A1,A10,A1,2F7.2,F10.1,F7.2,F6.1,F7.2,F6.1)` = name/lat/lon/depth/wind-speed/wind-dir/current-speed/current-dir; E(f,θ) free-format. Direction values are checked for COUNT only (`NTHI.NE.NTH` → reject) — order/monotonicity is NOT validated. **⚠ CORRECTED 2026-08-22 (trap 24): the conclusion this row previously drew — "so no re-sorting is needed" — was WRONG and cost ~2 weeks of mis-attributed Hs deficit.** "Not validated" means the readers ASSUME WW3's own bin order/sense: `ww3_bound.ftn` copies the spectrum positionally (`ABPIN((I-IFMIN+(IFMIN2-1))*NTH+J,IP)=SPEC2D(I,J)`) and carries only the FIRST direction into `nest.ww3`'s header (`MOD(2.5*PI-THETA(1),TPI)`, after snapping a first direction within 0.01 rad of `0.5*PI` to exactly `0.5*PI`); `w3iobcmd.ftn`'s `W3IOBC` compares that with the model's `TH(1)` (`SPCONV = ... ABS(TH1I-TH(1)).GT.0.01*DTH`) and otherwise assumes bin k sits at Cartesian `TH(1)+k*DTH`. The model's own axis, as `ww3_outp.ftn` prints it, is `MOD(2.5*PI-TH(ITH),TPI)` — nautical going-TO **90° DESCENDING** (first value 1.571, then 1.484, 1.396 …). A file whose axis runs nautical-ASCENDING from 180° (Cartesian descending — the pre-fix W2 writer AND the F4 reference generator) is ingested MIRRORED about the N–S axis (FROM_model = 360° − FROM_file): measured live 2026-08-22T00Z at buoy-co-located output points (S swell 155–175° vs buoy 189–196°; W-side 0.82 m @281° of 5–10 s energy arriving as 60–100° = outflow at the W edge, 0.10 m at the buoy vs 0.59 m observed). **The emitter MUST write the direction axis AND the energy columns in `ww3_outp`'s exact order/sense** (`services/ww3_formats.py::_ww3_direction_bin_order`, KATs in `tests/services/test_ww3_formats.py`). | `model/ftn/ww3_bound.ftn:315-435` (full read logic); specimen produced by our own `ww3_outp` per `regtests/ww3_ts2/input/ww3_outp_spec.inp` (OTYPE 1→3) and `regtests/ww3_tp2.8/input/ww3_outp.inp` (full ITYPE=1/OTYPE=3 documentation block) |
