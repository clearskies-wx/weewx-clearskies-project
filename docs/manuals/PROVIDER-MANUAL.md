...
- `_write_input_files(tmpdir, wind_field, boundary, bathymetry, grid_level, swan_level="L2")`: writes SWAN INPUT, BOTTOM.txt, WIND.txt, BOUND_SPEC.txt for a given grid level. **`swan_level`** (WC-D1) selects the physics package: `"L1"` emits GEN3 ST6 + SSWELL ZIEGER + NEGATINP; any other value (default `"L2"`) emits GEN3 WESTHUYSEN. Returns grid_info dict
...
| `wind_for_display` | `dict[str, dict]` | Per-timestep wind at the spot pin (not the coastline anchor), keyed by ISO-8601 time string. Each value: `{"windSpeed": float, "windDirection": float}` or `null`. Added H5 2026-08-02. **Z3.2 (2026-08-10, migration step 2 of WIND-PROVIDER-ARCHITECTURE-DESIGN-2026-08-03 §5):** no longer the primary source — the surf endpoint's display wind now reads the wind gatherer's assembled store (`services/wind_timeline_store.py`'s `get_wind_series()`) PRIMARILY, one in-process read per request. This field is now only the TRANSITION fallback used when the store refuses (gap) or is not yet populated, logged once per request; it is removed entirely in migration step 5. |
...
| Hourly fill | Any intervening (non-extended) HRRR cycle — unchanged by Z3 step 3, still the loop's own inline HRRR fetch; migration step 4's job | Full nest — L1 + L2 (+ L3 + L4 where triggered), same as a full run | Stationary sequence (24 stationary snapshots, hours 0-23, `stationary_sequence=True`, one `COMPUTE STAT` per forecast hour, latest HRRR wind trimmed to its first 24 hourly grids) | 24 hourly timesteps (hours 0-23) | not yet re-measured post-C3 (was <1 min for the pre-C3 single-snapshot path) | Every hour (per intervening HRRR cycle) |
