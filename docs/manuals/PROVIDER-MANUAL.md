...
**2-D bathymetry grid:** **(T4A.3, 2026-07-25) Downloaded and sized at `/setup/apply` time, not lazily on first SWAN run.** `endpoints/setup.py`'s `_run_marine_apply_chain()` (a `BackgroundTasks` job) resolves the bathymetry resolver priority chain (§14.7: operator file → NCEI regional OPeNDAP → Great Lakes → CRM fallback) for each grid level and writes the per-level caches at `/etc/weewx-clearskies/swan_bathymetry_L{1,2,3}.json` (180-day TTL) via `download_bathymetry_for_level()`. **At SWAN runtime, the same function is called with `allow_download=False`** — a missing cache is an ERROR (that level falls back to uniform 15m depth for the cycle) and a stale cache is used anyway with a WARNING; there is no runtime download path. `cudem_to_swan_bottom()` bilinear-interpolates the source grid onto SWAN grid dimensions. Sign convention: CUDEM (negative = ocean) → SWAN (positive = ocean). Vertical datum consistency enforced by matching CO-OPS tide prediction datum to the bathymetry DEM's native datum. No local datum conversion for the common case (ADR-098).

**Per-spot profile and grid-sizing caches** (T4A.3): `_run_marine_apply_chain()` also writes `/etc/weewx-clearskies/spot_profiles/{spot_id}.json` (PCHIP variable-resolution profile — API-MANUAL §17 — plus `structure_zone_depth`, `fine_zone_max_depth`, both contour distances, and the source grid's actual `vertical_datum`) and `/etc/weewx-clearskies/swan_grid_sizing.json` (the computed L1/L2/L3 `DomainSizing`, serialized via `services/swan_domain.domain_sizing_to_dict()`). The SWAN runtime loads both via `load_grid_sizing_cache()` and the per-spot profile read — it never calls `compute_domains()`/`compute_level3_domains()` itself. `download_bidirectional_profile()` (§14.7) has no remaining production caller as of this change; the coordinator is verifying and routing its disposition separately.
...
> **SUPERSEDED (T4A.11, P4A Round 2 "Blocker 2" operator ruling, 2026-07-25).** The paragraph above and the "grid shoreward reach = smallest value that expression ever produces here" formula a few lines up describe the *literal* ADR-093 Amendment 2 §2 reading — sizing L3's shoreward edge from the breaking-depth expression directly. **That reading has no implementable form**: it requires a minimum Hs (the spot's smallest expected swell), and none exists anywhere in this codebase — confirmed by grep, and the operator has explicitly ruled out adding one. The implemented rule instead: **the grid's shoreward edge is set by feature coverage** — for a structure-triggered cluster, 3× the structure's own length past its shoreward-most point (the shadow-zone extent, unchanged from the "L3 smart sizing" paragraph above); for a classification-only cluster (no structure), the pre-existing ~100 m near-shore pin margin, since no feature geometry exists to size a reach from. The breaking-depth expression still governs, but as a **runtime check** — T4A.9's per-hour handoff selection clamps a computed handoff to the nearest interior grid station and logs a WARNING when it would otherwise fall outside the grid, so an hour whose breaking depth sits shallower than the grid reaches is not silently mis-sampled. See `services/swan_domain.py`'s `_l3_shoreward_edge_reach_m()` docstring for the full ruling.
