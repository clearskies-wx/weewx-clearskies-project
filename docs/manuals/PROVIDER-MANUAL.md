...
**SWAN runner** (`services/swan_runner.py`) — **this subsection predates ADR-108/ADR-109 and describes an older three-level-SWAN design; `_run_level1()` was deleted 2026-08-23 (marine `3c550ae`/`c29266d`) and no longer exists. The deep-water leg is WW3 (§14.18); SWAN's own chain now starts at L2, fed by WW3 via BOUNDNEST3, not by a SWAN L1 subprocess's `NESTOUT`.** The current bullets below describe L2–L4 responsibilities:
...
- `_check_convergence(tmpdir, grid_level)`: health checks after each SWAN run — PRINT scan for `******`, NaN scan in hotstart/TABLE output, and valid-point fraction check; returns a boolean that callers use to record/refuse a failed level without promoting its output (see convergence gate subsection below)
- `_save_hotstart(run_dir, grid_level)`: copies a converged level's hotstart from its run directory into the tokened persistent chain when the caller enables hotstart saving. Production full stationary sequences enable it; hourly fast updates do not (see hotstart isolation subsection below)
...
Hourly fast updates do NOT save hotstart files. The full stationary sequence
...
**A1 automatic setup and paired output (deployed; recovery evidence remains open,
2026-08-31).** The recovery plan's approved implementation separates three setup-time
...
malformed partner refuses the producer transaction. The deployed L2 path reads the
boundary transfer directly through `BOUNDNEST3`; a fresh configuration rebuild
does not need retired L1 spectrum files as temporary scaffolding.

On 2026-08-31, live L2 accepted a regenerated direct boundary and retained an
approximately 1.0 m component. The run did not publish because no +7 through +72
hour continuation transfer was available. That result verifies neither the
73-record full/fast assembly nor the remaining A0, A0-I, A1, R1, R2, health,
publication, rollback, or retention gates. Each complete boundary/diagnostic pair
must remain together while selected, used, or retained as rollback. A later
complete, validated pair may replace it; the exact durable generation identity,
...
**Retry — the march is attempted until it exists, not once a day (J28, 2026-08-28, operator order; `service.py` `_ww3_horizon_march_due()` / `_ww3_horizon_march_attempt()` / `_ww3_horizon_catch_up_cycle()`).** As shipped, the march fired exactly once, only after the 00Z publish, and any refusal or kill left every cycle until the NEXT day's 00Z serving a frozen ocean past +6 h — on the production host it had never succeeded at all (08-27: `ww3_horizon_wind_short`; 08-28: `ww3_shel` killed, rc=-15, by the systemd stop of a deploy — the march is a child process of the service). J28 added: (1) the 00Z-only refusal (`ww3_horizon_wrong_cycle`) is removed — the march accepts any cycle hour (every cycle persists the +6 h restart and the boundary-cycle pin it needs); (2) after ANY cycle's publish, and from the runner loop's own 300 s tick as a **catch-up** (only when no full run, forced run, or hourly fill is queued and nothing is in progress), a march is attempted whenever the newest `level0/horizon_*/ww3_horizon_transfer.ww3` cannot cover the last published cycle's own 72 h SWAN L2 window (`vchain._L2_RUN_COVERAGE_H`) — no file on disk, no recorded `coverageEndTime`, or coverage ending before `cycle + 72 h`; (3) the designated 00Z cycle still always gets one attempt right after its publish; (4) bounded: at most **3** attempts per leg cycle (`_WW3_HORIZON_MAX_ATTEMPTS_PER_CYCLE`), **30 min** apart (`_WW3_HORIZON_RETRY_BACKOFF_S`); the counter is in-memory on purpose — a service restart is exactly when a fresh attempt is wanted, since a restart is what killed the last one. The catch-up continues from `fullRun.lastSuccessCycle` (restored from disk across restarts) and only spends an attempt when that cycle's `restart_<cycle+6h>.ww3` and `boundary_cycle_<cycle>.txt` are both present. The march still runs in the runner-loop thread, so a catch-up march defers the next production trigger the same way the daily march always has. These retry mechanics do not establish that the horizon merge is fixed; R3 blocks any newly observed L2 exhaustion from publication. Guards: `tests/test_j28_horizon_retry.py`, `tests/test_ww3_horizon_march.py::TestHorizonMarchAnyCycleMarches`.

**R10A/D9 execution choice (operator delegation, 2026-09-06).** Retain the
existing synchronous, single-process horizon monolith. No checkpointed worker,
concurrent worker, new process, new artifact, or cadence change is authorized or
needed if the required R10A measurement remains within the already-locked
resource and production-latency ceilings. R10A measurement is still required and
is not closed by this choice; a later worker implementation would require a new
...
**Operator testing hold (approved and implemented in source; live verification
pending).** The standalone test wrapper requests the runtime-only sentinel
`/run/weewx-clearskies/marine-test-hold`. The model runner, rather than a
point-in-time deployment guard, acknowledges the request. An already-running
model is never killed; after it returns, an acknowledged hold gates new full,
fast, and catch-up/horizon dispatch while queued work remains pending. The API
continues serving the last-good output and wind assembly remains independent.
Removing the sentinel resumes the existing schedule on a later runner iteration;
ordinary trigger, retry, and model cadence are unchanged. Testing begins only
after `/health` reports both `modelTestHold.requested: true` and
`modelTestHold.acknowledged: true`, `run_in_progress: false`,
`ww3Horizon.inFlight: false`, an active service, and no service descendants.
The wrapper's `--release` path is the manual cleanup path.
...
This section documents the provider architecture of the standalone marine
service (`weewx-clearskies-marine`) and its boundary with the API.

**Current state:** The §14 provider modules (`ndbc`, `coops`, `wavewatch`,
`nws_marine`, `nws_srf`, `hrrr`, `gfs`, `ofs`, `erddap_ocean`, and `swan`)
and their supporting marine components live in `weewx_clearskies_marine/`.
...
The following modules are internal to `weewx_clearskies_marine/providers/`
and are not registered in the API dispatch registry:

| Module | Marine service path | Domain |
|--------|---------------------|--------|
| NDBC buoy observations | `providers/buoy/ndbc.py` | `buoy` |
| CO-OPS tides & water levels | `providers/tides/coops.py` | `tides` |
| WaveWatch III forecasts | `providers/marine/wavewatch.py` | `marine` |
| NWS marine zone text | `providers/marine/nws_marine.py` | `marine` |
| NWS Surf Zone Forecast (SRF) | `providers/marine/nws_srf.py` | `marine` |
| HRRR wind | `providers/wind/hrrr.py` | `wind` |
| GFS wind | `providers/wind/gfs.py` | `wind` |
| SWAN runner | `providers/nearshore/swan.py` | `nearshore` |
| OFS ocean model data | `providers/ocean/ofs.py` | `ocean` |
| ERDDAP ocean data | `providers/ocean/erddap_ocean.py` | `ocean` |

Supporting components in the marine service:

| Component | Marine service path or ownership | Notes |
|-----------|-----------------|-------|
...
| OSM Overpass structure discovery | marine service discovery handler | Reached through an API setup proxy |
...
The marine service makes all outbound data fetches listed below. The API never
fetches from these sources directly:
...
The following marine-adjacent components remain in the API:
...
§15 is governed by ADR-099 (marine service separation, Accepted 2026-07-26).
The module-level implementation details (fetch interfaces, error taxonomy,
cache rules, and test patterns) are unchanged from §14 and continue to be
governed by the ADRs listed in §14's "Source ADRs" section.
