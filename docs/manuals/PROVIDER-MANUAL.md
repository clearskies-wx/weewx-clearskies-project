...
**Health surfaces.** New top-level `ww3Horizon` block (`lastSuccessCycleTime`, `coverageEndTime`, `wallClockS`, `refuseReason`, and — J28 — `inFlight`, `inFlightCycleTime`, `lastAttemptCycleTime`, `lastAttemptAt`) and new `fullRun.l2BoundaryExhausted` boolean: a detector scans SWAN L2's PRINT output every run for the "data on boundary file exhausted" warning and surfaces it (one WARNING log line + the health boolean, FALSE=healthy, TRUE=a regression signal). **J28 (2026-08-28): `l2BoundaryExhausted = true` now floors `status` at `degraded`** with a reason line `l2_boundary_exhausted: forecast hours beyond the WW3 leg's +6 h are a frozen ocean -- horizon march lastSuccessCycleTime=… coverageEndTime=… refuseReason=… inFlight=…` — it was `true` for three days with nothing visible saying so. The `ww3Horizon` block itself still never feeds `status`; the consequence of a missing march does. `ww3Horizon.inFlight` is also what `scripts/deploy-marine.sh` reads before restarting the service (it waits, up to 6.5 h, rather than kill a running march; `--force-restart` overrides). See OPERATIONS-MANUAL.md for the full monitoring-key list.
