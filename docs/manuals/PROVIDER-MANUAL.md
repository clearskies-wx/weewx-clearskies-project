...
- Production path: `endpoints/surf.py get_surf()` fills `TransectInfo.bathymetric_profile` from the per-spot profile cache (reusing `beach_profile._populate_local_bathymetric_profile`) BEFORE the strip's bathymetry guard — until 2026-08-22 it never did, so SurfBeat was silently skipped on every request (DEBUG log; now WARNING)
