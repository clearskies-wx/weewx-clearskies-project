...
### §14.18 WW3 deep-water leg — WW3 as OUR model (target — Phase W; ADR-109)

**Not (yet) a running provider — target state.** This subsection documents the WW3 deep-water leg's data-flow contract as fixed by ADR-109; the implementation lands in Phase W. Until Phase W lands, §14.3's WW3 forecast provider and the ADR-104/ADR-106 per-cell reconstruction (below, in the ARCHITECTURE.md L1-BOUNDARY-REBUILD-PLAN block) describe what is LIVE today — unchanged by this subsection.

**What changes:** today Clear Skies consumes NOAA's own WW3 output at the L1 boundary (§14.3, NOMADS `filter_gfswave.pl`). Under ADR-109, we additionally run WW3 **6.07.1** (NOAA-EMC/WW3 source, LGPL v3) as our own process — the "deep-water leg" — for the offshore domain, and SWAN's own chain begins at L2 instead of L1. **Always-on, no per-site conditionality** (ADR-109 D1, Q1 ruled 2026-08-15). **Shadow mode until a Phase V4 verdict-1 (cutover) ruling** (which may never be given) — the leg computes and stores its own artifacts; the live SWAN-L1 path (§14.15 above, ADR-108) keeps serving.
