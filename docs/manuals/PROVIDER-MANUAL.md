...
> **`(ruled 2026-08-13; lands with Plan Amendment A1 of MARINE-PAGE-FIXIT-PLAN)` — boundary perimeter moves with the big-L1 box (D3, ADR-108).** The per-wet-cell WW3 reconstruction mechanism above is UNCHANGED — same source, same time axis, same coverage-window rule (Z3.9a). Only the perimeter's position moves with the extended L1 box (D1): S side at ~32.60°N (~7–8 gfswave 0p16 wet cells), W side at ~119.25°W (~9 cells). No change to cell-selection logic, spectrum construction, or the `BoundaryNotViableError` viability floor.
...
> **`(ruled 2026-08-13; lands with Plan Amendment A1 of MARINE-PAGE-FIXIT-PLAN)` — D4 datum policy for the big-L1 box (ADR-108).** The existing chain (NCEI regional DEM → CRM fallback, `bathymetry_resolver.py`) is unchanged in mechanism; ADR-098 discipline applies to the big box in full. Hard requirements: (i) the big-L1 BOTTOM source must have a KNOWN datum — the existing UNKNOWN-datum rejection guard in `download_bathymetry_for_level()` stays binding; (ii) **CRM answering the big box is NOT automatically acceptable** — CRM has no guaranteed datum (see "CRM datum limitation" above), so it is a STOP-and-surface for the operator's datum ruling (A1.2), not an automatic pass, even though CRM is otherwise an acceptable fallback elsewhere in this chain; (iii) WLEVEL (STOFS ≈ LMSL) must be reconciled to the chosen BOTTOM datum via the existing ADR-098 match-at-source mechanism, verified for the NEW box; (iv) datum consistency ACROSS nest levels — the big L1's datum vs the child DEMs' (L2/L3/L4) datums — is part of A1.2's acceptance, since a level-to-level offset shifts every handoff depth.
...
> **`(ruled 2026-08-13; lands with Plan Amendment A1 of MARINE-PAGE-FIXIT-PLAN)` — wind-store bbox growth with the big-L1 domain (D6, ADR-108).** The wind-timeline-store gatherer's bbox derives from the L1 domain via the EXISTING derivation (unchanged mechanism, no new code) — extending L1 to ~138×163 km (D1) grows the gatherer bbox ~2.5× with it. HRRR CONUS and GFS both cover 32.6°N offshore, so no wind-source change is required. Disk/transfer growth is bounded and trivial versus the existing store; no schedule/cadence change, no store schema change.
