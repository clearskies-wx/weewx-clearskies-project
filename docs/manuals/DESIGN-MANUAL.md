...
**Consistency factor explainer wording (ADR-101 Amendment 1, S2 2026-08-27):** "Sets arrive in a
rhythm the spectrum's narrowness predicts; the bands are operator judgement pending observation." —
the factor's curve knots (interval, amplitude) are stated as operator judgement anchored on surfer-
reported evidence (`SET-TIMING-AND-AMPLITUDE-BRIEF-2026-08-23.md` §3.2/§4.3), not a physics law, and
the explainer text must say so rather than imply a precise/validated threshold.

> **Note (S2 doc-scope):** the table below and the surrounding "Scoring explainer modal" prose still
> describe the PRE-ADR-101 three-factor + three-penalty structure (Wave Height/Period/Organization).
> The live scorer is the five-factor ADR-101/S-SPEC-1 geometric mean (Size/Shape/Conditions/Power/
> Consistency — see API-MANUAL §17 `SurfScoringBreakdown`). That drift predates this round and is out
> of S2's scope (Consistency wording only); flagged here rather than silently rewritten.
...
| Surf | Heat Map Card | `full` | `2` | **`(ruled 2026-08-10; lands with Phase H of MARINE-PAGE-FIXIT-PLAN)` — target state (fixit log Item 5); row added because the prior version of this table omitted this card entirely (found during the fixit round).** Quasi-2D birdseye surf-height view (`HeatMapCard.tsx`, see DASHBOARD-MANUAL's Heat Map section for the full mechanism). 4x2 (`footprint="full"` + `rowSpan={2}`), same grid vocabulary as the other 2×-row cards on this page; chart area scrolls vertically inside the card when true-scale content exceeds it; header carries a `ChartFullscreenButton` opening the existing `ChartFullscreenOverlay` pattern (reused, not new). Colour cells render through a bilinear-smoothed raster (radar-mosaic style) over the product basemap background (M4 SURF-MAP-BASEMAP, PA9/Q5 — theme-selected: OSM raster in light, browser-rasterized local Protomaps tier in dark; no Esri/aerial photography on this or any user-facing surface, directive 15); nothing renders below the legends — attribution and the smoothing note both live in the info-icon modal instead. Single-ground-scale rule (C3S, DASHBOARD-MANUAL) governs every spatial element on this card — one `S` (px/metre) for data cells, imagery, and glyphs alike; this card is the reason that rule exists and remains its primary example. |
| Surf | Beach Profile Card | `full` | — | Cross-shore beach profile visualization (T5.3, pre-D5; see DASHBOARD-MANUAL's D5/D6 notes for the current scene, and the catch-up correction below for the latest display fixes). **SVG layout** (viewBox 820×292, PAD_LEFT=72, PAD_BOTTOM=72, PAD_TOP=32, PAD_RIGHT=12): shore on right, offshore on left. **9 elements** (pre-D5, superseded where they conflict): (1) Seafloor — CUDEM bathymetry, tan/brown polygon + stroke. (2) Water column — blue fill at 0.25 opacity (SURF-20 fix). (3) Hs envelope — wave height polygon, blue fill 0.32 opacity + 1.5px stroke. (4) Wave shapes — optional toggle button; Stokes/cnoidal/bore cross-sections at break points. (5) Surf zone overlays — impact zone (red, `rgba(220,38,38,0.14)`, "IMPACT ZONE" label in `var(--destructive)`) + reform trough (muted dashed boundary, no fill). (6) Break point markers — dashed red vertical line, wave-crest triangle, face/wave height label above, distance-from-shore label below, per-partition label, breaker type mini SVG icon + text. (7) Jacking annotation — amber badge "N.N× jacking" when jacking factor > 1.3. (8) Axis labels — Y-axis title (rotated -90°, "Depth (unit, datum)" or "Depth (unit)"), X-axis title ("Distance from shore (unit)"), depth tick labels on Y, distance tick labels on X including negative-distance ticks over the dry-sand strip. (9) Transect selector — `<label>` + `<select>` above SVG: Best Peak / Average / numbered transects; controlled by `selectedTransect` state in SurfingTab. Wave shapes `<button aria-pressed>` toggle shows when API provides `waveShapes` data. All new props optional for graceful degradation when T5.2 API data absent. `role="img"` + `aria-labelledby` + sr-only data table. ADR-097; T5.3. **Catch-up correction (2026-08-05, dashboard `a36e4e0` + `ad2ecf9`, task DOC-0):** item (5)'s foam zone (amber, "FOAM ZONE" label) is removed entirely — bands and legend chip both gone; impact zone is the only zone overlay that renders. The X-axis (item 8) now caps the dry-sand strip at ~50 ft (15.24 m, `DRY_BEACH_MARGIN`) landward of the waterline, and adds a negative-distance tick at the strip's landward edge when the normal tick step would otherwise stop at 0, so the axis always shows a negative number over the sand. |
| Surf | 72-Hour Forecast Card | `full` | — | HorizontalScrollNav with sticky row header column (card-glass bg, backdrop-filter). Three sections separated by `--border` dividers. **Score section:** time `<button>` + 0-100 score colored by star-tier mapping (scoreTierColor, not raw pct). **Current Conditions section:** WeatherIcon + air temp (with unit) + precip % + WindSymbol + wind quality (wraps to second line, 34px row height) — time-matched from `useForecast({ hours: 72 })`. **Swells section (T5.4):** water temp (1 decimal + unit) + swell height area chart (smooth cubic bezier, `#3b82f6` blue fill, Y-axis 0–12 ft min auto-scale, 3 ft tick gridlines, 140px) + **Surf Height row** (bold foreground, face/Hawaiian height from nearshore model; renamed from "Swell Height" in T5.4) + **Swell Height row** (muted foreground, raw offshore SWAN Hsig from `entry.swellHeight`; new in T5.4) + dom direction + period + energy. Chart line is continuous across day boundaries. Click time column → detail panel with chips + SwellBreakdown. |
| Fishing | Current Conditions Card | `full` | — | Shared normalized conditions card: weather state, air/feels-like temperature, humidity, dew point, wind/gust/direction, pressure + trend, visibility, surface water temperature, tide/current context, update time, and provenance. |
| Fishing | Fishing Forecast Card | `full` | — | Native Fish dropdown in the header plus the Surf 72-Hour Forecast grid pattern: sticky row labels, grouped day headings, fixed-width aligned period columns, and horizontal scrolling. Rows carry selected-species score/status, tide/current, water temperature, pressure, wind, swell, and solunar values; a time column expands the accessible detail below the shared grid. |
| Fishing | Sun & Moon / Solunar Card | `full` | — | Complete Almanac `SunMoonDetailCard` information and hierarchy, with major/minor feeding windows as the fishing overlay. |
| Beach Safety | Beach Conditions Card | `full` | — | Weather icon + air temp, rip current, UV index, wave/wind/temp stats |
| Boating | Current Conditions Card | `full` | — | Shared normalized conditions card: weather icon, air/feels-like temperature, humidity, dew point, wind/gust/direction, pressure + trend, visibility, surface water temperature, tide/current context, update time, and provenance. |
| Boating | Boating Forecast Card | `full` | — | Individual horizontally scrollable period cards using the regular forecast pattern, with aligned and labelled regional NWS wind, seas, visibility, and weather additions. |
| Boating | Offshore Observations Card | `wide` | — | Selected NDBC station identity, distance, timestamp, supplied measurements, and an explicit navigation/exit-context non-local warning. |
| Boating | Tides, Currents, and Water Level | `full` | — | Shared `TideChart` and water-level/current context; missing data remains visibly unavailable. |

**`(ruled 2026-08-10; lands with Phase K of MARINE-PAGE-FIXIT-PLAN, ADR-106 R4, PA5)` — dashboard `types.ts`
impact-zone comment, prescribed text.** The dashboard's `src/api/types.ts` currently carries a comment on
the impact-zone field describing a "50% energy loss" criterion — this has never matched the shipped server
behavior (which used a 5% roller-energy-floor criterion, ADR-102 X-D3) and is corrected doc-code drift
independent of this round's other findings. Once Phase K ships the R3 fixed-crash-band definition, that
comment is replaced with: *"impactZone: a fixed-width band shoreward from each published break marker
(server config `impact_zone_width_m`, operator-adjustable, default 25 m), clipped at the waterline. Not a
whitewater-decay measurement — the band's width is a configured distance, not a physics-derived one."* No
percentage of any kind (neither 50% nor 5%) belongs in the comment after this change — the shipped
mechanism has no whitewater-decay term on the published path.
