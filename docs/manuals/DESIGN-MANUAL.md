...
| Fishing | Fishing Forecast Card | `full` | — | Individual horizontally scrollable period cards. A non-wrapping species strip selects the server-scored species; each period exposes the selected-species score/status and expandable accessible detail. |
