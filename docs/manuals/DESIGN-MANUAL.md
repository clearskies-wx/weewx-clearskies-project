...
| Surf | Beach Profile Card | `full` | — | Cross-shore transect: inline SVG (viewBox 800×400) with bathymetry (brown/tan fill), water surface line, wave envelope (blue), break point markers at QB peaks. Y-axis: depth labels (0m, -5m, -10m...) with horizontal gridlines on the left (PAD_LEFT 55px). X-axis: distance-from-shore tick marks and labels at round intervals below the chart (PAD_BOTTOM 48px). Break markers: dashed vertical line + wave-height label at top + distance-from-shore label in the x-axis area. Multi-break spots show multiple markers. `role="img"` + `aria-labelledby` + sr-only data table. ADR-097. |
