...
| `waveOrganization` | int | Wave organization composite score (0–30). Rounds to 0 decimal places (`round(org_score * 30)`, no `ndigits`), while each `organization*` sub-factor below rounds to 1 decimal place independently (`round(..., 1)`) — **the four sub-factors are NOT guaranteed to sum exactly to `waveOrganization`** (drift of roughly ±0.2–0.3 is possible; corrected 2026-08-02, verified `surf_scorer.py:660` vs `:685-688` — there is no shared-rounding step between the two). |
| `organizationWind` | float | Wind effect contribution within organization. **Corrected 2026-08-02: true range 0–18, not 0–15.** Wind-quality multipliers can exceed 1.0 (glassy `1.1`, light-offshore `1.2`, `surf_scorer.py` constants `~:203-212`), so `organizationWind = wind_score × 0.50 × 30` reaches `16.5` (glassy) and `18.0` (light-offshore) — `18.0` is the true maximum, not `15.0` (`:685`). |
