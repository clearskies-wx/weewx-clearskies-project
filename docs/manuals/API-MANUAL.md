...
**Reshaped 2026-08-05 (Round S, ADR-101 — BREAKING wire change; marine commits local pending the operator worked-examples gate).** Per-factor scoring detail returned inside each `SurfForecast` entry. Five components, each internally 0–1.0 and published 0–100; the total is a **weighted geometric mean**: `score = 100 × Size^w1 × Shape^w2 × Conditions^w3 × Power^w4 × Consistency^w5`, weights normalized by their sum at computation time (shipped defaults 0.25/0.25/0.20/0.20/0.10, operator-configurable system-wide — see OPERATIONS-MANUAL `[marine][[surf_score_weights]]`). Bars do NOT sum to the total; any factor at 0 → score 0. Stars = score/20 unchanged (floors at 1★; a 0-star tier is a parked operator item). Within a component, sub-inputs blend by weighted arithmetic mean with fixed internal weights (research brief §7.2 is the binding inventory; single-use rule applies). **Ruin override (S1b, 2026-08-05):** a documented ruin state pins its whole component at ≤ 0.05 — peel angle ≤ 10° (closeout) pins Shape; blown-out wind pins Conditions — so no secondary sub-input can dilute a ruined day (`_RUIN_CEILING`, `surf_scorer.py`). Null handling: a missing sub-input renormalizes the component's internal weights; a fully-unavailable component is excluded and the five exponents renormalize; a measured zero is NOT missing. **Deleted with this reshape:** the 3-factor + 3-adjustment shape (`waveHeight`/`wavePeriod`/`waveOrganization`/`organization*`/`beachAlignment`/`directionalExposure`/`timeOfDay`), the adjustments concept, time-of-day scoring, and Tm01 as a scoring input. Beach alignment and directional exposure survive only as internal Size modifiers (never displayed). Known gap S-GAP-1: the §7.2 jacking sweetener (Shape) is implemented but unwired (no in-scope data path from the 1D pipeline to the scoring call site); pending an operator-authorized follow-up.

| Field | Type | Description |
|---|---|---|
| `size` | float | Wave Size component (0–100, 1 dp). Face-height band curve; beach alignment + directional exposure act inside it as modifiers. |
| `shape` | float | Wave Shape component (0–100, 1 dp). Peel angle (0.6) + breaker type/Iribarren (0.4); jacking sweetener pending (S-GAP-1). Closeout ruin pin ≤ 5.0. |
| `conditions` | float | Conditions component (0–100, 1 dp). Wind (0.6) + DSPR (0.25) + cross-swell (0.15). Blown-out ruin pin ≤ 5.0. |
| `power` | float | Swell Power component (0–100, 1 dp). Dominant `multiSwell` partition period (0.7) + height-as-energy-expression (0.3). Never Tm01. |
| `consistency` | float | Consistency component (0–100, 1 dp). SurfBeat set timing (0.6) + amplitude (0.4); swell-dominance ratio substitutes wholesale when SurfBeat unavailable (`dataState: "fallback"`). |
| `weights` | object | The five EFFECTIVE (normalized) weights actually used, keys `size`/`shape`/`conditions`/`power`/`consistency`, each 0–1 (3 dp). Reflects exclusion-renormalization. |
| `dataState` | object | Per-component data provenance, same five keys; values `"full"` \| `"partial"` (some sub-inputs missing, internal weights renormalized) \| `"fallback"` (substitute input in use) \| `"excluded"` (component unavailable, exponent redistributed). |
