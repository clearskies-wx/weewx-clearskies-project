...
| `faceHeightMinFt` / `faceHeightMaxFt` | float | — (already display-unit feet) | Yes | **LIVE 2026-08-09 (L1-BOUNDARY-REBUILD-PLAN Phase C, P13, marine `921ac21`); REMOVED `(ruled 2026-08-10; lands with Phase S of MARINE-PAGE-FIXIT-PLAN, ADR-106 R2, PA2)`.** Min/max of the per-swell breaking face heights at the representative break, over the same ELIGIBLE components as `swellHeightMinFt/MaxFt` (`PartitionBreakInfo.mean_face_height_m`, keyed by the SAME canonical partition index space `multiSwell`'s list position already is — the per-partition face heights the 1D pipeline already computes; R5's per-transect dominant-swell machinery is read, not modified). A component with no break data contributes to the swell-height range but not this one. `null` when no eligible component has break data. **Target state:** removed from computation and the served response — its only display consumer (the Swell Card's Breaking Face Height stat) rebinds to `modelSurfHeightMin`/`modelSurfHeightMax` above, the already-served best-surf-strip pair that matches the operator's original Breaking Face Height definition. `breakingFaceHeight`/`modelSurfHeightMin/Max` themselves are unchanged. |
| `combinedPeriodS` | float | `group_wave_period` | Yes | **LIVE 2026-08-09 (L1-BOUNDARY-REBUILD-PLAN Phase C, P13, marine `921ac21`); REMOVED `(ruled 2026-08-10; lands with Phase S of MARINE-PAGE-FIXIT-PLAN, ADR-106 R2, PA2)`.** Energy-weighted mean period over the same ELIGIBLE components, weights `Hs_i²` (worked example: `(1.5²·16.4 + 1.1²·12.8)/(1.5²+1.1²) ≈ 15.1 s`). One number, one decimal. `null` when no deep-water reference exists for this timestep. **Target state:** removed — operator ruling, verbatim: *"the period should never be combining periods, that is not how the physics works."* Periods of separate wave trains do not combine into a meaningful average. |
| `periodMinS` / `periodMaxS` | float | `group_wave_period` | Yes | **`(ruled 2026-08-10; lands with Phase S of MARINE-PAGE-FIXIT-PLAN, ADR-106 R2, PA2)` — target state, replaces `combinedPeriodS` above.** Lowest/highest peak period across the ELIGIBLE surfable swell components (same eligibility rule as `swellHeightMinFt/MaxFt` — a component is eligible unless it is classified wind swell AND its period < 5.0 s; the existing no-qualifier-fallback-to-all-components behavior is unchanged). Single eligible component collapses min=max. One decimal. `null` when no deep-water reference exists for this timestep. The Swell Card's Period stat shows this range (single number when min = max, same collapse rule as the height ranges) once served. |
