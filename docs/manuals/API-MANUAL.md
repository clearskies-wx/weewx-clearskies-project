...
| `photoUrl` | str | — | Yes | `/marine-photos/{locationId}.{ext}` when the operator has uploaded a location photo (JPG, PNG, or WebP; T4.1-T4.3); `null` otherwise. `_photo_url()` in `endpoints/marine.py` does a filesystem existence check for any supported extension — no config field read. Populated on `GET /marine` only; `GET /tides` uses its own `_location_summary()` (tides.py) which does not set it, so it is always `null` there. |
