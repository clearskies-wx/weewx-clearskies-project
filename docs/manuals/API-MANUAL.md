...
- **`modelHealth`** (R8a, schema version 1): the additive marine model-health ledger. It contains `overall`, `serving` (including `selectedFullCycleId`), `attempts` (`active` and latest `full`/`fast`/`horizon`/`recovery`), and the required stage tree with provider and SWAN children. In R8a every stage is `unknown`/`not_instrumented`, `overall` is `unknown`, and `serving` is `unavailable`; no visitor response field changes. The state vocabulary, precedence, required-stage matrices, and evidence rules are in OPERATIONS-MANUAL.md “R8a `modelHealth` ledger”.
