...
| `breakPoints` | list[object] | — | Yes | QB peak locations along the SWAN cross-shore CURVE transect (T3.4). `null` when no QB peak ≥ 0.25 detected (flat conditions, single-point mode, or QB data absent). Multiple entries for multi-bar beaches (outer bar + inner bar). **Real dominant null cause (added 2026-08-02, coordinator-verified):** the SWAN domain's shoreward edge sits at the ~1.78 m design-breaking contour (`l3_shoreward_edge_depth_m()`), while the CURVE itself spans roughly 3.4–15.8 m at HB — both seaward of where a SMALL swell actually breaks. On an ordinary small-swell day the true break line is shoreward of the entire CURVE domain, so QB along it never reaches the 0.25 threshold and this field is `null` as EXPECTED, ordinary behavior — not a defect signal. It only reliably populates on large-swell days when the break line reaches into the CURVE's own domain. **This `breakPoints` and the beach-profile endpoint's `breakPoints` are DIFFERENT, unrelated producers sharing only a field name:** this one (`/surf`) is SWAN CURVE QB-peak detection with the flat `{distance, depth, hs}` schema documented in the rows below; `/profile`'s `breakPoints` (§18) is the SwellTrack 1D pipeline's per-partition break-point detection, with a materially richer schema (face height, breaker type, Iribarren number, partition info). Do not assume one endpoint's `breakPoints` shape or null-semantics apply to the other. |
