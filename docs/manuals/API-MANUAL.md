...
| `swellDominance` | float | — | No | **Corrected 2026-08-02.** NOT a continuous 0–1 ratio. `surf_scorer.py` (`_swell_dominance()`, `:355-374`; constants `:229-235`) computes the energy ratio of ALL spectral components with period > 10 s (not just the primary/dominant one) to total energy, then BUCKETS the result: ratio > 0.8 → `1.0` ("pure swell"), ratio ≥ 0.5 → `0.6` ("mixed"), else → `0.2` ("wind chop dominant"). `0.5` when spectral component data is missing or carries zero total energy (`_SWELL_DOMINANCE_DEFAULT`). The served value is always one of `{1.0, 0.6, 0.2, 0.5}` — never an intermediate ratio. `organizationSwellDominance = swellDominance × 7.5` (25% of the 30-point organization factor, `:686`). |
