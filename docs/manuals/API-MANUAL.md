...
| `faceHeightMinFt` / `faceHeightMaxFt` | float | — (already display-unit feet) | Yes | **LIVE 2026-08-09 (L1-BOUNDARY-REBUILD-PLAN Phase C, P13, marine `921ac21`).** Min/max of the per-swell breaking face heights at the representative break, over the same ELIGIBLE components as `swellHeightMinFt/MaxFt` (`PartitionBreakInfo.mean_face_height_m`, keyed by the SAME canonical partition index space `multiSwell`'s list position already is — the per-partition face heights the 1D pipeline already computes; R5's per-transect dominant-swell machinery is read, not modified). A component with no break data contributes to the swell-height range but not this one. `null` when no eligible component has break data. Additive field; existing `breakingFaceHeight`/`modelSurfHeightMin/Max` are unchanged. |
| `combinedPeriodS` | float | `group_wave_period` | Yes | **LIVE 2026-08-09 (L1-BOUNDARY-REBUILD-PLAN Phase C, P13, marine `921ac21`).** Energy-weighted mean period over the same ELIGIBLE components, weights `Hs_i²` (worked example: `(1.5²·16.4 + 1.1²·12.8)/(1.5²+1.1²) ≈ 15.1 s`). One number, one decimal. `null` when no deep-water reference exists for this timestep. Additive field. |
