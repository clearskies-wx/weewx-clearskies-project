...
| `swellHeightMinFt` / `swellHeightMaxFt` | float | — (already display-unit feet) | Yes | **(ruled 2026-08-08; lands with Phase C of L1-BOUNDARY-REBUILD-PLAN, P13.)** Min/max deep-water swell height across ELIGIBLE swell components (server-side aggregation — dashboard stays a dumb renderer). Eligibility: a component is eligible unless it is classified wind swell AND its period < 5.0 s; if NO component is eligible, the aggregate is computed over all components (the card must always show real conditions, never blank). Single eligible component collapses min=max. Additive field; existing `swellHeight` (dominant-partition height) is unchanged. |
| `faceHeightMinFt` / `faceHeightMaxFt` | float | — (already display-unit feet) | Yes | **(ruled 2026-08-08; lands with Phase C of L1-BOUNDARY-REBUILD-PLAN, P13.)** Min/max of the per-swell breaking face heights at the representative break, over the same ELIGIBLE components as `swellHeightMinFt/MaxFt` (the per-partition face heights the 1D pipeline already computes; R5's per-transect dominant-swell machinery is read, not modified). Additive field; existing `breakingFaceHeight`/`modelSurfHeightMin/Max` are unchanged. |
| `combinedPeriodS` | float | `group_wave_period` | Yes | **(ruled 2026-08-08; lands with Phase C of L1-BOUNDARY-REBUILD-PLAN, P13.)** Energy-weighted mean period over the same ELIGIBLE components, weights `Hs_i²` (worked example: `(1.5²·16.4 + 1.1²·12.8)/(1.5²+1.1²) ≈ 15.1 s`). One number, one decimal. Additive field. |
