...
| `modelSurfHeightMin` | float | `group_wave_height` | Yes | **WC-D3 added 2026-08-07.** Smallest face height (m) among qualifying (Tp ≥ 5s) per-partition break points. `null` when zero qualifying partitions have break data. Distinct from NWS SRF `surfHeightMin` (on `SurfZoneForecast`, a different data source). |
| `modelSurfHeightMax` | float | `group_wave_height` | Yes | **WC-D3 added 2026-08-07.** Largest face height (m) among qualifying (Tp ≥ 5s) per-partition break points. Same value as `breakingFaceHeight` (backward compatibility). `null` when zero qualifying partitions have break data. Distinct from NWS SRF `surfHeightMax`. |
