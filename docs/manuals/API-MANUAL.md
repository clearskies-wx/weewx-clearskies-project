...
| `weatherCode` | str | — | Yes | Normalized weather-state code |
| `weatherText` | str | — | Yes | Normalized weather-state description |
| `isDay` | bool | — | Yes | Day/night flag from the selected weather source |
| `time` | str | — | No | Selected source update or observation time (UTC ISO-8601) |
| `feelsLike` | float | `group_temperature` | Yes | Apparent air temperature |
| `humidity` | float | — | Yes | Relative humidity percentage |
| `source` | str | — | No | Selected-location source role: station, forecast provider, mixed, or unavailable |
| `provenance` | object | — | No | Conditions, pressure, water-temperature, and tide/current source records, including explicit unavailable values |
...
for alignment. A matched period augments that regular-forecast point and never
replaces the location-weather provider's values.

| Field | Type | Unit group | Nullable | Description |
|---|---|---|---|---|
| `periodName` | str | — | No | Provider period label (e.g., "Tonight", "Thursday") |
| `issuanceTime` | str | — | Yes | UTC time when NWS issued the CWF product; preserved as provenance |
| `periodStart`, `periodEnd` | str | — | **Yes** | UTC validity bounds assigned by the API only after the label maps to an existing regular-forecast day/night column; both remain `null` for an unmatched or unrecognised period |
...
`validTime`, nullable provider fields, and provider-neutral hourly pressure
(`pressure` plus `pressureSource` when available).

The source boundary is location-first for current conditions, while the
regular hourly forecast remains the configured forecast provider's time series:

| Location state | Current `observation` weather fields | `regularForecast` | Regional CWF additions |
|---|---|---|---|
| Within the configured station-service radius | Station archive supplies supported current wind, gust, temperature, pressure, and related fields; forecast-provider weather state fields remain provider-sourced where present | Configured forecast provider queried at the marine location coordinates | Matching CWF period is attached to the existing regular-forecast column as `marineAdditions`; unmatched period remains unaligned |
| Outside the station-service radius | Configured forecast provider queried at the marine location coordinates | Same configured forecast-provider series at the marine location coordinates | Same label-to-existing-column matching rule |
| No provider, failed provider fetch, or no usable hourly points | Explicit unavailable/null values; no NDBC substitution | Empty list when no usable provider series is returned | No additions; CWF periods are not given invented bounds |

NDBC observations are not a source for `regularForecast` or for selected-point
weather. They remain separately labelled offshore context. The configured
forecast provider is fetched using the API's target-unit setting, so the
...
column exists in `regularForecast`; the assigned `periodStart` and `periodEnd`
are the corresponding explicit UTC bounds of that existing column. An
unknown label, an `EXTENDED` label, or a recognised label with no existing
column leaves the CWF period's `periodStart` and `periodEnd` null and attaches
no `marineAdditions`. No UTC interval is fabricated from issuance time,
period order, or the current clock.
...
| `swellDominance` | float | — | No | Continuous ratio (0.0–1.0), not bucketed (marine `a751c9a`, 2026-08-02). `surf_scorer.py` `_swell_dominance()` (`:355-383`; constants `:229-235`) sums the energy of ALL spectral components with period > 10 s ("swell") over total spectral energy across all components — not just the primary/dominant one — and returns that ratio as-is. **"Energy" here is `_partition_energy(c)` = `Hs² · Tp`, not the partition's raw `energy` field (corrected 2026-08-07, ADR-096 Amendment 2, marine `8e17e84` — see the dominant-swell-selection callout above).** `0.5` when spectral component data is missing or carries zero total energy (`_SWELL_DOMINANCE_DEFAULT`). The internal quality-score organization sub-factor is a SEPARATE value: `_swell_dominance_score()` buckets this same ratio (ratio > 0.8 → `1.0`, ratio ≥ 0.5 → `0.6`, else → `0.2`) for `organizationSwellDominance = bucket × 7.5` (25% of the 30-point organization factor, `:686`) only — the bucket is never what reaches the wire. Bucketed on the wire too until marine `a751c9a`, 2026-08-02. |
...
| `swellSource` | `"deep_reference" \| "nearshore_table" \| null` | — | Yes | **Added S3 (2026-08-27, `endpoints/surf.py:1388`) — was already served (Q16 C3, 2026-08-25), previously undocumented here.** Records which channel actually served this hour's `multiSwell`/`swellHeight` card. `"deep_reference"`: the true deep-water (≥200 m, post-island-shadowing, pre-refraction) WW3 catalog (`services/model_wave_source.py::get_deep_swell_catalog()`), read once per request and keyed by `valid_time`. `"nearshore_table"`: fallback to the 15 m SWAN L2 DWR-table partitions (the same source `multiSwell` used exclusively before Q16 Round B) for an hour the WW3 leg's currently-staged transfer file does not cover (today: hours beyond the leg's ~6h march window). `null`: neither source available this timestep — `multiSwell` is also `null` on that entry. Display-only signal; does not gate any other field. |
| `scoring` | SurfScoringBreakdown | — | Yes | Per-factor scoring breakdown (see below) |
| `swellHeight` | float | `group_wave_height` | Yes | Height of the dominant `multiSwell` partition — i.e. the deep-water reference, the same point as `multiSwell` (marine `83f0205`). **Dominant selected by total partition energy (`Hs² · Tp`), corrected 2026-08-07 — see the dominant-swell-selection callout above.** When the deep-water reference is unavailable for that timestep the field falls back to the cross-shore transect reference point's SWAN `HSWELL`, or to that point's `HSIGN` when `HSWELL` is absent — a different quantity at a different place; `multiSwell` still goes `null`. Nullable in the schema, but not emitted null in practice: a timestep whose reference point carries no `waveHeight` is skipped in its entirety, so an entry that exists always carries a `swellHeight`. |
| `breakingFaceHeight` | float | `group_wave_height` | Yes | **WC-D3 redefined 2026-08-07:** max of qualifying (Tp ≥ 5s) per-partition mean face heights (`PipelineResult.model_surf_height_max_m`), falling back to the BD-7 main-zone headline when no qualifying partition has break data. Same as `modelSurfHeightMax` below. `null` when unavailable. Was the BD-7 main-zone headline (or single-transect best peak on no-headline hours). Derivation: 1.27 × Hs (Rayleigh H1/10), per ADR-095 Amendment 1. |
...
| `breakPoints` | list[object] | — | Yes | **Re-sourced 2026-08-02 (V3-F4-IMPL, operator-approved: "once the handoff happens we should be using the better 1D model for actually generating data").** Break points on the REPRESENTATIVE transect (`representativeTransectIndex`, same forecast entry) — sourced from the SwellTrack 1D pipeline's `PipelineResult.per_transect` (`services/surf_1d_pipeline.py`), the SAME transect and the SAME producer family the beach-profile endpoint's `breakPoints` (§18) reads. Collects every break point across every swell partition for that transect (the SAME point set and the SAME seaward-to-shore order §18 emits for its own `breakPoints`), mapped to this endpoint's existing, unchanged flat `{distance, depth, hs}` schema (documented in the rows below) — NOT §18's richer per-point schema (`faceHeight`/`breakerType`/`iribarren`/`partitionInfo`); this endpoint does not serve those. `null` exactly when `representativeTransectIndex` is `null` (no pipeline result this hour), OR when the representative transect itself has no break points in any partition — never a fallback to a different transect, and never recomputed from a legacy picker. **`/surf`'s `breakPoints` and `/profile`'s `breakPoints` now share ONE producer family and read the SAME representative transect for the same forecast hour — they agree by construction; they still differ only in per-point SCHEMA, not in which points/transect they describe.** Superseded producer (retired this round): the legacy SWAN-CURVE QB-peak picker (`select_reference_point()`'s second return value, `services/surf_pipeline_timestep.py`, function itself unchanged and still used for OTHER purposes) served `null` at ordinary small-swell conditions because the SWAN domain's shoreward edge sat seaward of the true break line — that failure mode no longer applies to this field. |
| `breakPoints[].distance` | float | — | — | *(Field name corrected 2026-08-02, D4 audit — the wire key is `distance`; earlier prose here said `distanceFromShore`, a name that never existed on the wire.)* Distance from shore in meters, measured from the reference waterline (the DEM datum's 0 m / LMSL shoreline). Positive = seaward. **Can be negative** (ADR-093 Amendment 4): at high tide a break can occur on the beach face *landward* of the reference waterline, reported as a negative distance. Consumers must accept negatives; a display should treat a negative value as "at/onshore of the shoreline," not clamp or `abs()` it. |
| `breakPoints[].depth` | float | — | — | Water depth at the break point in meters. |
| `breakPoints[].hs` | float | — | — | *(Field name corrected 2026-08-02, V3-F4-IMPL — the wire key has always been `hs`, not `waveHeight`; this row previously documented a key that never existed on the wire.)* Significant wave height (Hs) at the break point, in meters (not unit-converted — physical position). **NOT face height** — see §18's `faceHeight` for that quantity, which this endpoint does not serve. |
...
| `modelSurfHeightMin` | float | `group_wave_height` | Yes | **WC-D3 added 2026-08-07.** Smallest per-partition face height (m) across the main break zone's qualifying transects (the 0.75σ best-surf selection). Only qualifying partitions (Tp ≥ 5s) on qualifying transects contribute. `null` when zero qualifying partitions have break data. Distinct from NWS SRF `surfHeightMin` (on `SurfZoneForecast`, a different data source). |
| `modelSurfHeightMax` | float | `group_wave_height` | Yes | **WC-D3 added 2026-08-07.** Largest per-partition face height (m) across the main break zone's qualifying transects. Same value as `breakingFaceHeight` (backward compatibility). `null` when zero qualifying partitions have break data. Distinct from NWS SRF `surfHeightMax`. |
| `swellHeightMinFt` / `swellHeightMaxFt` | float | — (already display-unit feet) | Yes | **LIVE 2026-08-09 (L1-BOUNDARY-REBUILD-PLAN Phase C, P13, marine `921ac21`).** Min/max deep-water swell height across ELIGIBLE swell components (server-side aggregation — dashboard stays a dumb renderer; `endpoints/surf.py::_compute_eligible_swell_aggregates()`). Eligibility: a component is eligible unless it is classified wind swell AND its period < 5.0 s; if NO component is eligible, the aggregate is computed over all components (the card must always show real conditions, never blank). Single eligible component collapses min=max. Converted meter→foot via `units/conversion.py::METER_PER_FOOT` — this is the one field pair on this endpoint served already in display-unit feet rather than SI, per the P13 ruling. `null` when no deep-water reference exists for this timestep (same gating `multiSwell` uses). Additive field; existing `swellHeight` (dominant-partition height) is unchanged. |
| `periodMinS` / `periodMaxS` | float | `group_wave_period` | Yes | **LIVE (Phase S of MARINE-PAGE-FIXIT-PLAN-2026-08-10, ADR-106 R2, PA2, operator ruling 2026-08-10).** Lowest/highest peak period across the ELIGIBLE surfable swell components (same eligibility rule as `swellHeightMinFt/MaxFt` — a component is eligible unless it is classified wind swell AND its period < `_MIN_SURFABLE_PERIOD_S` (5.0 s, shared with `services/surf_1d_pipeline.py`); the existing no-qualifier-fallback-to-all-components behavior is unchanged). Single eligible component collapses min=max. One decimal. `null` when no deep-water reference exists for this timestep. Replaces the retired `combinedPeriodS` energy-weighted average — operator ruling, verbatim: *"the period should never be combining periods, that is not how the physics works."* Periods of separate wave trains do not combine into a meaningful average. The Swell Card's Period stat shows this range (single number when min = max, same collapse rule as the height ranges). |

`faceHeightMinFt`/`faceHeightMaxFt` and `combinedPeriodS` were LIVE 2026-08-09 through 2026-08-10 (L1-BOUNDARY-REBUILD-PLAN Phase C, P13, marine `921ac21`) and are REMOVED as of Phase S of MARINE-PAGE-FIXIT-PLAN-2026-08-10 (ADR-106 R2, PA2). `faceHeightMinFt`/`faceHeightMaxFt`'s only display consumer (the Swell Card's Breaking Face Height stat) rebinds to `modelSurfHeightMin`/`modelSurfHeightMax` above, the already-served best-surf-strip pair that matches the operator's original Breaking Face Height definition; `breakingFaceHeight`/`modelSurfHeightMin/Max` themselves are unchanged. `combinedPeriodS` is replaced by `periodMinS`/`periodMaxS` above.
...
**Reshaped 2026-08-05 (Round S, ADR-101 + plan S-SPEC-1 — BREAKING wire change; marine commits local pending the operator worked-examples gate).** The binding design is EYEBALL-FIX-PLAN-2026-08-04.md §1 S-SPEC-1 (locked curves, thresholds, null policy, worked-example fixtures) — this section summarizes it; on any divergence S-SPEC-1 wins. Per-factor scoring detail inside each `SurfForecast` entry. Five components, each internally 0–1.0 and published as **int 0–100**; total = weighted geometric mean `score = 100 × size^w1 × shape^w2 × conditions^w3 × power^w4 × consistency^w5`, weights normalized by sum (defaults 0.25/0.25/0.20/0.20/0.10; operator-adjustable — see OPERATIONS-MANUAL `[surf_scoring]`). Bars do NOT sum to the total; any factor at 0 → score 0; stars = score/20. Sub-inputs blend by weighted arithmetic mean with fixed internal weights (§7.2 inventory + S-SPEC-1 curve tables; single-use rule). **Ruin clamps (in the locked design):** peel angle < 15° → shape = min(blend, 0.10) (closeout); onshore wind ≥ 25 mph or any direction ≥ 35 mph → conditions = 0.05 (blown out). **Null policy:** a null sub-input takes its listed neutral and stays in the blend; a whole factor with no data takes 0.5; NO exponent renormalization; there is NO data-provenance field on the wire. **Deleted:** the 3-factor + 3-adjustment shape (`waveHeight`/`wavePeriod`/`waveOrganization`/`organization*`/`beachAlignment`/`directionalExposure`/`timeOfDay`), the adjustments concept, time-of-day scoring, Tm01 as a scoring input. Alignment/exposure survive only as internal Size multipliers. Known gap S-GAP-1: the jacking bonus (+0.05 when any `jackingFactors[].factor` > 1.3) is implemented but unwired — no in-scope data path from the 1D pipeline to the scoring call site; operator follow-up pending.

| Field | Type | Description |
|---|---|---|
| `size` | int | Wave Size (0–100). Existing face-height table × alignment multiplier × exposure multiplier; face < 0.3 ft → 0. |
...
| `selectedSpecies` | str | — | No | Selected source species or disclosed collapsed group |
| `profileLevel` | str | — | No | Direct species, collapsed group, or geographic functional-category fallback |
| `score` | int | — | Yes | Selected-species score 0–100; `null` when a required core input is unavailable |
| `status` | str | — | Yes | Active, less active, low activity, or inactive; `null` when a required core input is unavailable |
| `coreScore` | float | — | Yes | Environmental core before bounded refinements; `null` when unavailable |
| `temperatureSuitability`, `tideCurrentSuitability`, `pressureSuitability` | float | — | Yes | Approved core-factor values, 0–1; all are `null` when a required core input is unavailable |
| `factorProvenance` | object | — | No | Required availability/source record for temperature, tide/current, and pressure. It is contract metadata, not a user-facing explanation. |
| `hardStopReason` | str | — | Yes | Closure or out-of-range temperature reason; score is then zero/inactive |
| `appliedAdjustments` | list[object] | — | No | Disclosed time, season, and solunar refinements |
| `waterTemperatureProvenance`, `weatherProvenance`, `swellProvenance` | object | — | No | Selected source records, each carrying explicit availability and valid time; water also carries coverage tier and depth |
| `conditionsText` | str | — | No | (locale) Natural-language conditions summary |
...
When a required core input is unavailable, the service logs the location,
period, input name, and source/provenance at warning level. It does not return a
visitor-facing explanation; score, status, core, and suitability values are
`null`.
...
|---|---|---|---|
| `waveHeight` | Surf-spot locations (`location.id in marine_config.surf_spots`) only: the wave model's own computed swell breakdown, via `services/model_wave_source.py` reading the same SWAN last-good cache the surf endpoint uses. | `null` when model output is unavailable, and always `null` for a non-surf selected location. A configured NDBC buoy belongs only in separately labelled Offshore Observations; it never supplies selected-point wave conditions. | meter → operator `group_wave_height` |
...
| `waterTemp` | Coverage-aware resolver: local sensor, then a configured regional source that covers the chosen point, then national fallback | Clearly labelled offshore observation only as last resort, otherwise null | Celsius → operator `group_temperature`; carry source, valid time, coverage tier, and depth. WCOFS is West-Coast-only. |
...
|---|---|---|
| `MarineBundle` | `MarineObservation`, `list[HourlyForecastPoint]` (`regularForecast`), `list[MarineForecastPoint]`, `list[MarineTextForecast]` | `GET /api/v1/marine[/{locationId}]` |
...
| `coordinates` | object | No | `{lat, lon}` |
| `forecast` | list[SurfForecast] | No | One entry per SWAN forecast timestep. Contains all four height fields (`swellHeight`, `waveHeightAtBreak`, `breakingFaceHeight`, `breakingHawaiianHeight`) plus `windSource`, `scoringBreakdown`, and `breakPoints` (1D-pipeline break points on the representative transect, V3-F4-IMPL 2026-08-02) per timestep. Empty list if SWAN has never run successfully. |
| `zoneForecast` | SurfZoneForecast | Yes | NWS SRF forecast for the covering county zone; `null` if unavailable |
| `waterTemp` | float \| null | Yes | **Added 2026-08-02 — was missing from this table.** Top-level (not per-forecast-entry) sea surface temperature in °C, `ocean_data_resolver.resolve(needs="surface")` sourced (on-premises sensor → OFS → MUR SST → RTOFS/ERDDAP, same tiered chain PROVIDER-MANUAL §14.12 documents). Falls back to `zoneForecast.waterTemp` — the separate NWS SRF hand-typed value — ONLY when the ocean data resolver itself returns nothing (`endpoints/surf.py`). **This is a different value from `zoneForecast.waterTemp`**, which remains present at its own nested key when `zoneForecast` is non-null; the two are independently sourced and can differ by several °C (the SRF value is a forecaster-entered estimate that does not update with live ocean conditions). `null` when both the resolver chain and the SRF fallback have nothing. |
| `spectralComponents` | list[SpectralWaveComponent] | No | **Corrected 2026-08-07 (ADR-096 Amendment 2, marine `8e17e84`) — no longer NDBC.** Sourced from the SWAN model's own `multiSwell` deep-water-reference decomposition, taken from the first forecast timestep that has one (same source as `forecast[].multiSwell`) — "everything coming from the same source, from our model and not a buoy" (operator ruling 2026-08-07). The NDBC buoy fetch was removed from the surf endpoint entirely; it is no longer called at all. Empty list if no timestep this forecast has a deep-water-reference decomposition. |
...
configured choice. The API forwards it only through the authenticated
API-to-marine proxy; the marine service rejects an unknown or ineligible value.
The returned score is always server-scored for that one selected species—the
Dashboard never derives or substitutes a score.
...
| `species` | list[str] | No | Derived practical choices exposed to the visitor; not an unvalidated raw catalogue list |
| `targetCategory` | str | No | Selected fishing category used by setup's matrix derivation (config may retain multiple categories) |
| `speciesSelection` | object | No | Derived selection record: region, category, selected choice, candidate group, disclosed source-species members, effective profile level, and complete-profile comparison result |
| `pressureCompatibility` | object | No | Live per-location provider check: `supported`, provider, check time, coverage window, and an unavailable reason. Setup blocks enabling or saving Fishing when the selected provider cannot supply the required hourly pressure series at that location. For NWS this is a real location-specific grid check, not a provider-wide assumption. |
...
| `tidePredictions` | list[TidePrediction] | No | CO-OPS tide predictions, also used server-side to derive each period's tide/current state |
| `source` | str | No | Composite source roles for this response; does not imply that NDBC supplied local weather or pressure |
| `generatedAt` | str | No | UTC ISO-8601 with Z |
...
> served values (surf score, `multiSwell`, break metrics, per-transect profiles) move because the inputs move, but
> their contract is unchanged. Do not add fields to the API in the geometry work.
...
Each partition × each transect → independent SwellTrack run (handoff to shore)
  ...
  → break point: Q_b (Battjes-Janssen statistical breaking fraction) state machine —
    onset where Q_b rises through 0.05, cessation where Q_b falls through 0.02 AND
    H<=Gamma*d; published marker = the zone's local-maximum-dissipation point, not the
    onset crossing (Round X, ADR-102; see §18 breakPoints for the full semantics)
...
open-only; structure-affected transects participate on their own merits):
  ...
  → breakPoints (V3-F4-IMPL, 2026-08-02, operator-approved) = every break
    point across every partition on the representativeTransectIndex
    transect, mapped to {distance, depth, hs}; null iff
    representativeTransectIndex is null or that transect has no break
    points in any partition
...
> **Dominant swell selection — corrected 2026-08-07 (ADR-096 Amendment 2, marine `8e17e84`, operator ruling: "This is not a design decision, this is a physics decision").** Every "dominant partition" pick in the pipeline above (`swellHeight` in the diagram, and every other place a single multiSwell partition is chosen as "the" swell) is now selected by **total partition energy**, proxied as `Hs² · Tp` — proportional to deep-water wave power — not by raw partition height and not by the partition's own `energy` field (which is peak spectral *density*, m²/Hz, and used to incorrectly rank short-period wind swell above a longer-period swell carrying more total energy). This matches the NDBC standard (waveobs.shtml): "If more than one swell is present, this is the period of the swell containing the maximum energy." Implemented once as `_partition_energy(c)` in `surf_scorer.py` and used by `_total_energy()`, `_swell_dominance()` (the `swellDominance` field below), `_dominant_partition()` (Power scoring), `_effective_swell()` (the display period/direction that feeds `conditionsText`), `_cross_swell_score()` (interference detection), and `endpoints/surf.py`'s own dominant-partition selection for the `swellHeight` display field — six call sites, one criterion. `score_surf()`'s Size factor is unaffected: it still receives the bulk `wave_period`/`wave_direction`, not a dominant-partition value.
>
> **Dominant-ELIGIBILITY floor — added 2026-08-13 (operator ruling R1: "a train with period < 5 s must never be flagged dominant" — it "may exist and may be listed," just never selected; Z3.10-I1 finding F1).** Before the energy-ranking above runs, every dominant-selection call site now filters candidates to `period >= _MIN_SURFABLE_PERIOD_S` (5.0 s, inclusive at exactly 5.0 — the same shared floor constant `periodMinS`/`periodMaxS` and `swellHeightMinFt`/`swellHeightMaxFt` above already use, `services/surf_1d_pipeline.py`) BEFORE picking the max. This governs SELECTION only — a sub-5 s train stays listed in `multiSwell` and in every other unfiltered component list untouched; it can simply never become "the" dominant one. Applies to `_dominant_partition()` and `_effective_swell()` (both `surf_scorer.py`) and `endpoints/surf.py`'s `swellHeight` selection — three of the six call sites above — plus a SEVENTH call site not previously named in this callout: `services/surf_1d_pipeline.py::_compute_peel_angle()`'s reference partition (the peel-angle/`peelClassification`/`peelDirection` fields below), whose representative partition is now the FIRST ELIGIBLE partition in Hs-descending order rather than unconditionally index 0. `_total_energy()`, `_swell_dominance()` (`swellDominance` below), and `_cross_swell_score()` are UNCHANGED — they still sum/consider every partition, sub-floor included. Zero eligible partitions at any of the four sites returns that site's own pre-existing "no partitions" contract (never a fabricated substitute): `_dominant_partition()` and `_compute_peel_angle()` return their existing null/`(None, None, None)` sentinels; `_effective_swell()` and `endpoints/surf.py`'s `swellHeight` fall back to their existing bulk/reference-point values. `_score_power()` (the `power` row below, `_dominant_partition()`'s sole caller) scores the SAME neutral 0.5 on zero eligible partitions as it already does when `multiSwell` itself is null.
...
>
> **Updated 2026-08-02 (V3-F4-IMPL).** This block's own QB peaks no longer feed `breakPoints` — that field moved to the SwellTrack 1D-pipeline flow above (see the `representativeTransectIndex`/`breakPoints` lines there). `select_reference_point()` itself, and everything else in this block, is unchanged: it still selects the reference point and still drives `swellHeight`/`directionalSpread`/the `HSIGN` skip-gate exactly as documented.
...
**Forecast window is up to 72 h, honestly shortened when the newest GFS cycle is not yet published** (operator ruling 2026-08-02, MARINE-FORWARD-PLAN.md V4/TA-C16). The card does not pad or extrapolate to a fixed 72 h when the GFS forecast needed for hours 48–72 has not posted yet — it serves however many hours it can actually forecast (as short as ~66 h has been observed) rather than fabricate the tail. This is correct behavior, not a defect: a shorter, honest window is preferred to a longer window backed by data that does not exist yet.
...
**Cross-shore CURVE transect output (ADR-095).** Each surf spot gets a CURVE transect perpendicular to the beach, from ~15m depth to ~1m depth, ~50m spacing (10–20 output points). Direction derived from `beach_facing_degrees + 180°`. **`beach_facing_degrees` becomes the per-transect isobath-normal** (target — Marine Geometry-Model Plan G1; ADR-093 Amendment 5 AD-1), so the CURVE's shoreward direction (`+ 180°`) reconciles automatically — the `+ 180°` relationship is unchanged, only the facing it is measured from is now the local isobath-normal rather than the drawn-segment perpendicular. Replaces the single-point OUTPUT POINTS command. TABLE output at each transect point: `HSIGN HSWELL DIR TM01 DEPTH QB DISSURF DSPR XP YP` (SETUP removed — SWAN SETUP command disabled in parallel OpenMP runs; `setup` field returns `null` in API responses). Break points identified by QB peaks along the transect.
...
**Two-tier schedule (rebuilt by Marine Model Restoration Plan E8, operator-approved 2026-07-27 — supersedes the "stationary inner nest only" quick-update below; see PROVIDER-MANUAL §14.15 "Two-tier schedule" for the full table):** Full runs fire on the four extended HRRR cycles/day (00/06/12/18Z) — full nest `L1 → L2 (+ L3 + L4 where triggered)`, 72-hour nonstationary forecast, ~7–12 min runtime. Every intervening (non-extended) HRRR cycle fires an **hourly stationary "fill"** — the *same full nest* run as a single snapshot with the latest HRRR wind (`SWANRunner.run_stationary_full_nest()`), reusing the last full run's WW3 boundary rather than re-fetching, then running the same 1D SwellTrack chain and merging into the existing forecast cache. The fill therefore refreshes the actual surf card at every spot — including open-beach (L2-only) spots — not just SWAN grid points, which the pre-E8 "L3-only" quick update did not do. A fill can be a no-op (post-full-run cooldown, no reusable WW3 boundary, lock contention, or zero usable spots); `run_quick_update()` returns whether it updated any spot, and only a real update advances the staleness signal (E8 finding F2). Not in the request path. Cache TTL = 6 hours (matches extended cycle interval). On failure, last-good cache retained indefinitely — stale SWAN data is always preferred to no data.
...
| `peelClassification` | Peel angle classification | Base quality class `"closeout"` (<30°) / `"fast"` (30-45°) / `"good"` (45-66°) / `"mellow"` (>66°), with a direction suffix (`_right`/`_left`/`_a_frame`) appended when direction is determined and the class is not `closeout` — e.g. `"fast_right"`, `"good_a_frame"`. Plain base when direction is undetermined. `"closeout"` is ALWAYS plain, never suffixed — closeouts don't peel in a direction. **Corrected 2026-08-02 (D8):** a prior pass of this row (2026-08-01) read "plain, undirected" for all four classes — that was a sampling artifact from an all-closeout-hours capture (closeout being the one class that happens to always be plain). Live-confirmed against `surf_1d_pipeline.py:750-786` 2026-08-02. Direction is also available independently via `peelDirection` below — do not parse this field for direction. |
| `peelDirection` | **NEW to this manual 2026-08-02 (D8) — was already served, previously undocumented here.** Break direction descriptor | `"right" \| "left" \| "a_frame" \| null`. Computed INDEPENDENTLY of `peelClassification` — served even on closeout hours (a geometry descriptor, not gated by peel quality; live-confirmed 2026-08-02: 36/36 hours `closeout` + `a_frame` on the same day). A consumer that wants "does this wave peel" must still check `peelClassification != "closeout"` separately — this field alone does not encode that. Source: `surf_1d_pipeline.py:750-786`. |
| `closeoutFraction` | **Added S3 (2026-08-27, `endpoints/surf.py:1479`) — PEEL-SEGMENTS, operator-approved 2026-08-26.** Fraction of the beach that closes out this hour | `float \| null`, `0.0`–`1.0`, rounded 2dp at the wire. `peelAngle`/`peelClassification`/`peelDirection` above report the BEST of the PEEL-SEGMENTS sliding windows (3 adjacent transects, stride 1, up to 2 pairs) — the peak a surfer picks; `closeoutFraction` is the fraction of those SAME windows whose own peel angle is below `_PEEL_CLOSEOUT_MAX_DEG` (30°, `surf_1d_pipeline.py`), i.e. how much of the beach is NOT surfable this hour even though the best window may be. `null` when peel is unavailable entirely, or when the segmented computation did not run (fewer than 2 valid windows — a tiny transect set falls back to the pre-PEEL-SEGMENTS whole-beach computation, byte-identical, and `closeoutFraction` stays `null` for that fallback by design). Source: `services/surf_1d_pipeline.py::_compute_peel_angle()`, `PipelineResult.closeout_fraction`. |
...
**All scoring data from SWAN + SwellTrack (ADR-096, amended).** NDBC spectral data is NOT passed to the scorer. The scorer uses multi-transect SwellTrack output for wave height (best peak or average from open transects), deep-water SPECOUT for swell composition, and SWAN TABLE for DSPR. **Corrected 2026-08-07 (ADR-096 Amendment 2):** NDBC is no longer fetched by the surf endpoint at all, so there is no NDBC data left to be "retained as reference data" — `spectralComponents` is now sourced from SWAN's own `multiSwell` (see the Surf bundle table above), same as scoring and `multiSwell` always were.
...
**Inputs:** The selected species' time-matched nearshore weather, provider-neutral
three-hour pressure change, CO-OPS tide/current state, depth-appropriate ocean
temperature, solunar marker, seasonal activity, and current period
window. NDBC is never the Harbour-local pressure or weather source.
...
1. An out-of-range depth-appropriate temperature is a hard stop: score `0`,
   status `inactive`.
2. Otherwise `core = 100 × temperature^0.50 × tide_current^0.30 × pressure^0.20`.
   Temperature is `1.00` optimal, `0.80` good, `0.45` marginal-but-active, and
   `0.00` outside the active range. Tide/current is `1.00` preferred, `0.70`
   acceptable, or `0.40` poor. Pressure uses the selected profile's researched
   falling, stable, or rising multiplier and pressure sensitivity, selected by
   the signed local three-hour trend; stable applies when its absolute change
   is below 1 hPa. There are no universal pressure-band scores.
3. Apply only documented bounded refinements: time `1.10/1.00/0.90`, season
   `1.15/1.00/0.85`, and the profile's major/minor, active-tide-alignment, and
   full-moon solunar terms. `final = min(100, core × time × season × solunar, core + 10)`.
4. Status is `active` only at final `65–100` with core at least `55`,
   `less_active` at `35–64`, `low_activity` below `35`, and `inactive` for a
   hard stop. Every selected row has complete profile treatments; runtime never
   turns an unfinished or blank matrix field into a neutral adjustment or
   borrows an adjustment from another species.

There is no generic Fishing score and no permanent pressure-sensitivity penalty.
The response exposes the signed pressure trend as factual context. The
visitor-facing page remains a simple selected-species score and explanation;
...
Red List assessment. A taxon assessed as `critically_endangered`, `endangered`,
or `vulnerable` is excluded from the selectable matrix. A `near_threatened`
taxon remains selectable, but its operational matrix row carries only the
`near_threatened` conservation flag. `least_concern`, `data_deficient`, and
`not_evaluated` records do not receive a conservation flag. The operational
matrix carries no IUCN assessment identifier, research URL, or other research
provenance; those are inputs to reconstruction and review, not runtime fields.
The screen applies to the exact source member, never directly to a familiar
collapsed group label. After excluded members are removed, setup may continue
to show that group only when every remaining eligible member has the same
...
| `FishingForecast.status` | "active" | `fishing.species_status.<value>` | `i18n.t("fishing.species_status.active")` |
...
|---|---|---|---|
| `observation.windSpeed` | Station hardware when `is_station_served(location.id)` | Otherwise the configured forecast provider at the marine location coordinates | Knots in API internal units |
| `observation.windDirection` | Station hardware when station-served | Otherwise the configured forecast provider at the marine location coordinates | Degrees |
| `observation.windGust` | Station hardware when station-served | Otherwise the configured forecast provider at the marine location coordinates | Knots |
| `observation.airTemp` | Station hardware when station-served | Otherwise the configured forecast provider at the marine location coordinates | °C internal |
| `observation.pressure` | Station hardware when station-served | Otherwise the configured forecast provider at the marine location coordinates | hPa internal; the three-hour trend uses the same source's archive or time-matched hourly series |
| `observation.visibility` | Station hardware when station-supported and station-served | Otherwise the configured forecast provider at the marine location coordinates | km internal |
| `observation.waveHeight` | Surf-spot locations only: the wave model's own computed swell breakdown via `services/model_wave_source.py`, never WaveWatch III. | `null` when model output is unavailable and for every non-surf selected location. NDBC values remain only in separately labelled Offshore Observations. | Meters internal. |
| `observation.waterTemp` | Coverage-aware `ocean_data_resolver.resolve()`: nearby local sensor first, then an OFS or regional source that covers the selected point, then national fallback | Clearly labelled buoy or tide-station observation only as a last resort; otherwise null | °C internal. WCOFS is eligible only on its West Coast coverage; it is never a nationwide default. The response retains selected source, source type, valid time, coverage tier, and depth. |
| `observation.weatherCode` | Configured forecast provider at the marine location coordinates | null | WMO code integer; never selected from NDBC |
| `observation.isDay` | Configured forecast provider at the marine location coordinates | null | boolean; never selected from NDBC |
...
**Offshore-only fields:** NDBC `dominantPeriod`, `averagePeriod`, and
`spectralComponents` appear only in an `OffshoreMarineObservation` record when
available. They are not fields of `MarineObservation` and never supply
selected-point conditions.
...
| `waterTemp` | float \| null | **Added 2026-08-02 — was missing from this table.** Top-level sea surface temperature, °C, ocean-data-resolver sourced — see the §16 surf-bundle table above for the full provenance/fallback note (distinct from the separate `zoneForecast.waterTemp` NWS SRF value). |
...
| `originLat` / `originLon` | float \| null | **NEW 2026-08-09 PM (P16, C3-COORDS, operator ruling, marine `223182e`).** This transect's real WGS84 origin — the point on the operator-drawn shoreline segment the transect extends seaward from (`TransectInfo.origin_lat/origin_lon`, already computed at spot setup — served, not recomputed). Present in every `transect_index` mode; `alongshoreM` (below) is `all`-mode only. |
| `alongshoreM` | float \| null | **NEW 2026-08-09 PM (P16, C3-COORDS), `transect_index=all` mode ONLY.** Cumulative geodesic (haversine) alongshore distance in metres from row 0's origin, monotonic by served-row order (ascending `transectIndex`; a failed/skipped transect is simply absent, never interpolated). Row 0 = `0.0`. `null` for a row whose `originLat`/`originLon` are themselves null. Unblocks the dashboard heatmap's ground→chart transform (was previously computable only from `transectIndex` × nominal spacing, which the operator rejected as not physically georeferenced). |
...
| `breakPoints` | list[object] \| null | **Round X detection semantics (2026-08-05/06, marine `bce8997`, operator-approved, ADR-102) — supersedes the bare "H/d = gamma crossings" description below for the production path.** A break point is no longer a bare ratio crossing: `_ddd_breaking_march()` runs a Battjes-Janssen statistical breaking-fraction (Q_b) state machine — onset where Q_b rises through 0.05 (visible/webcam-meaningful), cessation where Q_b falls through 0.02 AND H<=Gamma*d, re-onset forbidden shoreward of the 15 cm reform floor — and the published marker per breaking zone is that zone's LOCAL-MAXIMUM-DISSIPATION point, not the onset crossing itself. `run_1d_analytical()` (the only production caller) always passes these modeled onsets into `_find_break_points()`, which then applies only the publication floors below (depth, Hs) — no ratio scanning, no hysteresis, in production. **The re-arm-hysteresis paragraph immediately below (Round Z) describes `_find_break_points()`'s LEGACY `onset_indices=None` branch** (a bare H/d>=gamma ratio-crossing detector with re-arm hysteresis) — unchanged by Round X, but **not the path any production caller uses today**; it exists for a future caller with no modeled breaking state. Depth floor 0.15 m and Hs floor 0.10 m (below) apply identically to both paths, applied as a publication filter after either detector runs. See ADR-102 for the full Q_b/roller design and worked examples. Each carries `partitionInfo` (`partitionIndex`/`periodS`/`directionDeg`/`classification`/`heightM`), nullable — see "Two partition index spaces" above. **Primary vs. non-primary entries (BD-4, `SURF-ZONE-BREAK-DETECTION-SPEC-2026-08-01`, marine `03b33e1`/`ea62e85`/`b60ef92`, ADR-093 Amendment 7):** for each swell partition with break points, this array carries exactly one entry for the PRIMARY break (the partition's larger-face break — usually but not always the outermost/most-seaward) plus one entry for every OTHER break point that partition has (a genuine double-break day: outer bar + inner/shore break). **The primary entry's `distance`/`depth`/`breakerType`/`iribarren` are always the SAME break as its own `faceHeight`** (adversarial audit Finding F2 fixed this pairing — before the fix, a double-break day with a bigger inner break could report the outer break's position against the inner break's face height). Every non-primary entry likewise pairs its own geometry with its own freshly-computed face height — never the primary's. **No field in this array marks which entry is primary** — there is no `role`/`isPrimary`/similar key; the array is a flat list of break-point dicts, re-sorted purely by `distance` (seaward→shore) after every partition's entries are appended, so primary and non-primary entries from different partitions interleave by position. A consumer that needs to know which entry is "the" reported break for a given partition must group this array's entries by `partitionInfo.partitionIndex` and take the entry with the MAXIMUM `faceHeight` within that group — that is exactly the server's own `primary_break_index` selection rule, reproducible client-side; `perPartitionBreaks[].meanFaceHeightM` is a cross-transect MEAN and is not usable to identify a single transect's primary entry. **LIVE (shipped 2026-08-11, Phase K of MARINE-PAGE-FIXIT-PLAN, marine `57ee18d`+`daa0b14`, ADR-106 R3, PA4) — detection semantics, supersedes the Round X onset/cessation description above for the PUBLISHED path only.** Operator review found the cessation-gated marker rule (onset through 0.05, cessation through 0.02 AND H≤Γ·d) glues a beach's outer and inner breaks into one zone whenever Q_b never drops below cessation between them, so a visibly-crashing beach break gets no marker regardless of threshold tuning. **As shipped:** published marker detection DECOUPLES from cessation-gated publication — after the wave-height march completes, `_select_break_markers()` builds candidate regions with hysteresis (coordinator ruling (a), 2026-08-11, resolving the plan's literal single-threshold sentence against its own examples): a region STARTS where `Qb_i ≥ qb_breaking_onset` (new operator-adjustable config key `[surf] qb_breaking_onset`, default 0.05, this ADR's own unchanged `Q_B_VISIBLE` value) and stays alive while `Qb_i ≥ Q_B_CESSATION` (0.02, the march's own unchanged constant — mirroring the march's onset/cessation asymmetry, so a real profile's Qb dip below onset but above cessation does not split a physically continuous breaking region); within each region, a local maximum of dissipation publishes a marker only if its topographic prominence is ≥30% of the region's tallest maximum (`_BREAK_PROMINENCE_FRACTION = 0.30`, pinned, not configurable). This array may therefore carry MORE than one entry per swell partition within what is physically one breaking region (not just a genuine multi-bar day) — a deliberate, operator-accepted consequence, since that is exactly the mechanism that makes the beach break's marker appear. Existing `_MIN_BREAK_DEPTH_M`/`_MIN_BREAK_HS_M` publication filters (below) apply unchanged after detection. The internal Q_b solve, one-sided dissipation, and roller-energy reservoir are entirely UNCHANGED — only which points get published as markers changes. |
| `waveShapes` | list[object] \| null | Stokes 2nd order / cnoidal / bore surface shape samples. **Round P (2026-08-05, marine `4e0ff18`):** computed by the module-level `_compute_wave_shapes()` helper on the SAME pipeline arrays published as `transect[]` (blended Hs, tide-aware depths, full transect distances) with the dominant partition's Tp — the endpoint-local `run_1d_analytical()` side-run (dominant-partition-only, `tide_level=0.0`) is deleted. Samples span the full transect distance range (31 evenly spaced samples), as before. Empty when no dominant partition exists this timestep. |
...
| `beachElevation` | list[object] \| null | **NEW 2026-08-05 (Round P).** `[{"distance", "elevation"}, ...]` — the RAW signed CUDEM profile as elevations (`elevation = -signed_depth`, positive up, LMSL datum), native resolution, no clamping, covering all points with `distance <= max(transect distances)`. Land carries positive elevation (ADR-093 Amendment 4 signed sampling); this is the only field that publishes the unclamped land/beach geometry — `transect[].depth` remains solver-input depths clamped at 0.01 m. |
...
**`units` object:** `distance` and `depth` use the operator's configured distance display unit (foot or meter — same resolution path as `group_wave_height`'s unit group); `hs` uses the wave-height display unit. **Round P additions (api `ac96064`):** `waterlineDistance` and `beachElevation`'s per-item `distance`/`elevation` convert on the same distance path; `tideLevel` converts on the water-level path (entry pre-existed in the conversion table at `services/marine_response_conversion.py`). **Round Z addition (api `c99f6d5`):** `perBreakZones[].breakDistance` converts on the distance path with a `units.breakDistance` entry; the nested `impactZone`/`foamZone` start/end distances and depths are covered by the pre-existing leaf-name entries (the converter recurses into any list-of-dicts by structure and resolves groups by leaf key name — verified at `_walk`/`_resolve_group`). Unit conversion is applied to all wave-height and distance/depth fields; nothing in this response is a fixed physical unit.
...
- **`modelHealth`** (R8a/R8b, schema version 1): the additive marine model-health ledger. It contains `overall`, `serving` (including `selectedFullCycleId`), `attempts` (`active` and latest `full`/`fast`/`horizon`/`recovery`), and the required stage tree with provider and SWAN children. Compact attempt summaries include package, SWAN binary, WW3-pin, configuration, and grid identities from existing runtime authorities; absent identity evidence cannot be reported as a successful attempt. R8b persists those summaries and producer-owned observations for the WW3 leg, raw horizon, verified boundary merge, WCOFS currents, SWAN, SwellTrack, cache, and publication; absent evidence remains `unknown`/`not_instrumented` and can never become healthy by inference. No visitor response field changes. The state vocabulary, precedence, required-stage matrices, and evidence rules are in OPERATIONS-MANUAL.md “R8a `modelHealth` ledger”.
- **`modelTestHold`** (testability correction, additive): `{requested: bool, acknowledged: bool}` for the runtime-only marine test reservation. `requested` reflects the hold sentinel and `acknowledged` is set by the marine runner after it observes that request; both remain opaque to the API and visitor responses.

R8b/R8c are **not** a new API schema: `GET /setup/marine/health` remains an
authenticated, opaque pass-through of the marine service's `/health` JSON.
The API does not model `modelHealth`, `currentForcing`, internal provenance,
attempt identities, or stage observations, and no visitor response gains a
field. The Stack admin view parses the existing pass-through defensively for
operator display; CheckMK is not integrated. This is the current source shape
(`endpoints/setup.py::marine_health`, with the companion proxy's existing
transport, and Stack `admin/routes.py`), not live verification.
...
- `degraded` — the cycle completed and every input is available, but an input is stale past its own monitoring threshold, or a B2 invariant fired at or after `last_run`, **or a recorded H1 no-publish/L3-viability reason is live (below)**. R3's existing `no-publish: l2_boundary_exhausted` reason has the same status effect; the API passes it through unchanged.
- `ok` — otherwise.
...
- A **SWAN-cycle no-publish reason** — `"no-publish: <slug> <detail>"`, one of `no_grid_sizing_cache`, `hrrr_wind_failed`, `gfs_wind_failed`, `ww3_boundary_failed`, `tide_fetch_failed`, `currents_fetch_failed`, `bathymetry_failed`, `wave_setup_failed`, `swan_fatal`, `convergence_gate_failed`, `no_usable_handoff_timesteps`, `l2_boundary_exhausted` — recorded when the current SWAN cycle produces nothing to publish, cleared by the next cycle that actually publishes. R3's `l2_boundary_exhausted` is recorded after the L2 PRINT/convergence check when its PRINT reports `data on boundary file exhausted`; it takes precedence over a simultaneous convergence failure. Full runs re-raise for the same-cycle retry and quick updates return no update, preserving last-good output in both cases. A persisted successful fast update resets the exhaustion flag and clears only the matching exhaustion reason. The health reducer emits this no-publish reason once; for legacy/restored flag-only state it reports `attemptCycle=unknown` with separate `pendingFullCycle` and `lastGoodCycle`, never an invented fast-cycle identity. (`hrrr_wind_failed`/`gfs_wind_failed` cover the inline wind-fetch fallback at the top of `_run_all_spots_locked()` — `hrrr_wind_failed` is normally unreachable in production, since the runner loop always pre-fetches HRRR and passes a non-`None` result; `gfs_wind_failed` IS production-reachable, since the runner loop can itself pass `gfs_wind_field=None` on its own GFS fetch failure and this inline retry can also fail.) **`wind_coverage_failed`** — 14th instrumented exit, extends this registry (Phase W, L1-BOUNDARY-REBUILD-PLAN-2026-08-08, P6/D5-D6): recorded when the blended (stitched HRRR+GFS, or HRRR-only for the fill) wind field does not cover the L1 domain + a 0.3° pad. Two detectors raise the SAME `WindCoverageError`: the fetch-time bounds check (`providers/nearshore/swan.py`'s `_assert_wind_covers_domain()`, called before any SWAN input build — the PRIMARY detector) and, as defense-in-depth for anything that slips past that bounding-box check, `services/swan_formats.py`'s `hrrr_to_swan_wind()` (raises when interpolating onto the SWAN CGRID hits a wind cell outside the fetched grid). This replaces the former silent NaN→`0.0000` calm fill in `hrrr_to_swan_wind()`. The wind fetch bbox itself (`config/marine_config.py`'s `wind_fetch_bbox()`) is now derived from the L1 SWAN domain (the grid sizing cache) padded 0.3° on every side, replacing the old spot-±1.0° margin. **Both paths are covered, not just the 6-hourly full run:** `_run_all_spots_locked()`'s catch around `run_3level()` records the slug and re-raises (aborting the cycle, same shape as every other row in this list); `_run_quick_update_locked()`'s catch around `run_stationary_full_nest()` (the hourly stationary fill) records the SAME slug but does NOT raise — it returns `False` (fill skipped this hour, last-good forecast untouched), preserving that path's existing no-raise contract. Gate W's blind audit (2026-08-08) flagged the original W1-W4 delivery for covering only the full-run path — an hourly-cadence gap that could leave a recurring wind-coverage failure invisible to `/health` for up to 6 hours between full runs; both paths have been covered since.
- A **per-cluster L3 viability failure** — `"l3_viability_failed: <cluster_key> <detail>"` — recorded at config-push time when a cluster's L3 grid was triggered (a structure or operator classification justified it) but failed ADR-093 Amendment 2 §4's viability test (the grid cannot reach the feature it was sized for). Rebuilt fresh at every config push, independent of how many SWAN cycles run in between — L3 viability does not change just because a cycle using the L2 fallback succeeded. **Reading this reason correctly (2026-08-03, DIAG-FETCH part D):** it refers to the *smart-sized* L3 candidate grid (the fine feature-resolving grid the trigger asked for, which is also not wired as a handoff source today — documented gap, `transect_handoff.py`). A cluster with this reason live can STILL run an L3 grid in the SWAN cycle: the *coarse L4-containment nest* (a different role — it exists to contain the L4 grid, not to resolve the feature) is sized independently and, when containment succeeds, runs as `level3_0`. So "`l3_viability_failed` + a `level3_0` run in the journal" is a consistent, non-contradictory state, not a health-reporting bug.
...
### §19.7a WW3 deep-water leg + chain-serves surface (as-built; ADR-109, CHAIN-SERVES round, operator order 2026-08-19)

This subsection documents the WW3-leg + production-chain surface as built (ADR-109, CHAIN-SERVES round). No WW3-leg endpoint exists on the API — every WW3/chain signal reaches the admin surface through the existing opaque marine-health pass-through (below).
...
**Wizard/admin display — pointer only, no UI exists yet.** The wizard/admin install-state DISPLAY surface for the WW3 leg (health/status rendering, and the `ww3_chain_enabled` toggle) is designed but not built until Phase W task W5. When it lands, the CLAUDE.md doc-sync table's help-content-key process rule applies in full: `help.wizard.{step_id}.*` / `help.admin.{section_id}.*` keys land in the SAME commit as the UI change, not a follow-up. No wizard/admin surface change is authorized by this subsection alone.
