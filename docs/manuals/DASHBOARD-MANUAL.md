...
- **`(ruled 2026-08-10; lands with Phase M of MARINE-PAGE-FIXIT-PLAN)` — map layer / tile-error-handling contract, target state (fixit log Item 6).** The main map's base-tile layer has no error handling today: a failed tile server produces silent gray (base layer missing, street-name overlay + pin layer still rendered on top) with no visible signal. Target state: both `TileLayer`s (base + labels) gain `tileerror`/`tileload` handlers via `eventHandlers`. After **3 consecutive tile errors** on a layer, a non-blocking banner appears over the map ("Map imagery failed to load — retrying"); the layer retries via redraw with a **5 s backoff**, max **3 retries per mount**; the banner clears on the first successful load. Silent gray becomes impossible: either tiles render, or the banner is visible — never neither. Separately, a theme change (light/dark, including the sunrise/sunset auto-theme's self-correction) currently tears down and rebuilds only the base layer, leaving the street-names layer's already-loaded images in place — a second, independent way to produce "labels and pins, no map." Target state: the per-layer `key={baseTile.url}` moves UP to the `MapContainer` itself, keyed on `resolvedTheme`, so the whole map remounts atomically on a theme flip and labels can never outlive the base layer. No change to theme-provider logic.
...
3. **Swell Card** — `Card footprint="wide"`:
   ...
   - **C1 — server-computed range fields (2026-08-08, L1-BOUNDARY-REBUILD-PLAN Phase C, P13, `SurfingTab.tsx` "Card 3" stat-tile row):** when `SurfForecast.swellHeightMinFt`/`MaxFt` are both non-null, Swell Height renders "min–max ft" instead of the single `swellHeight` value (equal min/max collapse to one number, via a local `formatMinMaxFt()` helper — no client-side eligibility logic, the server has already decided which components count per wind-swell-period-< 5s exclusion rule). Same pattern for `faceHeightMinFt`/`MaxFt` on Breaking Face Height. `combinedPeriodS` (energy-weighted mean period, one decimal) replaces the raw `period` value on Period when present. Any single field null/absent (pre-C1 cached response) falls back to the exact pre-C1 expression for THAT stat only — dumb renderer, additive fields, incoming-swell table below unchanged.
   - **`(ruled 2026-08-10; lands with Phase S of MARINE-PAGE-FIXIT-PLAN, ADR-106 R2, PA2)` — Breaking Face Height and Period bindings target state, supersedes the `faceHeightMinFt`/`MaxFt` and `combinedPeriodS` bindings in the C1 bullet above.** Operator review (fixit log Item 2) found Breaking Face Height was wired to the wrong number — the spread across swell trains, which collapses to one value whenever one swell dominates — instead of the spread across the best-surf strips of beach, which is what the operator's definition describes and what the server already computes. **Target state:** Breaking Face Height rebinds to `modelSurfHeightMin`/`modelSurfHeightMax` (same `formatMinMaxFt()`-style collapse rule: single number iff min = max) — this pair is already served (WC-D3), no new field. Period rebinds to the new `periodMinS`–`periodMaxS` range (same collapse rule) — `combinedPeriodS` is retired; operator ruling, verbatim: *"the period should never be combining periods, that is not how the physics works."* The `faceHeightMinFt`/`MaxFt` bindings and the `combinedPeriodS` binding, and the corresponding fields in `src/api/types.ts` and the openapi mirror, are deleted in the same change (Phase S, dashboard side). Swell Height binding is unchanged by this ruling.
...
7. **Heat Map** — `Card footprint="full"` (T7.1, `HeatMapCard.tsx`; corrected 2026-08-02, D5.3 — a prior pass of this manual claimed no heatmap consumer existed; it does, and has since T7.1):
   ...
   - **Orthophoto background (LM-2, 2026-08-03, dashboard `fed72f8`; rotation/buffer superseded by C3 below — DISPLAY-ONLY):** when `SurfingTab` passes `spotLat`/`spotLon` (from the already-fetched `useSurfDetail` coordinates) and `useImageryConfig(lat, lon)` returns a config (API-MANUAL §12a; 404/any failure → `data: null`, silent degradation), a tile mosaic renders behind the colour cells (SVG `<image>` elements in a `clipPath` group replacing the plain background rect) and the colour-cell opacity drops from `HEATMAP_CELL_OPACITY_DEFAULT` (0.85) to `HEATMAP_CELL_OPACITY_ON_ORTHO` (0.55). No imagery → the pre-LM-2 render, byte-identical (KAT-pinned via golden-DOM comparison). Attribution renders verbatim below the SVG (ToS text, never translated). All tuning constants (`IMAGERY_MOSAIC_MAX_TILES_PER_SIDE=4`, `IMAGERY_ZOOM_MIN=14`/`MAX=19`, both opacities, `IMAGERY_VISIBLE_BUFFER_M=50`) sit in one documented block in `HeatMapCard.tsx`.
