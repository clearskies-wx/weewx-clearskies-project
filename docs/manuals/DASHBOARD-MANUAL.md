...
7. **Heat Map** — `Card footprint="full"` (T7.1, `HeatMapCard.tsx`; corrected 2026-08-02, D5.3 — a prior pass of this manual claimed no heatmap consumer existed; it does, and has since T7.1):
   ...
   - **C3 — ortho rotation, 50m buffer, y-axis title, structure-overlay removal (2026-08-08, Phase C of L1-BOUNDARY-REBUILD-PLAN, dashboard `HeatMapCard.tsx`):**
     ...
     - **50 m visible buffer:** the on-screen tile-placement rectangle (and its clip) extends beyond the core plot rectangle by a pixel amount derived from `IMAGERY_VISIBLE_BUFFER_M / radiusM`, applied independently on each axis (preserving that axis's own existing meters-per-pixel scale) — so imagery is visible 50 m beyond the heatmap extent on all four sides, letting the visitor orient against a landmark (pier, jetty) outside the modeled area.
