...
7. **Heat Map** — `Card footprint="full"` (T7.1, `HeatMapCard.tsx`; corrected 2026-08-02, D5.3 — a prior pass of this manual claimed no heatmap consumer existed; it does, and has since T7.1):
   - Quasi-2D birdseye view: custom inline SVG (not Recharts), X = cross-shore distance (shore right, offshore left, matches Beach Profile), Y = along-shore transect index (row 0 at top). viewBox 820×N (PAD_LEFT=60, PAD_RIGHT=12, PAD_TOP=28, PAD_BOTTOM=52, or 74 when the BD-7/9 overlay legend row below is shown); row height scales between 8–48px to fit up to ~40 visible rows before scroll.
   ...
   - **C3 — ortho rotation, 50m buffer, y-axis title, structure-overlay removal (2026-08-08, Phase C of L1-BOUNDARY-REBUILD-PLAN, dashboard `HeatMapCard.tsx`):**
     - **Fetch footprint — unchanged shape, buffer added:** the imagery REQUEST bbox stays the bounding CIRCLE around `spotLat`/`spotLon` (radius = largest tier-clipped cross-shore distance) the LM-2 note above describes, now with `IMAGERY_VISIBLE_BUFFER_M` (50 m) added to the radius. A circle's own rotation is a no-op, so "the north-up enclosing box of the rotated footprint" (the plan's literal wording) reduces to the same box a circle always has — a rectangular beach-frame footprint isn't derivable without fabricating an along-shore spacing value this data doesn't carry (the Y axis has no along-shore metric spacing, per the LM-2 alignment note).
     - **Display rotation:** the fetched tile mosaic's `<g>` is drawn rotated (SVG `rotate()`) about the CORE chart rectangle's center by `computeImageryRotationDeg(beachBearingDeg)`, so the beach's own cross-shore direction lines up with the real-world compass bearing the transects point along — "so that way it matches" (operator instruction). `beachBearingDeg`: the `representativeTransectIndex` row's `transectBearingDeg` when present and non-null, else the middle row's, else `null` → no rotation (0°). The outer clip (unrotated, sized to the buffered rect below) contains the rotated image; the heatmap grid itself is unchanged (already beach-frame).
     - **50 m visible buffer (C3-fix, 2026-08-09 — corrects a lead-measured live-DOM divergence from this same design; coordinator ruling, option (a)):** the on-screen tile-placement rectangle (and its clip) extends beyond the core plot rectangle on all four sides, but the two axes are sized from DIFFERENT scales, not one shared fraction:
       - **X (cross-shore):** `IMAGERY_VISIBLE_BUFFER_M` (50 m) converted to pixels via the chart's own real x-axis scale — the same slope `distToX()` uses (`CHART_W / (maxDist − minDist)`, in meters). This guarantees the buffer agrees with the drawn x-axis ticks by construction. The prior formula (`IMAGERY_VISIBLE_BUFFER_M / radiusM` × half of `CHART_W`) assumed `CHART_W` spans the full 2×radius diameter; it only ever spans `(maxDist − minDist)`, so it undersized the on-screen buffer (measured ≈33 m instead of 50 m at a live Huntington Beach spot, 2026-08-09).
       - **Y (transect row index):** UNCHANGED — still `IMAGERY_VISIBLE_BUFFER_M / radiusM` applied to half of `chartH`. The Y axis has no real along-shore physical scale anywhere in this component or its data (chartH is a pure layout value, N × rowH) — "50 m of Y buffer" only has meaning in the footprint model's own frame, which is what this formula already expresses. It is NOT rescaled to match the X axis's ruler; those are two non-comparable frames, and mixing them was rejected (coordinator ruling 2026-08-09).
     - **Y-axis title:** a rotated `<text>` reading "Transect" (`transectAxisLabel` key) — the axis is an ordinal transect index, not a physical distance, so no unit is fabricated to match the x-axis (the plan's "same unit family as the x-axis" wording assumed a metric axis this data doesn't have — deviation-with-reason, Gate C). The x-axis already had a title (`distanceAxisLabel`) before this round, so only the y-axis needed one.
     - **Y-axis tick labels (C3-fix, 2026-08-09):** the row-index labels (`row.transectIndex`, next to the y-axis title) use density-aware every-Nth labeling — `labelStep = max(1, ceil(12 / rowH))`, labeling every `labelStep`-th row plus always the last row, fixed 10px font. Replaces a prior `rowH >= 12` all-or-nothing gate that suppressed EVERY label once row density passed one row per 12px (e.g. 162 transects → 0 labels rendered). `rowH >= 12` (labelStep=1) renders identically to before this fix.
     - **Structure-affected-area overlay REMOVED:** the hatching pattern, the per-row reduced colour-cell opacity (was 0.35 when `isStructureAffected`), and the legend note are all deleted together as one feature — the underlying `isStructureAffected` field keeps serving and still backs the sr-only table's "Open" column (data, not a visual overlay); the structure's physical effect stays exactly where it already lives, inside SWAN L4's wave action.
