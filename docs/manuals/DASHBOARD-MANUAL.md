...
6. **Beach Profile** — `Card footprint="full"` (ADR-097; T5.3 rewrite, blended Hs):
   ...
   - **C2 target — dominant-swell-only wave train (ruled 2026-08-08; lands with Phase C of L1-BOUNDARY-REBUILD-PLAN):** the SYNTHESIZED water-surface rendering above currently interleaves every served swell's wave train (overlapping trains on multi-swell days). Target: draw ONLY the dominant swell's train. Selection: the partition R5 serves — `partitionInfo.partitionIndex` carried on the served `breakPoints` (all entries share it post-R5); fallback when no `breakPoints` are served: the largest-face component from the swell list (mirrors the backend's own dominance criterion, no second definition invented). Amplitude/wavelength/breaking envelope of the drawn train is unchanged — only WHICH train(s) draw. Until Phase C lands, every served train renders interleaved as described above.
...
7. **Heat Map** — `Card footprint="full"` (T7.1, `HeatMapCard.tsx`; corrected 2026-08-02, D5.3 — a prior pass of this manual claimed no heatmap consumer existed; it does, and has since T7.1):
   ...
   - i18n keys under `marine.surfing.heatMap.*` (see locale files): `transectIndex`, `openTransect`, `breakHeight`, `breakDistance`, `breakerType`, `shadowedTransect`, `breakerType.{spilling,plunging,surging}`, `mainBreakZoneLegend`, `representativeLegend`, `representativeSuffix`, `mainBreakZoneDesc`, `representativeDesc`, `modelUnavailable`, `loadError`, `orthoImageryDesc` (LM-2).
   - **Orthophoto background (LM-2, 2026-08-03, dashboard `fed72f8` — DISPLAY-ONLY):** when `SurfingTab` passes `spotLat`/`spotLon` (from the already-fetched `useSurfDetail` coordinates) and `useImageryConfig(lat, lon)` returns a config (API-MANUAL §12a; 404/any failure → `data: null`, silent degradation), a tile mosaic renders behind the colour cells (SVG `<image>` elements in a `clipPath` group replacing the plain background rect) and the colour-cell opacity drops from `HEATMAP_CELL_OPACITY_DEFAULT` (0.85) to `HEATMAP_CELL_OPACITY_ON_ORTHO` (0.55). No imagery → the pre-LM-2 render, byte-identical (KAT-pinned via golden-DOM comparison). Attribution renders verbatim below the SVG (ToS text, never translated). All tuning constants (`IMAGERY_MOSAIC_MAX_TILES_PER_SIDE=4`, `IMAGERY_ZOOM_MIN=14`/`MAX=19`, both opacities) sit in one documented block in `HeatMapCard.tsx` (~:155-190). **Alignment model (operator-eyeball item, honest limits) — CURRENT (superseded by C3 target below):** the mosaic covers a north-up, UNROTATED bounding circle around the spot (radius = largest tier-clipped cross-shore distance) stretched to the chart rectangle — it is NOT a per-cell geographic warp and is not rotated to the beach facing; consistent with this card's quasi-2D birdseye framing. Tile-count cap binding logs `console.debug` (never silent). Decorative tiles are `aria-hidden`; the `<desc>` gains one sentence (`orthoImageryDesc`).
   - **C3 target — ortho rotation, buffer, y-axis labels, structure-overlay removal (ruled 2026-08-08; lands with Phase C of L1-BOUNDARY-REBUILD-PLAN):**
     - **Rotation, client-side, provider-agnostic:** the imagery layer draws rotated by the beach bearing about the chart frame, so imagery and the heatmap grid (already beach-frame) share one frame — "so that way it matches" (operator instruction). The heatmap grid itself does not change. The imagery REQUEST bbox becomes the north-up enclosing box of the rotated heatmap footprint + buffer (padding computed, never hardcoded) — replaces the current unrotated-circle alignment model above. This is the possible-future-refinement the LM-2 note above flagged as not yet landed; C3 is where it lands.
     - **50 m ortho buffer:** visible imagery extends 50 m beyond the heatmap extent on all four sides, so the visitor can orient against a landmark (pier, jetty) outside the modeled area.
     - **Y-axis labels:** tick labels + a unit title in the same unit family the x-axis uses; if the x-axis itself lacks a title, both get one in this same change.
     - **Structure-affected-area overlay REMOVED:** the affected-area layer and its legend entry are deleted, dashboard-display-only — the underlying API field keeps serving (deprecation noted here, not a wire change), and the structure's physical effect stays exactly where it already lives, inside SWAN L4's wave action. Removed because the overlay was confusing and duplicated information the wave field itself already carries.
     - Until Phase C lands, the heatmap renders per the CURRENT bullet above: unrotated, no buffer note, no y-axis labels, structure-affected overlay present.
