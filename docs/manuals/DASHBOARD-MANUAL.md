...
3. **Swell Card** — `Card footprint="wide"`:
   - Top row ("CONDITIONS AT BREAK" label): 3 stats — Swell Height (HSWELL from API), Breaking Face Height (**stale label corrected 2026-08-01: NOT a Komar-Gaughan/K-G result on the normal path — K-G is the legacy direct-offshore fallback used only when a deep-water Hs is converted without SwellTrack, API-MANUAL.md "Breaker formulas"; the active value is `breakingFaceHeight` from the SwellTrack pipeline, and since Round 2 (BD-7, ADR-093 Amendment 7) that is the main-break-zone headline when `> 0`, else the whole-area best peak — see API-MANUAL.md's height-fields table**), Period. Direction removed from top row (redundant with compass below).
   - **C1 — server-computed range fields (2026-08-08 Phase C P13; rebindings shipped 2026-08-11, MARINE-PAGE-FIXIT-PLAN Phase S, ADR-106 R2/PA2, dashboard `355a83e` + marine `9562d28` — `SurfingTab.tsx` "Card 3" stat-tile row):** when `SurfForecast.swellHeightMinFt`/`MaxFt` are both non-null, Swell Height renders "min–max ft" instead of the single `swellHeight` value (equal min/max collapse to one number, via a local `formatMinMaxFt()` helper — no client-side eligibility logic, the server has already decided which components count per wind-swell-period-< 5s exclusion rule). Breaking Face Height renders `modelSurfHeightMin`–`modelSurfHeightMax` (same collapse rule; `group_wave_height` dynamic unit) — the spread across the best-surf strips of beach, per the operator's Item-2 ruling; the earlier `faceHeightMinFt`/`MaxFt` binding (spread across swell trains, which collapses whenever one swell dominates) is DELETED, fields retired server-side. Period renders the `periodMinS`–`periodMaxS` range (same collapse rule) — `combinedPeriodS` is retired; operator ruling, verbatim: *"the period should never be combining periods, that is not how the physics works."* The retired fields are gone from `src/api/types.ts` and both openapi files. Any range field null/absent (pre-range cached response) falls back to the pre-C1 single-value expression for THAT stat only — dumb renderer, additive fields, incoming-swell table below unchanged.
   - **Shadow face height** (D10.2, 2026-08-04, restores the T7.3 deferred field): a muted secondary line ("In pier shadow: {height} {unit}") directly under the top-row stats, rendered only when `shadowFaceHeight` is non-null — same display-height conversion as the headline stats above. Hidden entirely (not "—") when null (no structure-affected transect that hour).
   - **Set timing section — REMOVED 2026-08-23** (SurfBeat removed from the system; the `setTimingMinutes`/`setAmplitudeM` fields and the `setTimingLabel`/`setAmplitudeLabel` locale keys no longer exist).
   - "INCOMING SWELL (offshore)" section (T5.4, replaces old "Swell Components" label): Swell component breakdown via `multiSwell` in the API response — per-timestep spectral decomposition, not NDBC broadcast. **(Q16 Round B, 2026-08-25)** Source is now primarily WW3's own deep-water reference points (PROVIDER-MANUAL §14.19), not SWAN SPECOUT; hours the WW3 transfer doesn't yet cover fall back to the prior SWAN 15 m table source. The response's `forecast[].swellSource` field (`"deep_reference" | "nearshore_table" | null`) names which source produced a given hour; the dashboard does not currently render it.
   - "AT BREAK" sub-section (**corrected 2026-08-04, D10.2: this bullet previously named `partitionBreakInfo`, a field that was never server-side — restored against the real `perPartitionBreaks` field**, reusing the beach-profile `perPartitionBreaks` shape/serializer): renders only when `perPartitionBreaks` is non-null and non-empty; one row per swell partition, `{period}s {direction} {classification}` → `{distance} {unit}, {height} {unit} {breakerType}`. **No-fallback rule (aud-d102 F1):** a partition with no recorded break (`meanFaceHeightM` null) shows period/direction/classification only — the "→ ..." tail is omitted entirely rather than falling back to the partition's deep-water Hs under the "AT BREAK" heading.
...
6. **Beach Profile** — `Card footprint="full"` (ADR-097; T5.3 rewrite):
   ...
   - **D5 redesign (2026-08-05, dashboard `f85505b` — supersedes the element list above where they conflict):** full scene rebuild to the approved D5 mockup. **Y-axis semantics changed: LMSL-relative ELEVATION (was depth-below-surface).** Scene: sand/bed fill from `beachElevation` (dry, signed) blended with depth-derived seabed; water column with deep→mid gradient; SYNTHESIZED wave surface (display-only rendering — dispersion-driven local wavelength, breaking decay, secondary bumps at non-primary breaks; NOT a model output) with a whitewater band shoreward of the outer break; waterline marker from `waterlineDistance`; still-water/tide datum line; legend row. **D6: `perBreakZones` renders per-break impact bands by DEFAULT (no toggle) when present/non-empty; aggregate `zones` bands are the fallback only. D5.2: any band with width ≤ 0 (float tolerance) is skipped entirely — no fill, label, or legend artifact.** Wave-shapes toggle and jacking-factor UI dropped from the card (API fields untouched). Colors via the `--beach-profile-*` token family (DESIGN-MANUAL §Domain-Specific Colors).
   - **Catch-up display fixes (2026-08-05, dashboard `a36e4e0` + `ad2ecf9`, task DOC-0 — corrects/supersedes the D5 bullet above where they conflict):**
     - **Dry-sand cap (`a36e4e0`):** the landward `xMin` extent (TA-C19/ADR-093 Amendment 4 — `distance` can be negative, so `xMin` extends to the true observed minimum so no point renders off-canvas) is now capped at `waterlineD − DRY_BEACH_MARGIN`, `DRY_BEACH_MARGIN` = 15.24 m (~50 ft). The raw `beachElevation` series can run hundreds of feet inland and was dominating the chart; at most ~50 ft of dry sand now renders landward of the waterline.
     - **Negative X-axis ticks (`ad2ecf9`):** `computeDistanceTicks()` now takes `xMin` and emits negative-distance ticks (at the tier's normal `tickStep`) over the dry-sand strip. When the strip is narrower than one tick step (so the normal series would still start at 0), an extra tick is inserted at the strip's landward edge, rounded to the nearest 5 distance-units, so the axis always shows a negative number over the sand rather than stopping flat at 0.
     - **Caption line removed (`ad2ecf9`).** The single caption line that rendered above the chart (partition period/direction/classification chip, face-height + distance chip, tide chip, waterline chip — translated strings joined with " · ") is gone; it duplicated information shown elsewhere on the tab. The `surfing.beachProfile.captionPartition` / `captionFace` / `captionTide` / `captionWaterline` i18n keys are no longer read by this component.
     - **Still-water and waterline text labels removed (`ad2ecf9`).** The still-water/tide datum dashed line and the waterline dashed vertical line both still render; their overprinting text labels (the still-water "±N.N {unit}" datum label and the waterline chip repeated as a near-marker label) do not. `waterline marker from waterlineDistance` above now means the line only, no label.
     - **Foam zones removed (`ad2ecf9`).** `bandsFromPerBreakZones()` and `bandsFromAggregateZones()` no longer emit `kind: 'foam'` bands — only impact bands render from either per-break or aggregate zone data (the D6 bullet above is corrected accordingly: impact only, not impact/foam). The foam legend chip (swatch + `foamZone` label) is removed from the legend row, which is now Water / Whitewater / Impact zone / Sand. The `foamZone` i18n key and `--beach-profile-foam`/`foam-ink` color tokens remain defined but are not currently reached by any rendered band.
   - **C2 — dominant-swell-only wave train (2026-08-08, Phase C of L1-BOUNDARY-REBUILD-PLAN, dashboard `selectDominantPartitionBreakPoints()` in `BeachProfileChart.tsx`):** the SYNTHESIZED water-surface rendering's anchor break (`outerBreak`) and secondary-crest bumps are now selected from the served `breakPoints` filtered to the DOMINANT partition only — grouped by `partitionInfo.partitionIndex`, the group with the largest face height (falling back to `hs`) wins, mirroring the backend's own dominance criterion (no second definition invented). Amplitude/wavelength/breaking envelope math of the drawn train is unchanged — only WHICH breakPoints feed it. Post-R5 every served `breakPoints` entry already shares one `partitionIndex`, so this filter is defense-in-depth (explicit selection-by-contract) rather than a change in what live data renders. **Plan-text deviation (Gate C):** the plan's stated fallback — "when no `breakPoints` are served, use the largest-face component from the swell list" — is not implemented: this component has no "swell list" prop, and when `breakPoints` is empty the pre-existing code already draws one smooth non-oscillating envelope from `transect.hs` (nothing to select a train among). Adding a swell-list prop would be a data-contract change beyond C2's display-only scope (coordinator ruling, 2026-08-08).
   - **R3.2 — FIXED x-domain, supersedes the 3-tier data-driven scale (2026-08-09, SURF-REMEDIATION-PLAN §R3, dashboard `BeachProfileChart.tsx`):** `selectTier()` (the outermost-break-distance-driven tier picker described by the "Negative X-axis ticks" bullet below, and its `tierShort`/`tierStandard`/`tierExtended` tables) is DELETED. The x-domain is now exactly `[+metadata.displayWindowM, −metadata.displayLandwardM]` (`BeachProfileMetadata`, D-R2/R3.1), converted via `METER_TO_UNIT`, on EVERY render — never re-derived from where breaks/transect/zones fall that hour. Metadata absent (pre-R3 cached response) falls back to a hardcoded Huntington preset (150 m seaward / 30 m landward). Tick-STEP selection (25/50/200 m ladder) is unchanged, now keyed by the fixed window size instead of the outermost break's distance — the "Negative X-axis ticks" bullet's `computeDistanceTicks()` behavior is otherwise unaffected. Content beyond the domain clips; content short of it does not stretch (the dynamic `DRY_BEACH_MARGIN` landward-extent computation is also gone — `xMin` is exactly `−displayLandwardM`, not waterline-relative).
   - **LABELS — every served break point gets its own crest label (2026-08-09 PM, operator: "the fact that you cannot see two breaks means you are blind"):** the WC-D3 dominant-only annotation filter (`dominantBreakPoints`, one label for the largest-face break only) is DELETED. Crest labels, the sr-only aria description, and label-collision staggering (`computeBreakLabelStagger`, unchanged) now run over the FULL served `breakPoints` list — distinct from C2's `selectDominantPartitionBreakPoints` above, which still selects only the dominant partition's breaks for the SYNTHESIZED WAVE-SURFACE train (a different, untouched concern). **`(ruled 2026-08-10; lands with Phase K of MARINE-PAGE-FIXIT-PLAN, ADR-106 R3, PA4)` — marker rendering is UNCHANGED by the K2/K3 break-marker-decoupling ruling.** K2 changes which points the server publishes as markers (decoupled from cessation, prominence-filtered — see API-MANUAL's `breakPoints` row) and K3 changes how the published zone bands are derived (fixed-width crash band, not roller-energy decay — see the impact-zone bullet in the D5/D6 description above, superseded by K3's own doc-sync when it ships). This component's own rendering contract — every served `breakPoints` entry gets its own crest label, every served zone band renders (skipping zero-width bands per the existing rule) — needs no code change: it already renders whatever the server serves, and K2/K3 change only what that is.
   - Transect selector: `<label>` + `<select>` above SVG (Best Peak / Average / numbered transects). Controlled by `selectedTransect` state in SurfingTab. API re-fetch requires `useBeachProfile` hook extension (deferred). **Since 2026-08-01 (BD-9, ADR-093 Amendment 7, SURF-ZONE-BREAK-DETECTION-SPEC-2026-08-01 Round 2, marine `9719db1`): the "Best Peak" selector's underlying API call (`GET /surf/{id}/profile?transect_index=best`) now returns the pipeline's `representativeTransectIndex` transect — the in-zone transect closest to the main-break-zone headline face height, not a single-transect max-face pick — falling back to the legacy max-face selection only for pre-round-2 cached forecasts. The rendered profile is always that ONE transect's ACTUAL cross-section (bathymetry, Hs envelope, break points, foam/reform zones as SwellTrack computed them for it; SurfBeat removed 2026-08-23) — never an averaged or synthesized curve; the "Best Peak" label reads as "the beach's representative wave" rather than literally "the single highest wave," a relabeling the dashboard code has not yet been updated to reflect (label text unchanged in this pass — data-source semantics only).** Structure-affected transects are selectable and rendered identically to open ones — `isStructureAffected` plays no role in which transect gets served or how its own break anatomy is computed (BD-8 rescinded); it remains available on each transect entry as map/UI metadata only, for a consumer that wants to badge or filter by it.
   - All new props optional — chart renders with existing data; zones/jacking/wave-shapes only appear when T5.2 API data is present.
   - **BD-9 header (D5.2, 2026-08-02):** `BeachProfileCardBody` renders a small text line above the chart ("Showing the representative transect (transect {{n}})") whenever the default/"best peak" selection is in effect (`selectedTransect` is `undefined` or `'best_peak'`) — hidden when the visitor has picked a specific numbered transect or "average". Self-contained: sourced from the already-fetched `profile.transectIndex`, no additional API field.
   - A11y: `role="img"`, `aria-labelledby` pointing to embedded `<title>`, sr-only data table with all transect values + break zones. Selector and wave shapes toggle keyboard-reachable.
   - i18n keys under `marine.surfing.beachProfile.*` (see locale files): `depthAxisLabel`, `distanceAxisLabel`, `impactZone`, `foamZone`, `jackingAnnotation`, `transectSelectorLabel`, `bestPeakLabel`, `averageLabel`, `showWaveShapes`, `breakType.{spilling,plunging,surging}`, `representativeTransectHeader`, sr-only table keys.
7. **Heat Map** — `Card footprint="full"` (T7.1, `HeatMapCard.tsx`; corrected 2026-08-02, D5.3 — a prior pass of this manual claimed no heatmap consumer existed; it does, and has since T7.1):
   - Quasi-2D birdseye view: custom inline SVG (not Recharts), X = cross-shore distance (shore right, offshore left, matches Beach Profile). **Y axis (C3 rebuild, 2026-08-09 PM, L1-BOUNDARY-REBUILD-PLAN §C3, P15/P16 — supersedes every "transect index"/"footprint model" description below where they conflict): REAL alongshore ground distance (`row.alongshoreM`), not transect index.** viewBox 820×N (PAD_LEFT=60, PAD_RIGHT=12, PAD_TOP=28, PAD_BOTTOM=52, or 74 when the BD-7/9 overlay legend row below is shown); row bands are VARIABLE height (`computeRowBands()` — each band spans the midpoint gap to its neighbours' real alongshore position, so dense transect stretches render visibly denser than sparse ones) up to ~40 visible rows before scroll. Falls back to the pre-C3 uniform `rowH`-per-index grid ONLY when any row is missing `alongshoreM` (older cached response, no P16 fields) — same fallback threshold gates the imagery transform below.
   - Data source: `GET /api/v1/surf/{id}/profile?transect_index=all`. Does not gate the tab's loading spinner — degrades independently (error / `modelStatus: "unavailable"` / ok), same three-state contract as Beach Profile.
   - Elements: colour cells (Hs gradient blue→teal→green→amber→red, per cross-shore segment per row), impact/foam zone fills per row, structure-affected hatching overlay (reduced-opacity diagonal pattern), breaker-type glyphs (spilling/plunging/surging) at each row's break point(s). **Double-break-aware**: `splitBreakPoints()` splits a row's multiple break points into an outer-bar band and an inner-bar band (any row with ≥2 break points, e.g. a genuine multi-bar beach) — always was, verified against real live double-break data at D5.1.
   - **D7s display smoothing (2026-08-05, dashboard `f85505b`, operator-requested):** colour-cell values pass through `smoothedHsAt()` — a centered median filter over a 5-transect window (2-row radius, clamped at the row-count edge and at each neighboring row's own point-array length) applied at RENDER time only, so a few transects zeroing out cannot punch dark holes in the surface. Raw API data is never mutated; the per-cell `<title>` tooltip shows the smoothed value (single value, no raw/smoothed pair); an unconditional plain-words note below the SVG discloses the smoothing (`marine.surfing.heatMap.smoothingNote`). Falls back to the pre-existing break-point proximity model only when the whole window has no data.
   - **BD-7/BD-9 overlays (D5.2, 2026-08-02, additive, null-safe):** a purple (`rgba(168, 85, 247, 0.75)`) gutter band in the Y-axis label margin spans the rows from `mainBreakZoneStartIndex` to `mainBreakZoneEndIndex` (the main break zone, BD-7); a small triangle marker plus a bolded row-index label mark the `representativeTransectIndex` row (BD-9). Both are range-validated against the response's own transect count (a stale/mismatched index from a different cached response renders nothing, not a misplaced band). Both data points come from the current primary `SurfForecast` entry (the `/surf` endpoint) — NOT from this card's own `/profile?transect_index=all` data — threaded in as props by `SurfingTab.tsx`. Absent/null on pre-Round-2 cached forecasts: no overlay, chart unchanged. Legend row (swatch + triangle, each paired with text — no colour-only signal) appears only when at least one overlay is shown, extending the SVG's bottom padding to fit it.
   - A11y: `role="img"`, `aria-labelledby` → `<title>` + `<desc>` (the `<desc>` gains one sentence per active overlay, e.g. "Main break zone spans transects 35 to 54." / "Transect 49 is the representative transect."), sr-only `<table>` with per-row transect index / open-or-structure-affected / break height / break distance / breaker type, plus a "Main break zone" Yes/No column (present only when the zone data is valid) and a "(representative)" suffix on the representative row's index cell.
   - i18n keys under `marine.surfing.heatMap.*` (see locale files): `transectIndex`, `transectAxisLabel` (Y-axis title, pre-C3 fallback only), `alongshoreAxisLabel` (C3 rebuild, 2026-08-09 PM — Y-axis title when ground data is available; en-only at landing, other locales fall back to the `t()` `defaultValue` per the codebase's established pattern), `openTransect`, `breakHeight`, `breakDistance`, `breakerType`, `breakerType.{spilling,plunging,surging}`, `mainBreakZoneLegend`, `representativeLegend`, `representativeSuffix`, `mainBreakZoneDesc`, `representativeDesc`, `modelUnavailable`, `loadError`, `orthoImageryDesc` (LM-2). **`shadowedTransect` is no longer read by this component (C3 removed its only consumer, the structure-overlay legend note below) — the key stays defined in every locale file, unreached, same "orphaned key" pattern as `foamZone` on the Beach Profile card above.**
   - **Background mosaic — product basemap, theme-selected (LM-2 orthophoto, 2026-08-03, superseded by M4 SURF-MAP-BASEMAP, PA9/Q5, MARINE-AND-MAPS-PLAN §M4, 2026-08-27; rotation/buffer/mosaic-geometry mechanism unchanged from C3 below — DISPLAY-ONLY):** when `SurfingTab` passes `spotLat`/`spotLon` (from the already-fetched `useSurfDetail` coordinates) and `useImageryConfig(lat, lon)` returns a config (API-MANUAL §12a; the API always answers 200 with `provider: "basemap"` — no failure/disabled state as of §M4), a tile mosaic renders behind the colour cells (SVG `<image>` elements in a `clipPath` group replacing the plain background rect) and the colour-cell opacity drops from `HEATMAP_CELL_OPACITY_DEFAULT` (0.85) to `HEATMAP_CELL_OPACITY_ON_ORTHO` (0.55). **The `<image href>` source is theme-selected, not a single provider (directive 15 — no Esri, no aerial photography, on any user-facing surface):** light theme fetches the OSM raster tiles directly (`substituteTileUrl(imageryConfig.light.tileUrl, …)`, same mechanism as every other light-theme map on the dashboard); dark theme reads a PNG data URL rasterized IN THE BROWSER from the local Protomaps basemap tier via `src/lib/basemap.ts` `rasterizeBasemapTile(z, x, y)` (mirrors protomaps-leaflet's own `renderTile` call sequence: `View.getDisplayTile` → offscreen-canvas `paint()` with `darkBasePaintRules()`, no labels — a per-tile-clipped labeler would clip text on a 50 m-buffered study rectangle) — resolved incrementally per tile by `src/hooks/useRasterizedTiles.ts`, with a tile omitted (not a placeholder) until its own rasterization settles. The mosaic GEOMETRY (tile placement, rotation, buffer, pivot — everything below this bullet) is identical between themes; only the tile image source differs. Attribution (light or dark, theme-selected) renders verbatim in the card's info-icon modal (H3 below), never translated. All tuning constants (`IMAGERY_MOSAIC_MAX_TILES_PER_SIDE=8`, `IMAGERY_ZOOM_MIN=14`/`MAX=19`, both opacities, `IMAGERY_VISIBLE_BUFFER_M=50`) sit in one documented block in `HeatMapCard.tsx`, unchanged by §M4.
   - **`(ruled 2026-08-10; lands with Phase H of MARINE-PAGE-FIXIT-PLAN)` — heat-map target state (fixit log Item 5).** The card's C3S sign-off was struck by the operator (meta repo commit `30f2699`) for oversized cells, blocky imagery, and an unresolved photo-to-data registration doubt. Target state, in order:
     - **H0 — registration known-answer check FIRST.** One ground anchor (the pier-base transect origin served by the profile endpoint) is projected through both the data-layer ground→chart transform and the imagery-tile ground→pixel math; the two must agree within ≤10 m ground-equivalent. This is a permanent regression KAT — a failure STOPs the phase, it is not silently reworked around.
     - **H1 — framing + tile budget.** Cross-shore frame seaward edge becomes `max(data extent, pier seaward tip's cross-shore position) + the existing 50 m buffer` (the frame now includes the whole pier for context, so it never clips at the pier tip). `IMAGERY_MOSAIC_MAX_TILES_PER_SIDE` becomes **8** (was 4) — the C3S single-ground-scale rule (below) is PRESERVED unchanged; widening the frame is what shrinks the cells back down (more ground framed → smaller strips at the same scale), and the higher tile budget is what lets the photo load at proper sharpness (target ≤1.5 m/pixel at default card size) instead of upscaling a coarse mosaic.
     - **H2 — smoothing is a render-time raster, not a value change.** The per-cell colour field renders through bilinear interpolation between adjacent transect row bands and along each transect (offscreen-canvas upsample of the value grid, then colormap) — replaces the `smoothedHsAt()` median-filter approach described above. Cells with no data stay unpainted (no invented surf beyond the data extent); the served data and the ground transform are untouched.
     - **H3 — nothing below the legends.** Both the imagery attribution line and the D7s smoothing note (both currently rendered below the SVG) move into the card's info-icon modal; the attribution string still renders VERBATIM, never translated — as of §M4 (SURF-MAP-BASEMAP, PA9/Q5) it reads `imageryConfig.light.attribution` or `imageryConfig.dark.attribution` (theme-selected, product-basemap ToS text), superseding the Esri/NAIP-era `imageryConfig.attribution` this bullet originally described — only the SOURCE text changed, the verbatim-rendering discipline did not.
     - **H4 — 4x2 + fullscreen overlay.** `footprint="full"` + `rowSpan={2}` (see the DESIGN-MANUAL Heat Map row for the sizing detail); chart area scrolls vertically inside the card when true-scale height exceeds the card; header gains a `ChartFullscreenButton` opening the existing `ChartFullscreenOverlay` component (the same one already used elsewhere on the dashboard) — operator-ruled: the overlay (not in-place grid growth) is fine.
   - **C3 REBUILD (2026-08-09 PM, L1-BOUNDARY-REBUILD-PLAN §C3, P15/P16, dashboard `HeatMapCard.tsx`) — ONE ground→chart transform, supersedes every bullet below where they conflict. Operator revocation basis: the prior "footprint model" (bounding circle around `spotLat`/`spotLon`, transect-index Y axis) was reviewed live and rejected — "unacceptable," not physically georeferenced.**
     - **Y axis is now real ground distance:** each row's real origin (`row.originLat`/`originLon`, P16) and cumulative `row.alongshoreM` (P16, sourced from the pipeline's existing `transect_origins`, no new computation) drive the Y-axis scale directly — row 0 (alongshoreM=0, the southernmost real origin) renders at the chart top, increasing alongshoreM renders further down. Row bands are variable-height (`computeRowBands()`), not a uniform per-index grid. Y-axis ticks (`yTicks`) use the SAME unit family and tick-step ladder as the X axis (25/50/200 m, keyed by the along-range span), title "Alongshore distance ({{unit}})" (`alongshoreAxisLabel` key) — the transect-INDEX Y-axis labels and "Transect" title are DELETED (kept only as the pre-C3 fallback, below).
     - **X axis unchanged in mechanism, now ground-correct by construction:** `row.transect[].distance` was always each row's own real cross-shore distance along its own real `transectBearingDeg` from its own real origin — `distToX()` is unchanged; P16 just makes the ORIGIN that distance is measured from an explicit served fact instead of an implicit approximation.
     - **Imagery bbox/rotation/buffer — ONE fitted ground transform (`fitGroundTransform()`):** a total-least-squares (PCA) fit of the real shoreline tangent through every row's served origin, plus a fitted offshore bearing (circular mean of every row's own `transectBearingDeg`, snapped to whichever tangent-perpendicular it agrees with) — replaces BOTH the old bounding-CIRCLE fetch footprint (`boundingBoxAroundPoint(spotLat, spotLon, radius)`, deleted) AND the old single-row `beachBearingDeg` rotation source. One raster, one rotation angle (`computeImageryRotationDeg(transform.offshoreBearingDeg)` — formula itself unchanged), derived from the SAME transform, never a single row's own bearing. The 50 m visible buffer is now a REAL ground distance on BOTH axes (`IMAGERY_VISIBLE_BUFFER_M` converted to pixels via each axis's own real px-per-meter scale — the old Y-axis "footprint-model fraction" formula is gone, both axes now agree with their drawn ticks by construction) and on the imagery fetch bbox (the buffered rectangle's 4 corners, projected to lat/lon via the fitted transform, enclosed in a north-up bbox).
     - **Fallback (no imagery at all, not a re-approximation):** fewer than 2 rows carrying real `originLat`/`originLon`/`alongshoreM` (older cached response, pre-P16) → the WHOLE Y axis falls back to the pre-C3 uniform index grid AND the imagery layer renders nothing (no circle approximation) — never a mix of ground-fitted and index-uniform on one chart.
     - **Structure-affected-area overlay REMOVED (unchanged from the prior C3 round):** the hatching pattern, the per-row reduced colour-cell opacity (was 0.35 when `isStructureAffected`), and the legend note are all deleted together — `isStructureAffected` keeps serving and still backs the sr-only table's "Open" column (data, not a visual overlay); the structure's physical effect stays inside SWAN L4's wave action.
     - **Pre-C3 fallback bullets (apply ONLY when ground data is unavailable):** Y-axis title reads "Transect" (`transectAxisLabel` key); Y-axis tick labels use the C3-fix (2026-08-09) density-aware every-Nth scheme (`labelStep = max(1, ceil(12 / rowH))`, always includes the last row) that replaced a prior `rowH >= 12` all-or-nothing gate.
   - **C3S SINGLE-SCALE FIX (2026-08-09 PM, L1-BOUNDARY-REBUILD-PLAN Phase C3S, operator-ordered — supersedes the C3 X-axis/Y-axis/buffer bullets above wherever they conflict).** Operator-caught defect: the deployed C3 chart still drew with TWO independent rulers — the X axis was tier-clipped and normalized/stretched to fill `CHART_W` regardless of the true cross-shore span, the Y axis was budgeted into a fixed `300/N`-derived pixel allotment independent of real metres, and the imagery mosaic was fitted to a THIRD, separately-derived px-per-metre. Result: the 567 m pier rendered shorter than ~200 m transect strips, ~900 ft south of its true row, with surf cells drawn over downtown.
     - **THE RULE:** every spatial element on this chart — data cells (both axes), the imagery mosaic, glyphs, buffers — goes through ONE ground scale `S` (px per metre): `S = CHART_W / frameSpanM`, where `frameSpanM` is the real cross-shore data extent (every row's raw `transect[].distance`, converted to metres, min→max — no tier clipping, no per-row subsetting) plus a 50 m ground buffer (`IMAGERY_VISIBLE_BUFFER_M`) on both sides. Any two points' pixel distance ÷ `S` equals their true ground distance, in every direction, for every pair.
     - **X axis:** the 3-tier scale (`selectHeatMapTier`/`ScaleTier`/100·300·1000 m thresholds) is DELETED. `distToX(dist, distanceUnit, frameMaxM, S)` maps a cross-shore distance straight through `S` — `x = PAD_LEFT + (frameMaxM − distM) × S`. No clipping: every real transect point renders at its true position (a row's data naturally stops short of the frame edge, leaving the 50 m buffer empty — e.g. the pier clips at the frame's offshore edge with only its inner ~176 m visible; this is correct at true scale, not a bug).
     - **Y axis:** `rowH = 300/N` budgeting is DELETED for the ground-data path. `alongMToY(alongM, alongMinM, S) = PAD_TOP + (alongM − alongMinM + 50) × S` — same `S` as X. `chartH` is now the FULL buffered span, `(alongMax − alongMin + 100) × S`, so the chart grows tall with real coastline extent (a live HB-pier response spans ~1,708 m → `chartH ≈ 3,980` SVG units) — the card already grows with content. `computeRowBands()` now takes a `yOf` mapping function (not a `[chartTop, chartBottom]` pixel budget) — each row's band spans the midpoint gap to its real neighbours; the first/last row's outer edge mirrors its own inner half-pitch (symmetric edge) rather than stretching into the buffer margin, matching the X axis's behavior (colour cells never reach the frame edge). Non-ground fallback (pre-P16 cached responses, no `alongshoreM`) is untouched — still the old uniform `rowH`-per-index grid, out of scope for this fix.
     - **Shared axis ticks:** `computeAxisTicks(minUnit, maxUnit, distanceUnit)` — one tick-step ladder (100 m→25 m, 300 m→50 m, else 200 m, scaled by `distanceUnit`) shared by both X and Y ticks now (previously the X-axis ladder lived only inside the deleted tier system; the Y axis already used this exact ladder independently).
     - **Imagery — same `S`, real tile-snapped anchor, own-center rotation:** the mosaic's fetch bbox is still the buffered ground rectangle projected via `fitGroundTransform()`/`groundPointAt()` (unchanged). What changed: slippy tiles snap to tile boundaries, so the fetched mosaic's TRUE ground bbox is generally LARGER than the requested bbox — `snapImageryBBoxToTiles()` (extracted from `computeImageryTiles`) returns that real snapped bbox, and its own corners are projected through `S` via `groundFrameCoords()` (the exact orthonormal-dot-product inverse of `groundPointAt()`) — never assumed equal to the chart's declared rect (a coordinator-caught defect in an earlier draft of this fix: declaring the mosaic screen rect equal to the core chart rect by fiat would have passed a "px/m == S" test built against the same false assumption, without fixing the real photo scale). The mosaic's on-screen size is `mosaicWidthM/HeightM × S` (true north-up ground size at scale `S`), rotated about its OWN true-scale center (`pivotX`/`pivotY`, from the snapped bbox's real center point) — mathematically equal to directly projecting the mosaic's real corners through the ground frame. The visible clip is now exactly the CORE chart rectangle (`[PAD_LEFT, PAD_TOP, CHART_W, chartH]`) — the 50 m buffer is baked into the frame/`S` already, no longer a separate on-screen extension beyond the chart.
     - **Spot marker stays fetch-only:** `spotLat`/`spotLon` (the forecast data pin) are used ONLY to call `GET /api/v1/imagery/config` — banned from any spatial anchoring of the imagery layer or the grid (was already true before this round; explicitly re-confirmed and documented here per the operator's rule — a forecast pin is not a landmark).
8. **72-Hour Surf Forecast** — `Card footprint="full"`:
