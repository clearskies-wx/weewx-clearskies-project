...
> **`(ruled 2026-08-13; lands with Plan Amendment A1 of MARINE-PAGE-FIXIT-PLAN)` — `l1NestAge` health key and hourly refuse gate (D5, ADR-108).** Target state, lands at A1.4. New `/health` field **`l1NestAge`** reports the age, in hours, of the archived L1 nest (`level1/nest_out_<cycle>.dat`) the hourly (fast) cycle is currently feeding to L2 — the hourly cycle no longer runs L1 itself (D5). Refuse gate: **`L1_NEST_MAX_AGE_H = 9`** — when the archived nest's age exceeds this, the hourly cycle refuses to publish (no-publish, loud, named reason) rather than serve off a nest one missed full-run cycle stale plus margin, per the standing no-publish posture (rules/coding.md §1). **Force-full-run interplay:** a forced full run — the existing geometry-changing config-push trigger (`force_full_run_signal`, ARCHITECTURE.md's SWAN nearshore model section) — always rebuilds L1 and therefore resets `l1NestAge` to zero on success; the nest age only grows when a full run fails or is delayed past its scheduled extended-HRRR-cycle slot. This is additive to the existing `status`/`reasons`/`fullRun` fields above — no existing health key is renamed or removed.
