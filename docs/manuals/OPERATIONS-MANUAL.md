...
#### WW3 deep-water leg (target — Phase W; ADR-109)

**Not yet built — target state.** This subsection documents the WW3 deep-water leg's build/install/scheduling/monitoring story as fixed by ADR-109; the implementation lands in Phase W. Everything above this subsection (the marine service's own install path, `/health` fields, and supported environments) is unchanged and unaffected by this leg — the WW3 leg is an ADDITIONAL process on the same marine-service host, not a replacement for anything described above.
...
**Shadow-mode gate (config, not scheduling).** The per-site `ww3_shadow_mode_enabled` boolean (ADR-109 D12/PW5 — see API-MANUAL.md §19.7a for the config-push mechanics) gates whether a given site's WW3 leg runs at all; it does not change the cadence, thread budget, or refuse-gate values documented above, which are universal once the leg is enabled for a site.

**Vchain full-chain shadow run (as-built, Phase V wiring round, operator order 2026-08-18).** When the additional per-site `vchain_enabled` boolean (default false; same config-push scope, next to `ww3_shadow_mode_enabled`) is on, every SUCCESSFUL WW3-leg cycle is followed — serial on the marine runner thread, after the production publish — by the COMPLETE new chain: WW3 → SWAN L2 via `BOUNDNEST3` consuming the cycle's `ww3_outp` transfer file → nested SWAN levels → 1D handoff → SwellTrack with SurfBeat as its nearshore-stage companion, executed by the SAME production full-run machinery (`run_full_swan_cycle_from_store(..., vchain=...)`) into an isolated shadow root `SWAN_WORK_ROOT/vchain/work/` (SWAN L1 is skipped — WW3 is the chain's deep-water leg; `save_hotstart=False`; SWAN subprocesses nice'd with the `[ww3]` section's `nice_priority`). The chain feeds NOTHING that serves: no last-good cache, no run marker, no disk forecast cache, and all nested health-state writes are diverted by a thread-local shadow scope (`services/shadow_state.py`). Per cycle attempt (success OR refusal) ONE JSON ledger row lands under `SWAN_WORK_ROOT/vchain/ledger/row_<cycle>.json` (schema 1): cycle time, WW3 march wall-clock, WW3 Hs/Tp/dir at the nearest transfer points to the `[ww3] vchain_buoys` stations (default NDBC 46253/46222, with honest point-to-buoy distances), chain wall-clock, per-spot served-first-hours + DWR-dominant summaries + SurfBeat companion result, and matched-time NDBC realtime2 observations (obs `null` + named reason on fetch failure — the row is never skipped). Health surfaces via the read-only `/health` `vchain` block (`lastCycleTime`/`status`/`refuseReason`/`chainWallClockS`/`ledgerPath`; never feeds `status`/`reasons`). Named refuse slugs: `vchain_bad_cycle_time`, `vchain_skipped_budget` (the chain would overlap the next cycle's window — estimate is the last recorded chain wall-clock, or 45 min before first measurement, ×1.25 margin), `vchain_transfer_missing`, `vchain_scaffold_missing`, `vchain_chain_failed`, `vchain_chain_refused`, `vchain_no_payloads`, `vchain_ledger_write_failed`, and the escape catch-all `vchain_unexpected_error` (service.py wrapper). Compute budget: the chain roughly doubles SWAN compute per full-run cycle when enabled — the recorded wall-clocks in the ledger and health block are the data that answer the budget question. Ledger retention (Gate VCHAIN F3): rows are age-pruned at the start of each vchain cycle by their filename cycle token, retention `[ww3] vchain_ledger_retention_days` (default **45 days**, matching NDBC's realtime2 window so campaign rows outlive their comparison sources); pruning logs a named INFO line and is never fatal to the cycle.
