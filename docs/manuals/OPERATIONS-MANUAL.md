...
`forecast_cache.json` (`/var/lib/weewx-clearskies/swan/forecast_cache.json`; historical: `/var/run/weewx-clearskies/swan/forecast_cache.json`, RAM-backed tmpfs, corrected 2026-08-08 Phase R4) persists each spot's forecast payload — `forecast`, `spectral`, `spectral_dwr`, `transect`, `swelltrack`, and related keys — across marine-service restarts, so `fetch()` can serve immediately without waiting for the next full SWAN run. A full run replaces the file wholesale; the hourly quick update merges only the spots it touched, leaving others untouched. The private version-1 band-station codec stores invariant per-transect station index/depth geometry once and hourly component fields compactly; the loader validates and reconstructs the pre-codec `band_stations` dictionaries before the restart-safe 1-D path or any serving consumer receives them. The document is gzip-framed after compaction, so the 64 MB warning measures actual durable bytes. Legacy uncompressed JSON remains readable; malformed compact or compressed payloads are ineligible and never served.
