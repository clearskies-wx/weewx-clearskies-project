...
**Basemap (CS-BASEMAP, plan `MARINE-AND-MAPS-PLAN-2026-08-27.md` §M1, ADR-078 Amendment 2 —
Proposed):** the design calls for one "Basemap" admin section beside Geographic Features, showing
per-tier status (world/local/radar: available, size, updated-at, bounds) and one "update basemap"
action (`POST /setup/basemap/update`, already live in the API — 202 started / 409 already-running)
that extracts all three tiers in one background pass. **Not yet built as of this entry** — the
API's basemap endpoints (`GET /api/v1/basemap/{tier}/tiles`, `GET /api/v1/basemap/status`,
`POST /setup/basemap/update`) are live, but the stack repo's admin page/route for it has not
shipped (the M1-API round touched only the API repo). Once built, the Geographic Features section
is retired in the same commit that accepts ADR-078 Amendment 2 (plan journal J3) — until then both
sections coexist, same as their underlying API features.
...
- `degraded` — the last cycle completed and every required input was available, but an input was stale past its own monitoring threshold, a B2 runtime invariant fired at or after `last_run`, OR (Z3.9b, V6/V7) the full-run cycle is overdue AND has failed 3+ consecutive attempts (`fullRun` note below).
...
**Post-deploy sweep addition (Q16 Round A, 2026-08-25).** In addition to the standard post-deploy journal sweep (checking for new ERROR/WARNING classes since restart), a Q16 Round A deploy's sweep MUST also check: (1) `fullRun.l2BoundaryExhausted` on `/health` reads **FALSE** every cycle observed — TRUE means SWAN L2's PRINT still shows "data on boundary file exhausted" and the merged-transfer fix did not take; (2) journal-grep `l2_boundary_exhausted` for the detector's own WARNING line, which should NOT appear post-deploy (its presence names the cycle where the PRINT scan found the exhausted-boundary warning); (3) `ww3Horizon.lastSuccessCycleTime`/`coverageEndTime` advance after the first 00Z cycle post-deploy, confirming the daily continuation march ran and covered the expected horizon. See PROVIDER-MANUAL.md §14.18 and the `ww3Horizon`/`fullRun.l2BoundaryExhausted` key descriptions below.
...
**`fullRun`** (Z3.9b, V6/V7/V8, MARINE-PAGE-FIXIT-PLAN-2026-08-10 audit) — the full SWAN cycle's retry/success surface: `{"lastSuccessCycle": str | null, "lastSuccessAt": iso | null, "pendingCycle": str | null, "consecutiveFailures": int, "lastFailureReason": str | null, "lastFailureAt": iso | null, "overdue": bool}`. `pendingCycle` names the cycle currently retrying-on-failure (distinct from `lastSuccessCycle`, the most recently completed one). `overdue` is `true` when no success has landed within 8h. A run that fails 3+ consecutive attempts backs off from retrying every `check_interval_s` tick to a 30-minute retry cadence (server-side only — this field is how an operator observes that it's happening); see the `degraded` reason above for when this surfaces in `status`.
...
> **`(landed 2026-08-14, Plan Amendment A1; as-built)` — `l1NestAge` health key and hourly refuse gate (D5, ADR-108).** `/health` field **`l1NestAge`** reports the age, in hours, of the archived L1 nest (`level1/nest_out_<cycle>.dat`) the hourly (fast) cycle is currently feeding to L2 — the hourly cycle no longer runs L1 itself (D5). Live and verified: first archive `nest_out_20260814.000000.dat` (~223.7 MB, saved 04:57:43Z); `l1NestAge` measured 5.58 h (non-null) at the first post-deploy reading, independently corroborated by an adversarial audit dispatch reading `/health` directly. Refuse gate: **`L1_NEST_MAX_AGE_H = 9`** — when the archived nest's age exceeds this, the hourly cycle refuses to publish (no-publish, loud, named reason) rather than serve off a nest one missed full-run cycle stale plus margin, per the standing no-publish posture (rules/coding.md §1); KATs cover the exact-9.0h boundary (frozen-clock fix, marine `7bffaf7`). **Force-full-run interplay:** a forced full run — the existing geometry-changing config-push trigger (`force_full_run_signal`, ARCHITECTURE.md's SWAN nearshore model section) — always rebuilds L1 and therefore resets `l1NestAge` to zero on success; the nest age only grows when a full run fails or is delayed past its scheduled extended-HRRR-cycle slot. This is additive to the existing `status`/`reasons`/`fullRun` fields above — no existing health key is renamed or removed.
...
This subsection documents the WW3 deep-water leg's build/install/scheduling/monitoring story (ADR-109) and the production chain that consumes it for chain-enabled locations (CHAIN-SERVES round). Everything above this subsection (the marine service's own install path, `/health` fields, and supported environments) is unchanged and unaffected — the WW3 leg is an ADDITIONAL process on the same marine-service host, not a replacement for anything described above; for a legacy (non-chain) location the live SWAN-L1 path keeps serving exactly as before this round.
...
**Scheduling.** The WW3 leg runs on the SAME full-run cadence as the live SWAN-L1 path (6-hourly FULL cycles) — it does NOT run on the hourly fast-cycle cadence, matching the restart-chaining design (a fresh restart every full-run cycle, chained forward). Runs SERIAL with the SWAN cycle (never concurrent) to avoid resource contention, at `OMP_NUM_THREADS ≤ 4`, `nice -n 15` — the same E1/E2 contention protocol the SWAN benchmark baseline used (ADR-109 D12). Per-cycle wall-clock is in the tens-of-minutes class at production shape (measured: 4706.82 s ≈ 78.4 min for G1×P1 at F4b's 24 h production-shaped march; corroborated by the buoy-validation round's real-data marches at 4183.58–4509.74 s, ADR-109 D12) — well inside the plan's contention-budget wall-clock ceiling.

**00Z daily long march (Q16 Round A, 2026-08-25; ADR-109 amendment note).** In addition to the four 6-hourly legs above, the **00Z** cycle ALSO runs a continuation march once daily, strictly AFTER that cycle's own 6 h leg and production publish complete: `cycle+6h → cycle+96h` on the same G1 grid/binary/physics, starting from a COPY of the leg's own +6 h restart (the leg's restart chain above is untouched). Wall-clock ≈4 h at the same contention budget as the leg (never concurrent with a production full run); ceiling 6 h. This closes the frozen-forecast defect (SWAN's own "data on boundary file exhausted," the +6 h ocean held constant for forecast hours 7–72 every cycle since 2026-08-19) by giving SWAN L2 a MERGED boundary transfer each full cycle — hours 0–6 from that cycle's own leg, hours 7–72 from the newest 00Z march — rather than the 7-record leg file alone. Failure-isolated: a horizon-march failure never delays, blocks, or alters a production publish. See PROVIDER-MANUAL.md §14.18 for the merge/cycle-pin mechanics and ADR-109's amendment note for the full design.
...
**Monitoring keys — Q16 Round A additions (2026-08-25; ADR-109 amendment note).** New top-level `/health` block **`ww3Horizon`**: `{"lastSuccessCycleTime": str | null, "coverageEndTime": str | null, "wallClockS": float | null, "refuseReason": str | null}` — the daily 00Z continuation march's own status, independent of the leg's own health fields above (a horizon-march refusal never affects `status`/`reasons` or blocks a publish). New field **`fullRun.l2BoundaryExhausted`** (boolean): a detector scans SWAN L2's PRINT output every run for the "data on boundary file exhausted" warning and surfaces it here — **FALSE is the healthy/expected state every cycle; TRUE is a regression signal**, not a normal operating condition (this is the acceptance expectation stated for the round). Named refuse reason at the horizon march: `ww3_horizon_cycle_unpinned` (the march's NOAA-cycle pin, `level0/boundary_cycle_<token>.txt`, was unavailable — the march makes exactly one pinned fetch attempt, no fallback ladder). See PROVIDER-MANUAL.md §14.18 for the merge/cycle-pin mechanics.
...
**Disk/retention — Q16 Round A additions (2026-08-25).** `level0/horizon_<token>/` (the daily 00Z continuation march's own output, retention: newest **2**), `level0/hstage_<token>/` (merged-transfer staging), `level0/boundary_cycle_<token>.txt` (the NOAA cycle pin the horizon march re-fetches against). Disk: horizon transfer output runs to hundreds of MB/day, retention-bounded (394 GB free measured at implementation time). See PROVIDER-MANUAL.md §14.18.
