...
| 2026-09-05 | R4 / R8b fast live refusal | Natural 20Z fast-cycle trigger at 21:58:16 UTC accepted a complete 12-record wind window and 21 STOFS grids, then refused the 33.997-hour archived L2 boundary against the 9-hour ceiling before any SWAN fill. | Guard returned idle after the refusal. `/health` reports `run_in_progress=false`, fast attempt `2026-09-05T20:00:00Z` failed with `run_refused`, `boundaryMerge=failed/boundary_merge_failed`, and last-good remains explicitly valid. This is fresh failure-path evidence only; it does not close the required successful full/fast/current/publish gates. |
...
| 2026-09-06 | R4 / R8b live full-chain QC | Natural 18Z full cycle completed without intervention: WW3 leg `ok`; horizon completed after 13,991.8 seconds to 2026-09-09T18:00Z; STOFS supplied 73 hourly fields; SWAN L2, L3, and L4 each completed; SwellTrack completed 162 transects (139 open); vchain ledger recorded the cycle complete; full-run state marks `lastSuccessCycle=2026-09-05T18:00:00Z`, with cache and last-good publication `ok`. | This is the first fresh, natural successful WW3→SWAN L2–L4→SwellTrack→publication evidence on the deployed corrective candidate. It proves the successful chain and truthful stage observations for this cycle, but does not close the R4/R8b matrix while the new oversized forecast-cache finding below is unresolved. |
| 2026-09-06 | Forecast-cache boundedness | The successful 18Z run persisted `/var/lib/weewx-clearskies/swan/forecast_cache.json` at 148.5 MB (155,688,741 bytes), above its 64 MB M-0 warning threshold. Read-only payload inventory isolates 130.9 MB to cached `spectral`: 73 time entries, each carrying 162 transects' retained `band_stations`. The documented 2-D grids are already trimmed; `band_stations` is consumed by the restart-safe 1D pipeline and is restored from this cache for the serving path. | The growth is a real M-0 bounded-payload defect, but deleting or reshaping `band_stations` would change the persisted restart contract. Do not make that architectural change without operator approval; continue the other plan work and preserve the successful full-chain evidence. |
| 2026-09-06 | R10A daily-horizon trigger | A recovery-driven 18Z horizon already covered through 2026-09-09T18:00Z, but the fixed 00Z branch launched another long march solely because it was the designated hour. At operator direction, marine `f976fcf` now suppresses the designated run when a less-than-24-hour-old horizon already covers the cycle; normal next-day daily behavior and genuinely short coverage remain due. | Controlled local regression: 16 focused horizon tests pass. After guarded deployment/restart, an exact restored-state host check returned `due: false` for 00Z with the prior 18Z horizon named as satisfying the daily march. |
| 2026-09-06 | R4 / R8b fresh L4 current | The natural successful 18Z full cycle wrote L4 `CURRENT.txt` at 03:02 UTC. Its 51 x-interval by 169 y-interval deck requires 52 values per row and `73 × 2 × (169 + 1) = 24,820` rows; read-only measurement found exactly 24,820 rows with width set `{52}`. | This closes the previously missing fresh L4 current-file shape evidence for this full cycle. The broader R4/R8b matrix remains open pending the separate cache-boundedness repair and its controlled live verification. |
