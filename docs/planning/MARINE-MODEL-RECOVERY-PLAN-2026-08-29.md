...
| 2026-09-06 | Forecast-cache boundedness | The successful 18Z run persisted `/var/lib/weewx-clearskies/swan/forecast_cache.json` at 148.5 MB (155,688,741 bytes), above its 64 MB M-0 warning threshold. Read-only payload inventory isolates 130.9 MB to cached `spectral`; its documented 2-D grids are trimmed, but the retained per-transect `band_stations` restart candidates account for the growth. | Treat as an actionable M-0 bounded-payload defect. Preserve the existing restart-candidate contract while tracing every persisted/cache-recovery reader; do not drop or reshape a field until the smallest contract-preserving correction and its verification are established. |
