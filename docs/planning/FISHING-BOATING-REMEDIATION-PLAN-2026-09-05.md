...
4. Put one native Fish dropdown in the Fishing Forecast card header, sourced from the configured species list. Default to the first configured species and retain the visitor's selection while they move through the forecast. Do not render a species strip, selection-button wall, or other multi-choice control. The selection controls all score, suitability, seasonal, and explanation content below it.
5. Rebuild Fishing’s three-day forecast using the exact 72-Hour Surf Forecast grid pattern: a sticky left row-label column, grouped day headings, fixed-width aligned time columns, and horizontal scrolling. It presents the **selected species'** score/status, tide/current state, depth-appropriate water temperature, pressure trend, wind, swell height/period, and major/minor solunar state within the corresponding period columns. Selecting a time column expands that selected species' explanation below the shared grid. Do not render individual Fishing period cards, a generic Fishing score, or a standalone score table.
...
7. Fold Species Forecast into the Fishing forecast grid. The selected-species score, why it is active/less active/inactive, hard-stop reason when present, and detailed accessible data table all live within that one card on demand. Do not render a separate competing Species Forecast card or a generic-score card.
