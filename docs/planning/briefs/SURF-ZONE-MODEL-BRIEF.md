...
|---|---|---|
| `enrichment/wave_transform.py` | **Superseded 2026-07-25 (T4A.7).** Retains only `bilinear_interpolate()`, used by `endpoints/surf.py` to interpolate HRRR wind grid components — unrelated to wave height. | **Does not feed the 1D model.** This row described a design that was never implemented: the module's supplements never actually ran ahead of the 1D model's input. T4A.7's verification gate confirmed the 1D model's boundary condition comes directly from the SWAN handoff SPECOUT, emitted via `POINTS` at the exact requested coordinate — SWAN itself bilinearly interpolates to that point, so there was no grid-cell value left for a supplement to refine. Both of the module's surviving supplements (breaker-gamma correction, sub-grid interpolation) are deleted; the other two were already removed by ADR-095 and ADR-093 Amendment 2. |
