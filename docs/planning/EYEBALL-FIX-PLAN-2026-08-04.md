...
| **Round W** — real post-breaking physics (operator-ordered: "real physics and not fake clamping") | **IN PROGRESS 2026-08-05** | Dally-Dean-Dalrymple breaker decay (Γ=0.40, K=0.15, primary-source verified from the original paper); both clamps removed (kernel :1029 + pipeline combine :649); breaking becomes a modeled state (onset γ·d → decay toward 0.40·d → cessation/reform → re-break); detection from real onset events; implementer mid-build (W1-W3, brief ROUND-W-WAVE-REFORM-BRIEF-2026-08-05.md). THEN: guards/KATs (closed-form shelf solution) → blind audit → deploy → reality gate vs webcam-at-known-tide with operator's orthophoto anchors (~300 ft outer / ~100 ft inner off the waterline, approximations sufficient to judge the model) |
