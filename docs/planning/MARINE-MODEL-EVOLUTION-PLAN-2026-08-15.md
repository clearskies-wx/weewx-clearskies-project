...
| W1–W6 | Build the WW3 leg into the product (runner, boundary, handoff, sizing, health, tests) | 🔄 **W1 ✅ DONE 2026-08-17** (marine 9f51a20 + gate-remediation eb49046, local-only): `services/ww3_runner.py` + 18 tests per W DESIGN v1 W1-DESIGN; adversarial Gate W1 **PASS** (mutation drills killed both neutered refusal paths; 1 medium finding — untested tree-kill mechanism + overclaiming docstring — remediated same day with the suite's one real-subprocess test + honest-limits docstring; hung-binary timeout→kill sequence deferred to W-Accept live rows). Four agent-surfaced fill-values ruled and upheld (slug naming, config-level timeouts, nice config fields, ww3_grid under the 10-min "others" ceiling). **W2 ✅ DONE 2026-08-17** (marine 04f5c59): `ww3_formats.py` transfer-format writer + ww3_bound deck builder + additive-only output path in boundary_reconstruction (frozen constants diff-verified untouched); falsifiable transposed-control KAT (transposed emission detected at 19.21% error vs 0.0002% correct — the documented F4b defect-class magnitude); **REAL ww3_bound ingest KAT on librewxr (lead-run): rc=0, nest.ww3 73,170 B, all 313 G1 boundary points weighted, 2 records, zero read errors**; adversarial Gate W2 PASS (auditor mutation-flipped the production loop — KAT caught it; FACTOR divergence vs reference generator ruled out as absent-upstream-quantization; 1 cosmetic low). Mid-round design correction: ww3_bound interpolation = method 2 LINEAR (lead transcription error, agent STOP, verified 3-source, plan corrected 49f11e8e). **W3 ✅ DONE 2026-08-17** (marine 4a99882): BOUNDNEST3 selector in swan_formats + run_3level call site, live path BYTE-IDENTICAL (mutation drill: 1-char perturbation kills 3 tests), shadow branch inert by default (both production call sites enumerated, neither activates), corner-order misprint trap verified against the SWAN manual's own CGRID convention; Gate W3 PASS all 6 rows, zero findings; FREE-selection gap closed by F4.3 citation. **Pre-existing debt CONFIRMED + TRACKED → W6 input:** `tests/test_swan_runner_l4_nesting.py` fails 8/8 (`NoneType.by_side`, swan_runner:5224) at 4a99882 AND at pre-W2 eb49046 (lead bisect + auditor worktree reproduction, independently) — predates this session; W6's stale-test sweep triages it. **OFS deploy reality gate CLOSED — PASS** (scratch/OFS-DEPLOY-BASELINE.md): publish-liveness ✓, journal classes all pre-existing ✓, S3 rung exercised live during a real THREDDS 504 flap ✓; served-vs-buoy rows: periods/directions PASS both trains, combined Hs 36% low vs buoy total — OUTSIDE ±30% but NOT deploy-attributable (served swell fields written 58 min pre-deploy by the 18Z SWAN run; OFS touches ocean-data only) — the standing deep-water deficit this plan exists to fix, logged as a Phase V datapoint. **W4 ✅ DONE 2026-08-17** (marine f56d376 + gate-remediation 46444b1): `derive_ww3_setup()` + deck builder + `detect_depth_steps()` (node89 formula per research brief §5, detection-only) in swan_domain, additive `ww3_leg` persistence block in the sizing chain (failure-isolated, ERROR-loud, key-absent-detectable); dims formula = F2's own convention (111.32 km/deg, REFERENCE-gen_grids.py — the live SWAN helper's 111.0 would have produced an unvalidated 143×170; caught by the mandatory 143×171 KAT + agent STOP before code); Gaps G1/G2/G8 closed. **Gate W4 initially FAIL (adversarial working as designed):** F1 HIGH — the live-path KAT was a synthetic proxy, mutation-proven blind to real-call-site corruption; F2 — fixture docstring arithmetic wrong (53/57/109 vs actual 11/12/22, and the closeout had quoted the wrong numbers); F3 — thin status-map provenance. All remediated (real-chain KATs driving run_grid_sizing_chain end-to-end reading the persisted json; auditor re-ran its OWN mutation drill — new KAT catches it) → **PASS**. Two pre-existing-debt sets tracked → W6: L4-nesting 8/8 + the 8 adjacent-suite env failures (eccodes/pygrib absent on dev machine; set-identity proven at parent+HEAD twice). LESSON for gate definitions: always mutation-drill the REAL integration point, never accept a KAT that re-simulates the seam locally. **W5 ✅ DONE 2026-08-17** (marine a967265 + 3bf2935 + gate-remediation 0f2ac40, plus the W1 reopen 2959fd8): shadow-leg cycle integration (strictly after production full-run publish, serial), per-LOCATION `ww3_shadow_mode_enabled` (default false) + install-level `[ww3]` runner-binding section, trap-23 exact-stamp restart chaining + frozen-clock 9 h gate (mutation-verified at the exact boundary), refuse-loudly named states incl. the W4 carry (`ww3_derivation_missing`) and `ww3_mod_def_missing`, wind production path (Gap G7), read-only isolated `ww3` health block, `level0/` layout + retention. **Mid-round cross-task defect surfaced by W5's STOP:** W1's `run_leg_cycle` could not execute its own WD10 chain (callers had to name not-yet-existing staging paths) — W1 REOPENED and fixed (2959fd8: cumulative harvested-artifact seeding into later step CWDs, spec-wins-on-collision, trap-13 copies; dated W1-DESIGN clarification in this plan) — then W5's wiring completed (3bf2935: real runner invocation, no phantom chained paths, runner slugs verbatim into leg state). **Gate W5 PASS 8/8** (mutation drills: flag-off inertness, 9 h >-vs->= boundary; cold-start-substitution path proven unreachable); 1 low finding (shared refusal slug across 4 failure modes) remediated same day (0f2ac40, per-cause slugs). **W6 ✅ DONE 2026-08-17** (marine 6c79266..91eecf8 + row-8 remediation 300f190): full-chain seam test (real derivation→emitter→spec-construction→BOUNDNEST3, runner boundary only mocked; seam-mutation-proven); ALL 15 tracked pre-existing failures triaged with lead rulings + checkout-verified counts (9 env → probe-keyed skips that un-skip on librewxr; 6 stale harnesses repaired with original assertions preserved, root causes all Z3.9a/Z3.9b Fixit-era design additions); TWO vacuous sibling tests found + bite-proven fixed (incl. a MemoryCache reference-aliasing tautology — comparison against a live alias always true; deepcopy-snapshot pattern applied; third instance flagged for future); B2 sweep for L1-only-deep-water pins CLEAN. **W1–W6 ALL DONE — PHASE W CODING ROUNDS COMPLETE.** Preceding same-day repo work: OFS audit-fix round (Q8) deployed at a9ac72f — fix's S3 code path exercised LIVE during a real THREDDS 504 flap at 22:56Z, 2 min post-restart |
| VCHAIN | Full-chain shadow wiring (operator order 2026-08-18 "PUT THE SHIT TOGETHER") | ✅ CODE DONE + DEPLOYED 2026-08-18 (recorded retroactively 2026-08-19 — the session ended before the plan was updated; process miss, coordinator's). Marine `fa1ce2d..0bb6b32` + gate remediation `2bd1438`: every successful WW3-leg cycle followed serially by the complete chain (WW3→SWAN L2 via BOUNDNEST3→nested levels→1D handoff→SwellTrack+SurfBeat companion) into a shadow root + per-cycle buoy ledger + `vchain` health block, per-site `vchain_enabled`. Adversarial gate (scratch/GATE-VCHAIN-DEFINITION.md) PASS 6/6 drills, 3 findings remediated. Doc-sync meta `c1b4e6e1` + `4c7b2829`. **Marine repo PUSHED + deployed to librewxr at `2bd1438`** (deploy pulls from GitHub); both flags enabled for huntington-city-beach-pier. **NEVER RAN LIVE:** first armed cycle refused `ww3_derivation_missing` (fixed live: config re-push regenerated sizing with the `ww3_leg` block, `derivationPresent: true`); the 12Z restart continuation march was never run (session died 09:30Z) → every cycle since refused `ww3_restart_missing`; the session's watch polled a wrong file path (monitor-rule violation — silence looked like health for ~19 h). **SUPERSEDED by CHAIN-SERVES before ever producing a ledger row.** |
| CHAIN-SERVES | The new chain BECOMES the production pipeline (operator order 2026-08-19, in chat: "NO NOT IN SHADOW, JUST MAKE IT SO IT IS THE CHAIN ... THE ENTIRE INSTALL RIGHT NOW IS FOR TESTING") | ✅ CODE + GATE DONE 2026-08-19; **DEPLOYED 2026-08-19 (operator word "push" in chat):** pushed marine `2bd1438..ffce137` + meta; config migrated on librewxr (both shadow flags removed, `ww3_chain_enabled: true` for huntington; backup `.bak-prechainserves-20260819T063241Z`, ownership preserved); deploy waited out an in-flight run, then `deploy-marine.sh` — running commit `ffce137`, process started 06:37:08Z, post-deploy journal sweep ZERO new error/warning classes, health ok, `l1NestAge: None` (D4 gating live). All-terminal-states monitor armed for the first chain cycle (~10:30Z trigger for cycle 06Z; deadline-alarm 13:00Z so silence is impossible). Reality gate pending first cycle: Hs ±30% / dominant Tp ±2 s / dir ±30° vs NDBC 46253/46222, stated before looking. **FIRST CHAIN CYCLE SUCCEEDED 2026-08-19 10:34Z (cycle 06Z)** after three live deck defects found+fixed+deployed same morning (each loud, named, ledger-rowed, nothing wrong published; each fix mirrored from the proven same-day mint decks, guard tests proven failing pre-fix, same-commit test updates): (1) `ab74734` prep spec never staged mod_def.ww3 (instant rc=50); (2) `09441a8` shel deck grammar (one-line-of-ten-flags vs seven forcing lines; coord-pair STOPSTRING; Type-3 track line; Type-6 partition sub-line — "Bad repeat count" at deck read); (3) `af26a78` outp deck grammar (time line, `-1` all-points, OTYPE line; program names its own output `ww3.<yymmddhh>.spc`, promoted to canonical `ww3_l2_transfer.ww3`). Cycle evidence: WW3 march 1002 s rc=0 (restart 212 MB, transfer 31.5 MB), chain 1790 s, production publish (fullRun lastSuccessCycle 06Z), ledger row_20260819.060000.json status ok. **REALITY GATE — OUT OF TOLERANCE, disclosed, NO rollback (operator's standing order is to run the chain live for testing; gate numbers are the datapoint):** vs 46222 obs 05:56Z (Hs 0.9 m, DPD 7.0 s, MWD 261°): model at nearest point (23.8 km away) Hs 0.465 m (**48% low**), Tp 14.6 s (energy in swell band vs buoy wind-sea band), dir 219° (**42° off**). Caveats: single matched hour; poor collocation (23.8 km); 46253 obs=null (rate-limiter, below); conditions wind-sea-dominated = exactly the ADR-109 2026-08-18 caveat regime (wind-sea fidelity unvalidated — F4 wind forcing was longitude-constant). **OPEN DEFECTS from the first row:** (a) ledger obs fetch for 46253 starved by the shared NDBC 1-call/s rate limiter (pre-existing class, now materially harming the scorecard — needs retry/backoff); (b) SurfBeat companion in the ledger errored (`HSIGN column not found in TABLE header ... columns found: ['%']` under `vchain/work/surfbeat_*`) — and its residence in a vchain work dir needs review against the D5 read-from-production design. Cycles self-chain from here (12Z next); hourly fills unblock automatically now that the archived transfer exists. Build: marine `a943ce8..d5fcaf0` (D1 flag merge, D2 leg-first cycle, D3 ChainSpec production seam, D4 l1NestAge gating, D8 hourly-fill substitution) + meta doc-sync `b7d3ef70`. Operator mid-round rulings applied: pure L1→WW3 substitution, hourly feed mechanism unchanged (D8 amended), dual cadence = transitional until Phase L. Adversarial gate (scratch/GATE-CHAIN-SERVES-DEFINITION.md, 8 rows, firewalled auditor): PASS with 2 findings — F1 MEDIUM-HIGH: D8 caller wiring untested end-to-end (auditor's live mutation survived all 11 tests; W4 lesson again); F2 MEDIUM: docs named per-cause staging slugs the code never emitted. Remediated `b8e2e8d..0733cf7` + lead-direct `ffce137` + meta `1f1151be`; auditor re-verified with its OWN mutations (both killed by the new wiring test) and independently re-derived slug inventory = docs — ALL FINDINGS CLOSED. Lead independently reproduced every count (131/2/4 k-filtered; the 2 failures reproduce at baseline `2bd1438` in a clean worktree — pre-existing restart-fixture class, tracked). Restart bootstrap DONE same day: 2026-08-19 00Z+06Z restarts minted (real 24 h march, 4071 s wall-clock, log clean) and installed, lead sha256-verified on host. Parked: (1) hourly-path feed at Phase-L LUT conversion moots the dual cadence; (2) `/home/claude` perms changed to 750 on librewxr between 08-18 and 08-19 (ops agent worked around read-only; surface if other automation reads `~claude/ww3-baselines`). Deploy sequence on operator word "push": push both repos → deploy-marine.sh → config flag rename to `ww3_chain_enabled` → all-terminal-states monitor → reality gate vs NDBC 46253/46222 within one cycle. |
| TRAP-24 + RECON-SHAPE | Buoy-co-located model output; boundary direction-axis fix; reconstruction shape for short-period swell (operator direction 2026-08-22: "attack the wind-sea… deal with the island shadowing last") | ✅ **TRAP 24 CLOSED + VERIFIED 2026-08-22** (marine `7bc743f` co-located Type-2 points B46222/B46253 in the shel deck + vchain match-by-name; `5844e44` **trap-24 fix**: the transfer-file emitter wrote the direction axis nautical-ASCENDING from 180° (same as the F4 reference generator) while `ww3_bound` copies bins POSITIONALLY and `W3IOBC` assumes `ww3_outp`'s own order (nautical going-TO, 90° DESCENDING, first dir π/2) → every boundary spectrum was ingested MIRRORED about N–S (FROM_model = 360 − FROM_file); `ww3_formats._ww3_direction_bin_order()` now emits axis + columns in `ww3_outp` order; SYNTAX-607 row 7 corrected, ADR-109 D5 caveat; tests `b6bb163..cee05c7`, 387 passed on librewxr / 1 unrelated parked (`test_double_break_transect55_kat`); deployed `cee05c7` on the operator's word "proceed"). **Reality gate (stated before looking: directions per band; band-split model-at-buoy vs NDBC data_spec+swdir):** 06Z stage files carry `1.571 1.484 …`; 06Z run shows the W band at 46222 swing 171°→268° and grow 0.05→0.42 m as corrected boundary energy crosses the grid; **12Z clean read: directions RIGHT in every band at both buoys** (46222 W 268° vs obs 275°, S 179° vs 203°; 46253 264° vs 265°, 182° vs 181°), magnitudes ~0.6× in all bands. **Deficit attribution (numbers: `scratch/band_split_check.sh`, `scratch/SESSION-2026-08-22-TRAP24.md`):** (1) wind-sea band = reconstruction SHAPE (light local winds 1–2 m/s in model AND obs; 5–6 s NW chop enters the W edge as a GFS-Wave *swell* partition → Gaussian with zero energy >0.21 Hz vs 46025's broad spectrum; totals equal, split wrong) → **PW10 ruled "1 ok", implemented this row** (`_SHORT_PERIOD_SWELL_MAX_PERIOD_S = 8 s` JONSWAP branch + KAT K8 falsifiable at 6 s/8.0 s; PROVIDER-MANUAL + ADR-104 amended; deployed marine `88fa3ee` 2026-08-22 on the operator's "push"). **PW10 REALITY GATE 2026-08-23 00Z — PASS on the stated expectation** (`scratch/band_split_20260823_00Z.txt`): the >0.2 Hz band went from the worst band (~0.6×) to the best — 46222 model 0.39 m vs obs 0.47 (0.83×, dir 294/285°), 46253 0.48 vs 0.47 (1.02×, 295/286°); boundary W-edge files now carry >0.2 Hz energy (0.33–0.60 m @287–295°); 0.1–0.2 band 0.68–0.81×; S-swell <0.1 band unchanged 0.56–0.60× (the island problem, LAST); totals 0.70×/0.81×. Caveat: different day's sea (trend read, not A/B); (2) S-swell <0.1 Hz: source RIGHT (46086 on the S edge 0.58 vs boundary 0.62) but in-domain 0.6–0.7× where reality shows ~1.0× at unshadowed buoys — reconstructed lobe σθ 15° vs measured 27–31° + no diffraction → deep island lees = **the island problem, operator-ordered LAST**; (3) W band: source 0.77× at S edge + lee. Side findings: pre-existing trace-only `SpecoutParseError` on L1-skipped hourly cycles (nest-handoff trace parses the WW3 transfer as SWAN specout, since ≥08-20); SurfBeat `HSIGN column not found` = parser reads SWAN's `%` comment line as the header (CHAIN-SERVES defect (b), still open). |
| SURFBEAT-FIX | SurfBeat (SWAN IEM) strip: deck to the manual, parsers to the real files, reality gate (operator escalation 2026-08-22 "IS FUCKING IMPORTANT"; ruling "1 ok" on the coordinator's recommendation) | 🔄 **DISPATCHED 2026-08-22 ~20:45Z** (dev agent on the marine repo; brief `scratch/BRIEF-SURFBEAT-FIX-2026-08-22.md`; results-free gate `scratch/GATE-SURFBEAT-DEFINITION.md`). **Findings (lead-verified on the live work dir + manual):** SurfBeat has NEVER produced a result since the 2026-07-25 port — (D1) TABLE parser reads SWAN's `%` comment line as the header and raises every run (production path catches → IG null, no blended profile; 24 failures/3 days on the vchain path); (D2) IG cut-off fig = CGRID `[flow]` (manual :3960–3962, summary :6359–6369) and the deck uses 0.004 Hz → free-IG range `[0.01, 0.004]` EMPTY (PRINT "NTF 1 / MSC 1", second SPEC block NaN/zeros, TABLE row 2 = 0/-9) — no IG exists in the run; (D3) SPECOUT without `L` (manual :5455–5504) — IG spectrum never written; (D4) single `OBSTACLE TRANSM 0 REFL` before both COMPUTEs (manual :3978–3982 wants TRANSM 1 first, REFL second); (D5) GEN3+TRIAD+wind active though the manual (:3962–3964) says the IEM sea-swell run carries only propagation/shoaling/refraction/breaking/friction; (D6–D8) SPECOUT parser exact-matches `RFREQ` (files say `AFREQ …`), reshapes direction-major (files are frequency-major), and uses per-radian dθ on per-degree `VaDens` (Hs_ig ~7.6× low); (D9) "last TABLE row" logic reads the free-IG row as sea-swell; (D10) zero tests on any of it; (D11) strips run inside the `/surf` HTTP handler (25 × ~28 s per request) — and the 48 h journal to 2026-08-22 20:40Z shows ZERO invocations from that path (the only 8 SurfBeat starts were the vchain ledger companions, all failing at the parse) → production IG has had no live source at all; the cycle-vs-request wiring question is architectural (triggers 5/6) and goes to the operator with this round's results. **Pre-approved by the operator:** D2/D3/D4/D5 (spectral domain + physics set) + additive result fields. Sequence: dev → lead live run on librewxr (scratch dir, outside a cycle) → firewalled auditor → reality rows → operator "push". **DEV DONE + LIVE RUN DONE + DEPLOYED 2026-08-22:** marine `4d6ead7` (agent: deck + parsers + 22 tests + pre-fix fixtures) → operator "push/deploy" → lead live run of the new deck in the operator-named scratch dir `/tmp/surfbeat-fix/run1` (real HB bathymetry 806 meshes, boundary 0.63 m / 10.3 s / 207°; normal end 37 s, no Errfile; **COMPUTE 2 now NTF 8 / MSC 8** vs 1 pre-fix; SWAN prints `HBig`; L-file header = the 8-bin IG axis; **bound-IG identity: SPECIG block 1 = 0.0363 m vs TABLE row-1 HBig 0.0368 m at S17**; free IG = TABLE row-2 Hsig 0.022 m at S16 ≈ REFL 0.5 × bound 0.033 m; **the L file's second block = bound-IG array after COMPUTE 2 (1.31 m == row-2 HBig), NOT the free IG** → lead-direct fix `0d993f4`: set timing from the bound block only, post-fix live fixtures + 3 tests; re-deployed 21:05Z; `pytest tests/test_boundary_reconstruction.py tests/test_surfbeat_runner.py tests/test_surfbeat_distance_axis.py` on librewxr **52 passed**). Shoreline numbers this run: Hs_sw 0.61→0.29 m offshore→shore, Hs_ig(S16) 0.040 m (bound 0.033 ⊕ free 0.022), set timing 0.48 min (bound-IG peak 0.035 Hz — the 10 s swell's group band). **(a) CLOSED 2026-08-22 (operator: "manual's decision" / "fix it"):** manual :3690–3706 — a reflecting line must lie inside the grid with wet points on both sides and not through grid points → marine `200098f`: grid = profile meshes + 2 wet pad cells at the shore-end depth, obstacle at `nx_profile·dx + dx/2` for both computes, stations/SPECOUT stay on the profile (shore station half a cell seaward of the line); live run 2 (`/tmp/surfbeat-fix/run2`): NTF 8, COMPUTE 2 converged 99.78%, shore row-2 Hsig 0.0227 m (was 0.0012 on the boundary), HBig 0.0368 → Hs_ig 0.043 m; deployed 23:57Z, 65 tests pass. **(b) CLOSED 2026-08-22 (operator: "this shows something is broken — fix it"):** root cause — `get_surf()` never filled `TransectInfo.bathymetric_profile` (left empty by `compute_spot_transects()`; the beach-profile endpoint already had the cache fill), so the strip's bathymetry guard returned None and SurfBeat was skipped at DEBUG on every request → marine `53dd04f` reuses `_populate_local_bathymetric_profile` before the guard, skip → WARNING, tests `tests/test_surf_surfbeat_bathy_fill.py`; deployed 23:52Z. Production verification (a real `/surf` request producing "SurfBeat strip START/DONE" from that path) pending; (c) **Gate SURFBEAT-FIX: PASS 2026-08-22** (firewalled auditor, results-free gate `scratch/GATE-SURFBEAT-DEFINITION.md`, own numbers: deck traced line-by-line to the manual incl. GEN3-default :2898 and PRINT physics flags; NTF 8 from (fig−df)/df+1; TABLE hand-parse exact; own synthetic nf≠nd SPEC2D KAT; Hs_ig identity 1.29% — row 5 flagged contaminated by a `git show --stat` that printed the commit body, recomputed independently anyway; second OBSTACLE REPLACES the first per PRINT labels; scope diff = brief Scope IN exactly). 2 findings, both remediated same day in marine `582c251` (deployed 21:20Z; 80 tests pass on librewxr across surfbeat/vchain/reconstruction): F1 medium — the named transpose KAT fed a ready matrix to the integrator and survived a transpose mutation of the reader (rewritten through `_read_stationary_spec2d`; lead-verified to fail under an emulated transpose); F2 low pre-existing — `vchain._run_surfbeat` recorded `runtime_s` via `getattr(result,"runtime_s")` (field is `runtime_ms`) → always None (now `runtime_ms/1000`, key kept). **Reality row 8 (stated before looking: shoreline Hs_ig/Hs_offshore vs a cited literature envelope): OUT OF ENVELOPE — model 0.040 m / 0.63 m = 0.06, vs Guza & Thornton (1982) IG swash ≈ 0.7 × incident Hs (Ruessink-class observations: IG height grows to comparable-or-larger than sea-swell in the inner surf zone).** Caveats that keep this a datapoint, not a verdict: the model's "shore" station is 68.65 m seaward of the waterline in 0.42–0.47 m depth (IG keeps growing to the swash); the v1 boundary is a single parametric JONSWAP with 10° spread (bound-IG forcing depends on spectral groupiness — a narrow single train under-forces it); REFL 0.5 is a placeholder; mild conditions (0.63 m / 10 s). Tier-1 external check (NDBC/CDIP IG products) not yet done. **2026-08-23 00Z — first real production /surf run (post-(b)): 25 strips STARTED, 25 timed out at 120 s, IG still null; request 3009 s.** Timed one production-size strip in scratch (`/tmp/surfbeat-fix/run3`): **216 s, converged** — production runs the strip over the full 15 m-contour median profile (1965 m, 3931 cells, 14 m at the boundary) while forcing it with the offshore-most SERVED point (463 m / 9 m depth) → (A) boundary value applied 1.5 km from where it is valid; the vchain companion uses the served transect (403 m, 37 s) — extent = trigger 3, **SURFACED** (recommend: served transect, as vchain). (B) **Direction-frame defect FOUND + FIXED (marine `83c826b`, local, awaiting "push"):** both callers passed the WORLD direction (213°) into a strip whose shore-normal is "from 270°" (manual :3964–3966) → 57° oblique, swell leaked out the strip's open side (Hs 0.58 → 0.04 m in 9 m of water; vchain 0.61 → 0.27 m over 400 m) — `strip_frame_direction()` rotation (wave + wind) inside the runner, `beach_facing_deg` required, tests `test_surfbeat_strip_frame.py`; **every IG number before this (incl. reality row 8) came from a leaking strip — row 8 to be re-run after deploy.** (C) per-request design (25 strips in the HTTP handler: 90 min at the current extent, 15 min at the served-transect extent) = triggers 5/6, **SURFACED**. Record: `scratch/SESSION-2026-08-22-TRAP24.md` §2026-08-23. **OPERATOR RULINGS 2026-08-23 (chat):** (1) the coordinator's "shorten the strip" recommendation REJECTED — long strip (15 m contour → shore) stays; what is wrong is the FEED: "it SHOULD GET FED FROM THE POINT IT STARTS" → a per-hour boundary value at the strip's own seaward end (no such output exists today — outermost carried point is 463 m / 9 m; new SWAN nest output at the 15 m-contour point per spot required); (2) SurfBeat is NOT a request-time computation — it runs in the cycle like the rest of the model, and EVERY forecast hour (cadence 3 h → 1 h) like the rest of the surf forecast (coordinator had misdescribed get_surf()'s request-time SurfBeat loop as the design; it is the T2.2 port's placement, not the architecture); (3) width may be cut only if accuracy-neutral, never more than half → **measured NOT neutral** (scratch run4d/e: 250 m costs −10 % Hs / −12…−17 % IG on a 400 m strip at 9° obliquity; 100 m −37/−45 %; SWAN REFUSES fed lateral boundaries with SURFBEAT — "boundary specification is not correct in case of surfbeat") → width stays 500 m (itself marginal for the 2 km strip); cost levers = dx (0.5 m over the whole 2 km is what makes a run 216 s; SWAN REG forces one dx) and parallel workers. Direction-frame fix alone on the 400 m strip: shoreline Hs_ig 0.04 → **0.19 m (0.30 × Hs_offshore)**. **(4) OPERATOR RULING 2026-08-23: "surfbeat should NOT BE USED FOR THE PROFILE"** — the T2.3 blended beach profile (SurfBeat strip sea-swell Hs seaward of the break, SwellTrack shoreward, 50 m taper) mixed two models on one curve = a root cause of the weeks-long profile-accuracy fight (and, since the strip was leaking energy through its side, the "physically accurate" approach-zone Hs it grafted in was itself wrong). **REMOVED marine `b576d3c` — PUSHED + DEPLOYED 2026-08-23 05:18:37Z on the operator's "push" (with `83c826b` strip-frame fix; `deploy-marine.sh`, health/manifest 200, 93 tests pass on librewxr across strip-frame/runner/distance-axis/beach-profile/bathy-fill/refinement, post-deploy journal sweep: only pre-existing NDBC-quota + HRRR-404 classes). NOTE: production SurfBeat still times out at 120 s on the 2 km / 0.5 m strip until the rebuild round (feed-at-start, in-cycle hourly, dx) — IG fields stay null until then.** `_blend_hs_profiles`, the `surfbeat_hs_profile` input, the surf.py→beach_profile result hand-off cache; profile Hs = SwellTrack `hs_total_profile` point-for-point; SwellTrack-only envelope pin test. SurfBeat's consumers are now exactly: setTimingMinutes (bound-IG peak; 60 % of the score's 10 % consistency row), setAmplitudeM = igWaveHeightM (shore IG Hs; 40 %). Sizing follows from that use (shore-station numbers; strip station spacing 25 m): cross-shore dx is the cost lever (0.5 m → 216 s/run; 5 m → ~22 s) and the SURFBEAT df (0.005 → 8 timing values 25–200 s) is the timing-quality lever — operator decision pending on dx; flagged: the scorer's timing bands are 3/10/20 MINUTES while the bound-IG peak is physically 25–200 s (ADR-101 spec question, not touched). Doc-sync: ARCHITECTURE.md :149, API-MANUAL §17 + "Beach profile Hs is SwellTrack only", DASHBOARD-MANUAL :1173, PROVIDER-MANUAL §14 index line. |
...
| **Q17-WIND-DEPTH** | Horizon march never ran — GFS far-window fetch f096 → f108 (Q17, operator ruling 2026-08-27 "a") | 🔄 **CODE DONE 2026-08-27 (lead-direct, local, NOT pushed): marine `2a05856`** — `wind_gatherer.py` `_GFS_FAR_FETCH_END_HOUR = 108` + comment corrected; `tests/services/test_wind_gatherer.py` pin updated same commit (f048..f108, 21 hours). Lead test run (Windows, `python -m pytest tests/services/test_wind_gatherer.py tests/test_ww3_horizon_march.py -q`): **31 passed / 1 failed** = `TestColdStartReconcile::test_loads_store_then_polls_and_persists`, reproduced identically with the change stashed (the known pre-existing Windows-path failure from Round A) — zero introduced. Evidence `scratch/SESSION-2026-08-27-HORIZON.md`. Doc-sync: ARCHITECTURE ⚓ WW3 leg, ADR-109 fetch-depth note, PROVIDER-MANUAL §14.18, CHANGELOG. **Live gate owed at deploy (stated before looking):** next 00Z cycle's `ww3Horizon.lastSuccessCycleTime` non-null with `coverageEndTime` = cycle+96 h, `wallClockS` ≤ 5 h, zero overlap with the 06Z production run; the following full cycle shows `fullRun.l2BoundaryExhausted: false` and zero "boundary file exhausted" lines in SWAN L2's PRINT; served DWR partition varies across hours 7–72. Awaiting operator "push". |
| **CS-BASEMAP** | Q18 Round 1 — Clear Skies product basemap (marine + seismic maps only): OSM light kept; Protomaps dark + marine label overlay from a self-served extract derived from station+quake radius and marine locations (+ z0–6 world baseline); CARTO removed | ⬜ RULED 2026-08-27 — brief next |
| **RADAR-STRIP** | Q18 Round 2 — radar/satellite box = empty Leaflet + provider tiles; remove CS base/labels/outlines and the whole ADR-078 geographic-features feature (API, admin, config, dashboard) | ⬜ RULED 2026-08-27 — ships only AFTER LIBREWXR-BASEMAP serves ground+labels |
| **LIBREWXR-BASEMAP** | Q18 Round 3 — LibreWxR fork renders its own basemap + place labels + outlines (OSM/Protomaps extract of its own BBOX) under/into radar + satellite tiles; not CARTO, not Esri | ⬜ RULED 2026-08-27 — fork brief next (not a Clear Skies repo) |
...
| W-Accept | Deploy in shadow mode (runs but does not serve) | ⬜ NEXT — awaiting operator push word; live rows owed: real-binary chain on librewxr (tree-kill POSIX branch, real march), shadow-run liveness, zero served-path impact (baseline/diff), compute inside D12 budget. WW3 binary provisioning from /tmp/ww3-feas artifacts + sha256 pinning is part of this deploy's runbook |
| Gate W | Independent audit per round | ✅ **ALL ROUNDS + PHASE-CLOSE SWEEP PASS 2026-08-17** (adversarial, results-free definitions, firewalled): W1 PASS+remediation (tree-kill coverage honesty); W2 PASS (transposed-control mutation-killed; real ww3_bound ingest rc=0/313 points lead-run); W3 PASS 6/6 (byte-identity 1-char mutation-killed); W4 FAIL→remediated→PASS (proxy-KAT blind to real call site — the phase's defining finding); W5 PASS 8/8 (+slug split); W6+phase-close PASS 9/9 (row-8 refuse-gap FAIL→remediated 300f190→re-verified; deck-line catalog traceability full-walk; forbidden-syntax clean; live path change-inventory fully KAT-covered). DOC-W-FINAL as-built checklist: `scratch/DOCWFINAL-CHECKLIST.md`. LESSONS captured in rows above: real-integration-point mutation drills mandatory; one committing agent per working tree; proxy-KAT pattern named |
...
(`ww3_horizon_wind_short` every attempt — wind fetched 6 h short by arithmetic; see Q17) so
`fullRun.l2BoundaryExhausted` is still TRUE — Round A has delivered nothing live yet. Q17 ruled
"a" and coded lead-direct (Q17-WIND-DEPTH row) — awaiting "push", then the live gate at the next
00Z cycle. Second operator report same session: CARTO basemap tiles now render an "API KEY
REQUIRED" watermark on the marine/seismic/radar maps — RULED (Q18): three rounds — CS-BASEMAP
(Protomaps dark + labels for marine/seismic, OSM light kept, CARTO gone), RADAR-STRIP (Clear
Skies provides nothing for the radar box; ADR-078 removed), LIBREWXR-BASEMAP (labels + ground
belong in the LibreWxR fork). Briefs after the Q17 push.**

**Previous status 2026-08-25: Q16 BOTH ROUNDS PUSH-READY.** Operator-reported "missing swells" traced
...
pipeline for this test install (V4 serving gate overridden by that order). Phases
F/DOC-W/W complete; the vchain shadow round (2026-08-18) is superseded before it ever
produced a ledger row — see its retroactive checklist row for the 19 h restart-missing
...
| PW10 | **Boundary-reconstruction frequency shape for short-period swell partitions** (added 2026-08-22): swell partitions with `SWPER ≤ 8 s` take the wind-sea JONSWAP shape (γ 3.3, fp = 1/SWPER) instead of the σf = 0.015 Hz Gaussian; spread, long-swell shape, wind-sea train all unchanged. Evidence: 2026-08-22 12Z W-edge boundary spectrum zero above 0.21 Hz vs NDBC 46025 broad 0.13–0.27 Hz 23 km inside (band totals equal, split wrong); same at S edge vs 46086. Code `services/boundary_reconstruction.py` (`_SHORT_PERIOD_SWELL_MAX_PERIOD_S`), KAT K8. | 1 | Operator 2026-08-22 in chat ("1 ok" to the coordinator's recommendation (a): JONSWAP-type shape for swell partitions ≲ 8 s) |
...
- Reconstruction spectral constants (JONSWAP γ 3.3, cos²ˢ spreads s=28/s=7, adaptive σf
  ...
  an output format, never touches how a spectrum is built. **AMENDED 2026-08-22 (PW10,
  operator ruling in chat "1 ok"):** swell partitions with `SWPER ≤ 8 s`
  (`_SHORT_PERIOD_SWELL_MAX_PERIOD_S = 8.0`) take the JONSWAP frequency shape (γ 3.3,
  fp = 1/SWPER) instead of the σf Gaussian; spread s=28 unchanged; long swell unchanged.
...
### Q18 (2026-08-27) — ✅ RULED IN CHAT: CARTO basemaps out; map ownership corrected (three rounds)

**Trigger:** CARTO began (~2026-08-25) watermarking every raster basemap tile requested without an
API key ("API KEY REQUIRED", confirmed on our own tiles, `scratch/carto_dark_tile.png`) and says the
raster product is being retired. Every map in the product that used CARTO is broken in both themes.
Operator rulings, verbatim essentials: **"NO ESRI!"** (compliance + caching restrictions); keep OSM
raster for light ("easier ... our dark maps contain MUCH LESS information"); Protomaps (OSM data,
no key, vector, our own extract) for the dark maps; **the radar/satellite box is NOT a Clear Skies
map** — "WE DO NOT PROVIDE ANYTHING FOR THE RADAR ... whatever they provide, we dump it in our
leaflet box, that is all"; LibreWxR (our customised fork) is an external provider like RainViewer,
its bounds/tiles are not inputs to anything Clear Skies derives; and the radar labels that were
coded into Clear Skies "needed to be a change to LibreWxR" — "you need to undo that and move it."

**Round 1 — CS-BASEMAP (Clear Skies product basemap, marine + seismic maps ONLY):**
- Light theme: OSM raster (`tile.openstreetmap.org`) — unchanged.
- Dark theme: Protomaps vector rendered by `protomaps-leaflet` (already a dependency) from a
  Protomaps BBOX extract Clear Skies serves itself. Protomaps' dark theme as the starting point,
  tuned to today's sparse dark look; a labels-only rule set replaces the marine map's CARTO
  `light_only_labels` overlay (both themes).
- **Extent is DERIVED from Clear Skies' own configuration only** — no "station area" exists as a
  product concept, and no external provider's extent may be used: union of (station ±
  `[earthquakes] default_radius_km` × 1.15 — the seismic map's own initial bounds; 200 km on this
  install) and (bounding box of all configured marine locations + padding, detail to z15 for the
  z14 hero view); plus a z0–6 world baseline file so any map dragged anywhere shows coarse
  land/coast instead of blank. Sizes measured in the round (today's single-box extract: 36 MB).
  The operator-typed `[geographic_features] bounds` key does not survive (see Round 2).
- CARTO removed from Clear Skies entirely: URLs, `CARTO_OSM_ATTRIBUTION`, About-page row, tests,
  DASHBOARD-MANUAL §12.

**Round 2 — RADAR-STRIP (Clear Skies provides NOTHING for the radar box):**
- The radar/satellite box becomes an empty Leaflet container + the provider's tiles. Removed from
  Clear Skies: the OSM/CARTO base under the radar, the CARTO `voyager_only_labels` satellite
  overlay, `GeoFeaturesLayer`, and the whole ADR-078 geographic-features feature (API
  `/api/v1/geographic-features/*`, `services/geographic_features.py` extract, `POST
  /setup/geographic-features/update`, admin page, `[geographic_features]` config, `pmtiles` CLI
  dependency on the API host). ADR-078 → Superseded.
- Deploy ORDER: Round 3 ships and serves ground+labels BEFORE Round 2 strips Clear Skies, so the
  radar view never goes bare in between.

**Round 3 — LIBREWXR-BASEMAP (in the LibreWxR fork, `deploy/shaneburkhardt` branch — NOT a
Clear Skies repo):** LibreWxR's tile output carries its own basemap with place-name labels and
outlines (OSM data via a Protomaps extract of LibreWxR's OWN `LIBREWXR_BBOX`, rendered
server-side into/under the radar and satellite tiles). Not CARTO, not Esri. Design to be written
in the fork's own brief; Clear Skies has no knowledge of it beyond "dump the provider's tiles."

Rounds 1 and 2 are the Clear Skies deliverables; Round 3 is the LibreWxR deliverable. Briefs and
dispatch follow the Q17 push.

### Q17 (2026-08-27) — the daily 96 h WW3 horizon march has NEVER run: it asks for wind that does not exist yet when it fires. Your ruling on the fix.

**What is happening (all numbers from the live host, 2026-08-27, commands in
`scratch/SESSION-2026-08-27-HORIZON.md`):**
- `GET /health` → `ww3Horizon.lastSuccessCycleTime: null`, `refuseReason: "ww3_horizon_wind_short"`;
  `fullRun.l2BoundaryExhausted: true`. The Q16 Round A fix is deployed (marine `b62008f`
  running since 02:26Z) but has delivered nothing: SWAN's forecast beyond +6 h is still frozen
  every cycle.
- Journal 03:36:17Z: *"WW3 horizon march: native wind coverage ends at 2026-08-30T18:00Z, more
  than one native step (3 h) short of the window end 2026-08-31T00:00Z — refusing."*
- The march runs once a day, right after the 00Z cycle publishes (03:36Z today). It demands wind
  out to 00Z + 96 h. The wind store's far window comes from NOAA's GFS model, and the gatherer
  always holds the GFS cycle that is one step BEHIND the HRRR cycle (18Z the day before), fetched to
  that cycle's +96 h — which is only 00Z + 90 h. Six hours short, by arithmetic, every day.
- It cannot be rescued by waiting a little: NOAA posted the 00Z GFS +96 h file at 04:00:46Z (probed
  `Last-Modified`), 24 minutes AFTER the march fired, and the gatherer does not even look for the
  00Z cycle until 04:30Z (`gfs.py` 4.5 h availability lag). There is no retry — one refusal and the
  day's march is gone (`service.py:2226` is the only call site).
- The same design already knew this: the gatherer's own comment says GFS "structurally lags the
  triggering HRRR cycle by ≥1 of its 6 h steps" and deepened the 72 h run's fetch for exactly that
  reason — then the 96 h march assumed no lag ("48 + 3×16 = 96 lands exactly on the step").
- The ocean boundary side is NOT short: the fetch depth derives from the window end
  (`ww3_partition_fields.py:659–666`; pinned to the 18Z cycle it asks for f105, NOAA publishes to f180).

**Options:**
- **(a) RECOMMENDED — deepen the gatherer's GFS far-window fetch from f096 to f108.** One
  constant (`wind_gatherer.py:190`). Covers the march's +96 h even when GFS is two steps behind
  (the same two-step margin the 72 h run already uses). Cost: +4 GRIB2 files per GFS cycle
  (17 → 21). The march then finds its wind already in the store when it fires — no waiting, no
  retry machinery, no schedule change. Side effect: wind for the march's hours 90–108 comes from
  the 18Z GFS run instead of 00Z — the same NOAA cycle the ocean boundary is already pinned to.
- **(b) Make the march wait for the 00Z GFS cycle** (poll the store until it covers +96 h, ~04:40Z
  start). Adds retry/deadline machinery, and a ~4.3 h march starting 04:40Z runs into the 06Z
  production cycle (~07:50Z) — breaks Q16's own "zero overlap with production runs" gate.
- **(c) Let the march run with what it has (90 h) and record the real coverage end.** The merge
  already tolerates a short horizon (`ww3_horizon_short` WARNING). Leaves the next day's 00Z
  cycle frozen for its last 6 forecast hours only. Smallest change but keeps a known hole.

**Triggers:** (a) changes a fetch depth the Q16.1 approval named as "+96 h" (trigger 7 territory,
since you approved that number) — hence this question rather than a fix. Nothing has been changed.

**→ RULED 2026-08-27 (operator, chat: "a").** Implemented lead-direct (one constant + its
pinning test + doc-sync) — see the Q17-WIND-DEPTH checklist row for commit, test run and the
live gate owed at the next 00Z cycle.
...
### Q14 (2026-08-23) — approve the set-timing / set-amplitude definitions in `docs/planning/briefs/SET-TIMING-AND-AMPLITUDE-BRIEF-2026-08-23.md`?

**RULED 2026-08-23 (operator, chat): "q14 recommendation is fine."** Lead's reading, stated back in chat: (a) the brief §7 definitions are accepted as the ADR-101 row-5 amendment — graded interval curve + waves-per-set curve (timing = 0.7/0.3), set interval from the dominant partition's spectral group statistics with the two-swell beat override, amplitude = set-strength index S = 0.4·C' + 0.6·κ_dom from SwellTrack per-partition break heights + spectrum; (b) the brief's own §4.5 recommendations stand — the at-the-break partition share is a distinct measured quantity (filler term KEPT), H_set enters only as a ratio denominator (accepted), `swellDominance` stays the no-spectrum fallback. §3.3 step 4 and §5 (SurfBeat cross-check / relabel) are MOOT — SurfBeat was removed by the later ruling the same day. Next: the two pre-coding verifications (§7: Longuet-Higgins expressions or Kimura route; typical partition-band ν/κ from cached 15 m spectra), then ADR-101 amendment + coding round with known-answer tests → row CONSISTENCY-SCORING.

**What it proposes (structure and weights of the score untouched; only what feeds the two terms inside Consistency changes):**
- **Set timing** is scored on a GRADED curve of the expected set interval, anchored on what surfers report: sets every 2–6 min = best (1.0); 10 min = 0.85 (still the groundswell norm, half the opportunity); 15 min = 0.6 ("fifteen or more" is where sources stop calling it normal); 20 min = 0.4 (the "15–20 minutes for a set that never comes" frustration line); 30 min+ = 0.25 (the "half an hour" bad tail); under ~1 min = 0.8 (no discernible sets — continuous train; still rideable, which Size/Shape judge). Plus a waves-per-set curve: 3–6 waves = 1.0, 1–2 = 0.6–0.75, 8 = 0.95, 10 = 0.9, 14+ = 0.8. Timing = 0.7 × interval + 0.3 × waves-per-set. Not binary, not swell-type-dependent.
- **Where the interval comes from:** our own hourly wave spectrum at the 15 m point, restricted to the dominant swell partition — its narrowness gives the expected waves per set and sets per N waves by 40-year-old wave-group statistics (Kimura 1980, verified from the paper; Longuet-Higgins form to re-verify before coding), × mean period = set interval; a two-swell "beat" (1/|1/Tp₁−1/Tp₂|) overrides when a second swell holds ≥ 25 % of the energy. No new model runs.
- **Set amplitude ("set strength") from OUR 1-D model + spectrum, not SurfBeat IG height:** absolute height is out (Size's input; and for a single swell the set-wave/lull-wave ratio is a Rayleigh constant ≈ 2.3 on every day). What varies is (a) coherence — do big waves come together (κ from the spectrum) and (b) the lull floor — what the OTHER partitions put in the water between sets, which SwellTrack gives per partition AT THE BREAK (`hs_at_break_m`). Contrast C = 1 − H_lull/H_set (H_set = 1.27·Hs_total at the break; H_lull = Rayleigh low-run of the dominant swell ⊕ mean wave of the rest), normalised; set-strength S = 0.4·C' + 0.6·κ; f_amp bands 0.5–1.0 (judgement, labelled).
- **SurfBeat's role:** its peak period is the dominant wave-GROUP period (tens of seconds, within a set) — logged as a cross-check and a "groups per set" diagnostic; its height is display only; `setTimingMinutes` relabelled to what it is.
**Needs your ruling:** (a) accept these definitions as the ADR-101 row-5 amendment (then a coding round with known-answer tests; the brief lists two things to verify first — the exact Longuet-Higgins expressions or use the Kimura route, and typical partition-band narrowness from a week of our cached spectra); (b) single-use flags §4.5: the at-the-break partition share correlates with Power's deep-water energy share — accept as a different measured quantity, or drop the filler term and let κ alone carry amplitude; H_set is Size's face height used only as a ratio denominator — acceptable?

### Q12 (WITHDRAWN — see above; original text kept for the record) — what should "consistency" MEAN in the surf score?

**Context:** The research (`docs/planning/briefs/RESEARCH-SET-CONSISTENCY-2026-08-23.md`) found the word is used
two near-opposite ways. (i) **Surfers' sense:** well-defined, regularly spaced sets with real lulls —
a clean long-period groundswell, far-travelled, one swell in the water. Narrow spectrum → strong
groups → widely spaced sets. (ii) **Surfline's sense:** how many rideable waves arrive per hour —
a broad, short-period windswell scores HIGH here (waves keep coming, sets blur together); Surfline
itself says this is "not a measure of surf quality". On the physics axis (spectral narrowness) these
two rank days in opposite order, so no scoring curve can be written until you say which one you
want. **Recommendation:** (i), the surfers' sense — it is what ADR-101's own rationale describes
("regular, predictable sets give more ride opportunities") and what the rest of the score leans
toward. **Your call.**

### Q13 (2026-08-23) — which definition, once Q12 is ruled?

**A (recommended):** compute set regularity from the hourly wave spectrum we already have at the
15 m point — its narrowness gives, by 50-year-old wave-group statistics, the expected waves per set
and set interval; two near-equal swells add a slow "beat" override. No new model runs; computable
today; explainable ("a narrow 15 s swell arrives in well-spaced sets, a broad windsea arrives
continuously"). Its numeric bands are judgement until validated (see below).
**B:** keep using the surf-beat model's output but correctly — in seconds (waves per group = IG
period ÷ wave period), and the surf-beat height as a RATIO to offshore height (removes the size
bias); to see anything beyond 200 s the model's IG frequency step must be refined (a model-extent
change, needs your approval). Honest about what the model gives, but it is a group-period sensor,
not a set-interval sensor, and it omits the free infragravity energy that dominates in reality.
**C:** Surfline's rideable-wave rate — not recommended: it is a function of the face height the
Size factor already scores (double-counting) and Surfline says it is not a quality measure.
**Regardless of choice, two things are defects to fix:** the minutes-vs-seconds bands (the timing
sub-score is stuck at 0.8) and the uncited absolute-metre amplitude bands (a disguised size bonus).
**Validation:** no set-interval observations exist anywhere; the cheapest real test is counting sets
on the spot's webcam time-lapse for a couple of weeks across a groundswell and a windsea. **Your
call** on A/B(/C), and whether to run the webcam validation.

**Q10 RULED 2026-08-23 (operator: "10. delete"):** remove `surfbeat_cadence_hours` everywhere — marine parser (`config/marine_config.py`), the manuals' config tables, and the API/wizard/admin surfaces that write or describe it (spans the api + dashboard repos — a small follow-up round; task row SURFBEAT-CADENCE-KEY-REMOVAL below).

**Q11 RULED 2026-08-23 (operator):** "there is no such thing as 10 percent of the total as it is not a sum score" — correct, the coordinator mis-stated it: ADR-101's score is a **geometric mean** of weighted factors, and consistency is one factor with weight 0.10 in that product, not 10 % of a sum. Ruling: **"the entire score for that section seems completely wrong and poorly researched. It needs researched properly."** → a research task (targeted web searches + the project's own data; NOT the deep-research skill) producing a brief on what "consistency" / set timing physically is, what is observable/computable from our model (SurfBeat IG peak, spectral bandwidth / groupiness, partition structure), how the published literature and commercial forecasts treat it, and a proposed scoring definition with citations — then an ADR-101 amendment proposal for the operator. No code changes to the scorer until the operator accepts the research. Task row CONSISTENCY-RESEARCH below.

### Q10 (2026-08-23) — remove the dead `surfbeat_cadence_hours` setting?

**Context:** You ruled on 2026-08-23 that the surf-beat strip runs every forecast hour inside the
forecast cycle, like the rest of the model. Before that it ran every 3rd hour, and the number "3"
came from a per-spot setting called `surfbeat_cadence_hours` (admin page / config file; default 3).
Since the change, nothing reads that setting any more — the auditor confirmed zero readers in the
whole marine repo. It still exists in the config parser (`config/marine_config.py`), in the
admin/wizard help text and in the manuals' config tables (now marked "dead"). Leaving it is
harmless but misleading: an operator could change it and nothing would happen.

**Options:** (a) delete the key from the parser, the manuals' tables and the admin/wizard surfaces
(removing a config key is a hard-block trigger, hence this question); (b) leave it, documented as
dead. **Recommendation:** (a), in a small follow-up round together with the ADR-101 question
below (the score's set-timing bands). **Your call.**

### Q11 (2026-08-23) — the surf score's "set timing" bands are in minutes, the physics gives seconds

**Context:** The consistency part of the surf score (ADR-101, 10 % of the total) rates set timing
with bands at 3 / 10 / 20 **minutes**. The surf-beat model's set timing is the period of the
strongest infragravity band, which is physically 25–200 **seconds** (today's live value: 33 s).
So that score can only ever land in its bottom band. Not touched this round — it is a scoring
specification question, not a model one. **Options:** re-band in seconds (ADR-101 amendment), or
leave and revisit with a real set-timing observation. **Recommendation:** re-band after the first
week of live cached values shows the range. **Your call.**
