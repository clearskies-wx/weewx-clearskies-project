...
| ~~**B-3b-winddir**~~ ⏸ **DEFERRED (low severity)** | `windDirLabel()` recomputes compass client-side. | `now.tsx:49-52`, `forecast.tsx:16-19`. | **The BFF flattens `windDir` to a bare number** (`data.windDir=258.0`, `units.windDir='°'`) — no compass label in the payload to consume, so "use BFF label" needs a coordinated BFF change to expose a cardinal field. Nothing else renders a conflicting label → ADR-042-purity cleanup, not a visible bug. Revisit with a BFF `windDirCardinal` field. | realtime+dashboard |
