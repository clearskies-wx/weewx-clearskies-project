...
| D1 | As-built doc re-sync + zero-drift audit (the predecessor's DOC-W-FINAL) | ✅ **DONE 2026-08-27** — docs re-sync meta `fd30d233`…`de1f646a` (8 commits; Gate M F2/F3 + four more stale "not yet shipped" sentences fixed; health blocks, chain-gate paragraph, PROVIDER working-directory paragraph, contract S2 fields, CHANGELOG consolidation); **Gate D1 PASSED 12/13 + X9 fixed** (firewalled auditor: every ARCHITECTURE/manual/ADR claim resolved to code at the pinned hashes; OPERATIONS-MANUAL procedures executed against a scratch layout; all four amendments Proposed; removed things never described as live; every checklist hash resolves. X9 MEDIUM: the three still-mounted ADR-078 routes had no contract paths → meta `ee6cb141` adds them `deprecated: true` + API-MANUAL §12c). Report-only: J12, J15. Artefacts archived below |
