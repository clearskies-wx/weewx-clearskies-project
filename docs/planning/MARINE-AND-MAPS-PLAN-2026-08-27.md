...
| M4 | **SURF-MAP-BASEMAP** — the surf height map's background becomes the product basemap (light OSM / dark Protomaps); Esri World Topo (IMAGERY-MAP) and NAIP removed from every user-facing surface; the wizard's Esri satellite toggle STAYS (operator-only, not user-facing) | 🔄 **M4-API landed 2026-08-27** (API `f4a42a7`, `87924c9`, `07e0322`, tests `39cd51a`; meta `897a79b3` openapi Imagery paths — a pre-existing contract gap closed; API-MANUAL §12a as-built): `/imagery/config` always answers `provider: "basemap"` (light OSM template + dark `basemap/local` tier, zoom 0–19), never 404s; `[imagery] provider` values are ignored with one startup WARNING (modules/key/admin/wizard stay — Q10-6 open). Lead reproduced **56 passed**. **M4-DASH ACCEPTED 2026-08-27** (dashboard `ba00a35`…`b307797`: `rasterizeBasemapTile` (protomaps `View`/`paint`, LRU ≤ 256), `useRasterizedTiles`, HeatMapCard dark href = data URLs / light = `light.tileUrl`, About row removed, env-gated dev stand-in; tests `1e1bac4`…`b4179d7` (100 passed; geometry byte-identity KAT); meta `ace31e69` DASHBOARD-MANUAL §12; lead reproduced tsc 0 + suites (a 6-suite run hits Windows EMFILE — run singly); renders: light OSM ground, dark rasterized with ZERO external tile requests; axe = 2 pre-existing app-shell only). **Gate M4 PASSED** (auditor's own probes/renders: `/imagery/config` byte-exact; light = 168 OSM hrefs, dark = 168 data-URLs; dark theme makes ZERO external requests; every geometry helper byte-identical to `43afaee`; killing only the pmtiles/status routes → 0 image elements, never a remote fallback; tsc 0; suites green singly; H3–H5 superseded by Q10-6 (gate-definition wording, annotated). Informational: pre-existing axe `empty-table-header`/`scrollable-region-focusable` on the Surfing tabpanel (journal J18); dev-recipe env-gate co-hosts two stand-ins (note for the next auditor)). **M4-B IMAGERY-REMOVAL LANDED 2026-08-27** (Q10-6 "get rid of it"): API `a733c24` (providers/imagery/* deleted, dispatch rows, `ImagerySettings` + `[imagery]` loading, startup checks, `/imagery/tiles` NAIP proxy, `ImageryTileQueryParams`, setup `_PROVIDER_DOMAINS`), `cc38c37` CHANGELOG, `4e61355` comment cleanup; tests `496b93e` (8 files removed), `2308c0b` (basemap-config test setup amended, 2 dead classes removed), `99d43c2` guards (7); stack `72a76e4`…`d3da6ba` (admin section, template + include, wizard fieldset, `imagery_api_key`, 4 en.json help keys, OPERATOR-MANUAL + CHANGELOG), tests `9a26a5a`/`c572421`/`d3d8bec` guards (18); meta `07fba991` (contract `/imagery/tiles` removed, API-MANUAL §12a, OPERATIONS-MANUAL migration note, PROVIDER-MANUAL §16 pointer, CHANGELOG) + `dea315ab`. Lead reproduced API **34 passed**, stack **48 passed** (stack now has a lead-built `.venv_local` — the repo had no runnable local test env). `/imagery/config` byte-identical; `step_marine.html:931` toggle untouched; renders confirm no Imagery UI. **Gate M4-B PASSED 14/16 + F1 fixed** (auditor's own TestClient/stack-app probes; legacy `[imagery]` conf loads clean; `/imagery/config` byte-identical to the pinned body incl. required `lat`/`lon`; toggle untouched; docs/contract consistent. **F1 MEDIUM** (pre-existing, but the brief's own inventory named it): `_write_api_conf()` wrote ANY provider domain the apply payload sent — a real `/setup/apply` call with `{"imagery": …}` deposited `[imagery] provider = naip` in api.conf → lead-direct API `811fe88`: write allowlist = the five known domains, unknown domains WARNING-logged and dropped, allowlist pinned to the read-side tuple; 37 passed; auditor's real-route script re-run → no `[imagery]` written). Findings kept: J16 (pre-existing stack test failures), PEP 420 empty-dir note in OPERATIONS-MANUAL |
| Gate M | Adversarial gate per round + one end-to-end row (every map surface rendered in both themes, screenshots side-by-side) | ✅ **PASSED 2026-08-27** — per-round gates M1-API (13/13 + 2 fixes), M1-DASH (16/17 + D16 fix), M4 (all rows), M4-B (14/16 + F1 fix); **phase sweep** (firewalled, `scratch/GATE-M-PHASE-DEFINITION.md` P1–P12): 10+ own renders both themes, provider grep clean (only the wizard's operator-only Esri toggle), extent derivation traced (station/quake/marine only; radar = coverage box), PMTiles headers parsed independently (44.8 / 513.5 / 204.5 MB, matching status.json; Q12 accepted), kill-the-tile-server → marine banner, no remote fallback anywhere, ADR-078 A2 Proposed with the old feature still additive (removal correctly not executed), ADR-046 still met, tsc 0, all suites green. Findings: F2/F3 MEDIUM — two stale "not yet shipped" sentences (ARCHITECTURE.md:12/:384 dashboard consumption; OPERATIONS-MANUAL:658 stack admin page) → **D1**; F1 LOW gate wording re-worded; F4 LOW stray untracked screenshot dir in the dashboard tree (lead removed); F5 LOW `seismic.tsx` has no dedicated vitest (pre-existing, J21). Held: surf-map live render against weather-test (pre-deploy backend never served the Surfing tab data), satellite frames (J20), axe against a deployed instance |
...
| S2 | **CONSISTENCY-SCORING** — code the Q14-approved set-timing/set-amplitude definitions into the surf score (ADR-101 row-5 amendment first). NOT in code today — the scorer still runs the interim swell-dominance bucketing | ✅ **ACCEPTED 2026-08-27 — Gate S2 PASSED 14/15, F1 fixed.** Q10-1 ruled yes (A–E stand). Code: marine `9d802e0` (`services/wave_groups.py` — Kimura/Battjes–van Vledder run lengths at ρ = 1.80, κ at lag Tm02, ν from moments, half-way bands), `0fb9a5e` (Consistency = 0.6·f_int(T_set) + 0.4·f_amp(S); per-component scalars `nu/qp/kappa/tm02S/nRep/tSetS/bandHz` attached at DWR parse; wire fields optional; fallback byte-identical), `d0f6848` CHANGELOG; KATs `3c11049` (57 tests, independent dblquad references); meta `5a49df71` (API-MANUAL §17, DESIGN-MANUAL). Lead reproduced **70 passed**. Rulings: beat "secondary" = highest raw `energy` partition (SET-TIMING §3.3 Step 3); dominant↔`per_partition_breaks` match exact via `surf.py:1219`/`surf_1d_pipeline.py:1449`; `main_zone_face_height_m ≤ 0 → None → S = κ_dom`. **Gate S2** (auditor's own Kimura integral matched to < 0.04 %; pre-change scorer byte-identical across 5 fallback cases; wire round-trip through the API converter under 2 unit systems; units-slip hypotheses refuted): F1 MEDIUM — fallback branch unlogged → lead-direct `1a23f7b` (WARNING once/process then DEBUG; guard test; 72 passed); F2 LOW — the API repo's dead `SpectralWaveComponent` Pydantic model lacks the 7 fields (not on the wire path; journal J15 → Phase D). API repo needs no code change (`_walk` passes unknown keys). Live: first cycle's `tSetS`/`kappa` distribution held until push |
| S3 | **Substitution cleanup** — CORRECTED after the adversarial review: `_reused_l1_boundary_command_lines()`, the `swan/level1/` directory and `ww3_chain_enabled` are LIVE dependencies (production L2 scaffold; buoy-ledger gate) and are NOT deleted. Remaining: `level1` label rename (cosmetic), the health `ww3_boundary` entry (record it or remove it — Q10), doc corrections (ARCHITECTURE.md:130/132 wrong "no-op"/"vestigial" claims + gap rows #12–#16, PROVIDER-MANUAL:2529 swell-card bullet, API-MANUAL §17 `swellSource` + `closeoutFraction`, ADR-109 G7 struck, D14 item 2 disposition), hotstart-age gate (Q10: drop or own row) | ✅ DONE 2026-08-27 — (a) doc round landed (meta `60738f16`, `d4e89b87`, `16dfe52e`, `5422800a`, `1d56f7a8`; lead corrected one over-claim the same hour: the chain path and scaffold reuse are unconditional, the flag gates ONLY the buoy-ledger writers); (c) `ww3_boundary` recorder landed (marine `ee406a7`, guard tests `d6d4a1e`, lead reproduced 23 passed; Q10-7); (d) dropped (Q10-5). **(b) `level1` → `deep_water` rename LANDED 2026-08-27** (marine `c57bb8e` — `DomainSizing.deep_water`, `compute_deep_water_domain`/`_compute_deep_water`, persisted sizing key read-both/write-new at `domain_sizing_to_dict`/`from_dict` + `_domain_geometry_signature` with one INFO on a legacy read, named `SWAN_LEVEL1_DIRNAME`-style constants for the on-disk `level1/` paths; `c98b1ba` CHANGELOG; KATs `7e434c8`/`7aeee99` (5, incl. a grep-guard pinning the 48 surviving literals — all on-disk paths, the two read-compat sites, one `$` deck comment, one historical citation, `vchain.production_level1_dir` by ruling); 29-file stale-test sweep `e1d993a`; meta `533959fc` ARCHITECTURE/PROVIDER/OPERATIONS/CHANGELOG. Lead reproduced **412 passed / 1 skipped** and **59 passed**; 193 → 48 literals. Journal J13 (the name `deep_water` — veto = mechanical re-run), J19 (41 pre-existing top-level `tests/` failures found by the sweep, none rename-related)) |
...
| Gate S | Phase S close sweep (firewalled auditor, `scratch/GATE-S-PHASE-DEFINITION.md` Z1–Z10) | ✅ **PASSED 2026-08-27 after 2 fixes** — Z1 every touched frozen-core file named in a brief; Z2 every architectural change traces to PA1/5/6/7/8/10 (+Q9/Q11); Z3 no existing physics constant altered (only `HANDOFF_BREAK_CLEARANCE_M`, `F_DRY`, `SET_WAVE_THRESHOLD_RHO`, the Consistency curves — all new); Z4 475 passed/1 skipped on the plan's sets; Z6 all four ADR amendments read Proposed; Z7 every new refusal path serves nothing; Z8 log/memory discipline held; Z9 (150-station transect, missing seam point, one-bin spectrum, box-edge cell) no crash/no silent value. **F1 HIGH → fixed** (marine `6c9dc15`): invariants 4 and 8 read the pre-restart handoff picks — a post-S12 convergence of transects onto one band station was invisible; now re-checked on the settled list inside the per-transect path (M9's own relocation pattern), KAT with two converging transects. **F2 LOW → fixed** (`23980e2`): S5 warm-start fixtures pre-dated the S8.1-B hook. Held (Z10): S0 gate, S12/S8.1 reality gates, seam row per cycle, buoy ledger continuity — until push |
| D1 | As-built doc re-sync + zero-drift audit (the predecessor's DOC-W-FINAL) | 🔄 **2026-08-27 — docs re-sync LANDED** (meta `fd30d233`…`de1f646a`, 8 commits: Gate M F2/F3 fixed; `handoffRestart`/`ww3.gridRebuiltAt` health blocks in OPERATIONS + API-MANUAL §19.7; the OPERATIONS-MANUAL chain-gate paragraph corrected (flag gates only the buoy-ledger writers — the docstring fixed lead-direct marine `5c0cb21`); API-MANUAL §12b + ADR-078 A2 "not yet shipped" → shipped; DESIGN-MANUAL radar dark source named; PROVIDER-MANUAL working-directory paragraph no longer describes a SWAN L1 subprocess; contract `SpectralWaveComponent` gains the 7 S2 fields; CHANGELOG consolidated commit ranges + 9 missing round entries). Report-only: J12 drift measured 591/538 lines (dashboard copy lacks the basemap + setup paths), J15 dead Pydantic model. No ADR status flips. **Firewalled zero-drift auditor running** (`scratch/GATE-D1-DEFINITION.md` X1–X13) |
