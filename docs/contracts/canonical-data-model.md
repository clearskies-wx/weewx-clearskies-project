...
- **IQAir nearest-city endpoint** Community-tier returns only AQI (`aqius`) + dominant pollutant code (`mainus`) on `data.current.pollution`; **no per-pollutant concentrations** at the free tier (the `pollution` block is `{ts, aqius, mainus, aqicn, maincn}` only). Paid Startup+ tier adds per-pollutant concentrations on the same `/v2/nearest_city` endpoint per IQAir's plans page (wire field names unverified at draft — see `docs/reference/api-docs/iqair.md`). For v0.1, IQAir CAPABILITY enumerates only the verified free-tier fields; pollutant concentration fields stay PARTIAL-DOMAIN (categorical, not tier-conditional null) until paid-tier real-capture lands.
