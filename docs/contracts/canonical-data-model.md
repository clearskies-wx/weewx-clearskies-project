...
**Status:** Draft (Phase 1 deliverable; companion to [`openapi-v1.yaml`](openapi-v1.yaml) and [ADR-010](../archive/decisions/ADR-010-canonical-data-model.md))
**Last updated:** 2026-05-05

This document is the exhaustive catalog [ADR-010](../archive/decisions/ADR-010-canonical-data-model.md) defers and that [`openapi-v1.yaml`](openapi-v1.yaml) deliberately does not duplicate. Three things live here that no other artifact owns:
...
2. **Per-field unit mapping** for each weewx `target_unit` system (US / METRIC / METRICWX) — populates the `units` block every API response embeds per [ADR-019](../archive/decisions/ADR-019-units-handling.md).
3. **Provider → canonical mapping tables** for every day-1 provider in [ADR-007](../archive/decisions/ADR-007-forecast-providers.md), [ADR-013](../archive/decisions/ADR-013-aqi-handling.md), [ADR-016](../archive/decisions/ADR-016-severe-weather-alerts.md), [ADR-040](../archive/decisions/ADR-040-earthquake-providers.md), [ADR-015](../archive/decisions/ADR-015-radar-map-tiles-strategy.md).
...
Recap of [ADR-010](../archive/decisions/ADR-010-canonical-data-model.md) §Decision; this section is a one-paragraph pointer, not a re-derivation.
...
Every API response embeds a `units` object mapping each canonical field name to a unit string. The unit reflects whatever weewx is configured to write that field in (its `[StdConvert] target_unit`). The server does not convert per [ADR-019](../archive/decisions/ADR-019-units-handling.md).
...
When weewx emits a field that doesn't appear above (operator-added column not yet promoted via [ADR-035](../archive/decisions/ADR-035-user-driven-column-mapping.md)), the field appears under `extras` and the `units` block carries no entry for it. The dashboard renders extras in a generic "Other observations" panel labeled by column name; per-extras unit metadata is acquired via the ADR-035 promotion flow.
...
**Provenance.** The first-class field set above mirrors `STOCK_COLUMN_MAP` in `weewx_clearskies_api/db/reflection.py` (the stock-column lookup table loaded at startup by the schema reflector per [ADR-012](../archive/decisions/ADR-012-database-access-pattern.md) + [ADR-035](../archive/decisions/ADR-035-user-driven-column-mapping.md)). When weewx adds a new stock column, both the lookup table and this section get a new entry; `extras` is reserved for operator extensions. Columns the operator's archive schema doesn't include surface as `null` — the JSON key is always present.
...
| `forecastText` | string | Yes | — | NWS-style forecast narrative text for this period. Generated by the GFE text engine for non-NWS providers; pass-through of `detailedForecast` for NWS provider. Null when hourly data is insufficient for generation. |
...
One earthquake event from the configured earthquake provider per [ADR-040](../archive/decisions/ADR-040-earthquake-providers.md).
...
AQI snapshot. EPA 0–500 scale per [ADR-013](../archive/decisions/ADR-013-aqi-handling.md).
...
| `pollutantSubIndices` | object | Yes | — | Dict of canonical pollutant id → numeric sub-AQI on the same scale as `aqi`. Keys: `"PM2.5"`, `"PM10"`, `"O3"`, `"NO2"`, `"SO2"`, `"CO"`. `null` when provider doesn't supply (IQAir free tier, weewx Path A). |
...
Station identity. One per deploy per [ADR-011](../archive/decisions/ADR-011-multi-station-scope.md).
...
| `timezone` | string | No | — | IANA TZ identifier per [ADR-020](../archive/decisions/ADR-020-time-zone-handling.md). |
...
### 4.1 Forecast providers (per [ADR-007](../archive/decisions/ADR-007-forecast-providers.md))
...
### 4.2 AQI providers (per [ADR-013](../archive/decisions/ADR-013-aqi-handling.md))
...
| `aqiCategory` | `periods[].category` | (derive from `us_aqi` via EPA bands) | (derive from EPA bands after conversion) | (derive from `aqius` via EPA bands) |
...
| `pollutantPM25` | `periods[0].pollutants[]` where `type=="pm2.5"` → `valueUGM3` (µg/m³) | `current.pm2_5` (µg/m³) | `list[0].components.pm2_5` (µg/m³) | — (paid Startup+ tier; wire path unverified) |
...
| `pollutantSubIndices` | `periods[0].pollutants[].aqi` keyed by canonical id | `current.us_aqi_*` or `current.european_aqi_*` keyed by canonical id | — | `pollution.{p2,p1,o3,n2,s2,co}.aqius` or `.aqicn` keyed by canonical id (Startup+ tier; null on free tier) |
...
- **µg/m³ → ppm conversion** for O3/NO2/SO2/CO: `ppm = µg/m³ × 24.45 / (molecular_weight × 1000)`, equivalent to `ppm = µg/m³ × 0.02445 / molecular_weight`. 24.45 L/mol is the molar volume of an ideal gas at 25°C and 1 atm. The `/1000` factor is required because `µg/m³ × 24.45 / MW` alone yields ppb (parts per billion), not ppm. (Pre-2026-05-11 versions of this footnote omitted the `/1000` factor; the impl was correspondingly off-by-1000 — corrected in 3b-11 round close after the OWM real-capture fixture surfaced an absurd "Hazardous" AQI for a normal-air Seattle reading.) Per-pollutant molecular weights (O3 = 48.00, NO2 = 46.01, SO2 = 64.07, CO = 28.01 g/mol) live in `weewx_clearskies_api/providers/aqi/_units.py`.
...
- **IQAir nearest-city/nearest-station endpoint** Community-tier returns only AQI (`aqius`) + dominant pollutant code (`mainus`) on `data.current.pollution`; **no per-pollutant concentrations** at the free tier (the `pollution` block is `{ts, aqius, mainus, aqicn, maincn}` only). Paid Startup+ tier adds per-pollutant objects keyed by code (`p2`, `p1`, `o3`, `n2`, `s2`, `co`), each with `conc` (µg/m³), `aqius` (US EPA sub-index), `aqicn` (China MEP sub-index). **Not all 6 pollutants always present** — only those with data at the station appear. All concentrations are µg/m³ (verified from `data.units` block in real Startup-tier response, 2026-06-22). IQAir CAPABILITY now declares all concentration fields + `pollutantSubIndices`; operator_notes documents Startup+ requirement.
- **Operator's own AQI extension (Path A)** — column names operator-defined; mapping happens at setup via [ADR-035](../archive/decisions/ADR-035-user-driven-column-mapping.md). Common patterns: `aqi_value`, `pm2_5`, `pm10_0`, `o3` — but the operator can name them anything and maps them to canonical fields explicitly.

### 4.3 Alert providers (per [ADR-016](../archive/decisions/ADR-016-severe-weather-alerts.md))
...
### 4.4 Earthquake providers (per [ADR-040](../archive/decisions/ADR-040-earthquake-providers.md))
...
### 4.5 Radar providers (per [ADR-015](../archive/decisions/ADR-015-radar-map-tiles-strategy.md))
...
Concrete config for `weewx_clearskies_api/models/canonical.py`. Source: [ADR-010](../archive/decisions/ADR-010-canonical-data-model.md) §Implementation guidance, restated for completeness.
...
- **Display-side unit conversion / per-user override** — see [ADR-019](../archive/decisions/ADR-019-units-handling.md). Out of v0.1.
- **`extras` promotion UX** — see [ADR-035](../archive/decisions/ADR-035-user-driven-column-mapping.md).
...
- **Multi-station** — see [ADR-011](../archive/decisions/ADR-011-multi-station-scope.md). Single-station only at v0.1.
...
1. **A new canonical field is added** (e.g. wview_extended column promoted out of `extras` after dashboard demand). Updates: add a row to the entity catalog (§3), add a row to the unit-system table (§2.1) if unit-bearing, add columns to relevant provider mapping tables (§4) where they supply the new field. OpenAPI schema gets a corresponding non-breaking minor bump per [ADR-018](../archive/decisions/ADR-018-api-versioning-policy.md).
...
This document is co-authoritative with [ADR-010](../archive/decisions/ADR-010-canonical-data-model.md) and [`openapi-v1.yaml`](openapi-v1.yaml). Changes here that conflict with either need to be resolved before merging — the ADR/OpenAPI win on architecture, this spec wins on per-field unit and provider-mapping detail.
