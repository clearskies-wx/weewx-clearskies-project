...
> **Amended 2026-06-26** — Radar provider set revised. LibreWxR added as optional provider (Caddy-proxied, not API-proxied). RainViewer stays as default but degraded (zoom 7, no nowcast, single color scheme since Jan 2026). Aeris dropped from radar domain — 3,000 map units/day unviable for radar tiles (retained for forecast/AQI/alerts). NOAA direct (WMS-T via IEM/MRMS) dropped — raw imagery too noisy, would require building post-processing that LibreWxR already provides (see WMS-T research at `docs/reference/wms-t-rendering-research.md` §8, ADR-074). Expanded radar view added — full-viewport overlay at `/radar` with animation controls, color scheme picker, alert overlays, opacity control. Caddy proxy routing model: Caddy routes `/librewxr/*` to the LibreWxR instance (tiles + alerts); API provides metadata/capabilities only, never touches tile traffic. `iem_nexrad` and `noaa_mrms` deprecated (log migration warning) but modules retained on disk. `openweathermap` and `iframe` unchanged. `dwd_radolan` and `msc_geomet` unchanged (not added to wizard — regional providers, separate scope).
