...
12. **Never run `chown`, `chmod`, or any ownership/permission change on production containers.** File ownership on weewx and weather-dev is set up once at container standup per OPERATIONS-MANUAL.md §11. If a command fails with a permission error, the root cause is running as the wrong user — use the deploy scripts (`scripts/deploy-api.sh`, `scripts/redeploy-weather-dev.sh`) which handle user-switching correctly via `sudo -u ubuntu`. Running `chown` to fix a symptom creates ownership drift that breaks the next deploy and violates the filesystem permissions model. **Why (2026-07-08):** An agent ran `git pull` as the `claude` SSH user instead of `sudo -u ubuntu`, got EACCES on `.git/FETCH_HEAD`, then ran `sudo chown -R` on the entire repo and web root — producing hundreds of errors against the read-only webcam bind-mount and leaving files owned by the wrong user.
